...
class SessionDraftDeletionTest {
    ...
    @Test fun sessionEngineDraftRestoresAndCapturedCreateCannotClearANewerChoice() = runTest {
        ...
        try {
            ...
            val owner = journalCodingProjects(fixture.kv, fixture.json, fixture.chatJournal, Dispatchers.Main, projects)
            val controlled = object : CodingProjectOwner by owner {
                override suspend fun dispatch(projectId: String, input: CodingMachine.Input): CodingMachine.Transition {
                    if (input is CodingMachine.Intent.CreateSession) { entered.complete(Unit); release.await() }
    ...
    @Test fun failedSessionCreationKeepsTheEngineDraftAndCanBeRetried() = runTest {
        ...
        try {
            ...
            val controlled = object : CodingProjectOwner by owner {
                override suspend fun dispatch(projectId: String, input: CodingMachine.Input): CodingMachine.Transition {
                    if (fails && input is CodingMachine.Intent.CreateSession) error("private-backend-token")
                    return owner.dispatch(projectId, input)
                }
            }
