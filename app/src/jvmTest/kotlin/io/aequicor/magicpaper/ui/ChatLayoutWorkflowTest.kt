...
class ChatLayoutWorkflowTest {
    @Test fun acceptedRequestCapturesProjectAndRestoreNeverLaunchesEditor() = runTest {
        ...
        fun service() = DefaultChatService(testGatewayRuntime(gateway, fixture.search, EmbeddedDocRepository()),
            fixture.newChatStore(), fixture.settings, fixture.profiles, null, workerDispatcher = Dispatchers.Main,
            responseExtensions = listOf(LayoutChatExtension(LayoutChatAgent(gateway, editor),
                project = { id -> if (id == null) selected else listOf(first, second).find { it.id == id } })))
        var chat = service()
        try {
