...
class QuestionnaireViewModelTest {
    ...
    @Test fun arrivalDoesNotPreemptDraftAndDoubleConfirmationSendsOneReply() = runTest {
        ...
        val vm = f.prepareCoding(runtime, repo)
        try {
            runCurrent()
    ...
    @Test fun leaveStoppedIsDurableAndDoesNotInventAnotherUnansweredUserMessage() = runTest {
        ...
        try {
            ...
            assertEquals(listOf("old"), repo.messages("p", "s").map { it.id })
            assertEquals(CodingMachine.Phase.UNKNOWN, vm.state.value.coding.currentSession!!.runPhase)
        } finally { vm.shutdownCoding() }
        val restored = f.prepareCoding(Runtime(), repo)
        try {
            runCurrent()
            assertTrue(restored.state.value.coding.interactions.isEmpty())
            assertTrue(restored.state.value.coding.currentSession!!.canResume)
            assertEquals(CodingMachine.Phase.UNKNOWN, restored.state.value.coding.currentSession!!.runPhase)
            assertEquals(listOf("old"), repo.messages("p", "s").map { it.id })
        }
