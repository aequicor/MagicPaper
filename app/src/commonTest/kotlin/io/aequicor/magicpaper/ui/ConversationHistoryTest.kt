...
class ConversationHistoryTest {
    ...
    @Test fun failedHistoryWriteKeepsMessagesAndDoesNotLaunchEditedRequest() = runTest {
        ...
        val repo = object : CodingProjectRepository by stored {
            override suspend fun replaceHistory(projectId: String, sessionId: String, expected: List<CodingMessage>, messages: List<CodingMessage>) { error("disk unavailable") }
        }
        val runtime = Runtime(); val service = f.prepareCoding(runtime, repo)
        try {
            runCurrent()
            ...
            assertEquals("", stored.sessions("p").single().piSessionId)
        } finally { service.close(); Dispatchers.resetMain() }
    }
    ...
    @Test fun finishingChatDuringForkDoesNotStrandAcceptedQueue() = runTest {
        ...
        val repo = object : ChatRepository by f.chats {
        ...
        val runtime = Runtime().apply { gate = CompletableDeferred() }
        val service = DefaultChatService(runtime, repo, f.settings, f.profiles, null, workerDispatcher = Dispatchers.Main)
        try {
            service.start(); service.activate("first")
