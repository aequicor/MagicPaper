...
class ChatComposerAttachmentRenderTest {
    @Test fun errorPreviewDoesNotBlockImageOnlySendAndSessionChangeDropsOldDraft() {
        ...
        ImageComposeScene(680, 220) { MagicPaperTheme { Surface {
            ...
            render(16_000_000L)
            assertTrue(nodes().none { it.config.getOrNull(SemanticsProperties.ContentDescription) == listOf("Показать параметры") },
                "The composer must not offer an empty options panel")
