...
fun ChatScreen(vm: DefaultChatComponent, state: ChatState) {
    ...
    val question = state.current?.let { session -> session.pendingRun?.let { pending ->
        requests.firstOrNull { (it.sessionId == session.id || it.ownerSessionId == session.id) && it.runId == pending.runId }
    } }
    ResearchWorkspace(vm, state) {
        PaperResearchReading {
            MessagesList(state.current, state.busy, draft = draft, modifier = Modifier.fillMaxSize(),
                ...
                onPause = vm::pause, onResume = { vm.resume("", emptyList()) }, resumeAction = ResearchResumeAction.CHECK_SAVED_RESPONSE, footer = {
                    if (state.current?.pendingRun != null && !state.busy) PaperAction(vm::discardPendingRequest) {
                        PaperText("Оставить без продолжения", role = PaperTextRole.LABEL)
                    }
                    ChatInputSurface(question, question?.let { answerDrafts[it.id] },
                        onDraft = { question?.let { current -> vm.updateQuestionnaireDraft(current.id, it) } },
                        onSubmit = { question?.let { current -> vm.submitQuestionnaire(current.id, it) } }) {
                    Composer(
                        ...
                        onPause = vm::pause,
                        onResume = vm::resume,
                        resumeAction = ResearchResumeAction.CHECK_SAVED_RESPONSE,
...
@Composable
internal fun ChatInputSurface(
    request: UserInteractionRequest?, draft: QuestionnaireDraft?,
    onDraft: (QuestionnaireDraft) -> Unit, onSubmit: (List<PlanningAnswer>) -> Unit,
    composer: @Composable () -> Unit,
) {
    if (request == null) composer()
    else UserInteractionDock(request, draft ?: QuestionnaireDraft(request.initialAnswers), onDraft, onSubmit,
