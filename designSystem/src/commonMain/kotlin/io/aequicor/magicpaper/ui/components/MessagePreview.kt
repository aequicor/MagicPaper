...
fun PaperMessagePreview(
    ...
    val expansion = LocalPaperMessageExpansion.current?.takeIf { it.source == source }
    Column(modifier) {
        if (expanded) {
            reader(Modifier.fillMaxWidth())
            PaperCollapseMessage { expanded = false }
        } else Box {
            ...
            if (clipped) PaperText("Читать далее",
                Modifier.align(Alignment.BottomEnd).paperChatDisclosure {
...
@Composable
fun PaperChatPlainText(text: String, modifier: Modifier = Modifier, style: TextStyle = paperTextStyle(PaperTextRole.BODY),
    ...
    PaperMessagePreview(text, previewText.length < text.length, modifier, preview = {
