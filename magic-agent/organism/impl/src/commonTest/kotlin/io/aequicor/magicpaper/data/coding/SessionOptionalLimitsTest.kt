...
class SessionOptionalLimitsTest {
    private class Fixture(val limits: OrganismLimits = OrganismLimits()) {
        ...
        val store = DefaultSessionOrganismStore(storage, journal) { now }
    ...
    @Test fun liveLimitChangesKeepActualSpendAndNeverRestoreStoppedWork() = runTest {
        ...
        assertEquals(unbounded, DefaultSessionOrganismStore(f.storage, f.journal) { f.now }.get(unbounded.id))
    }

    @Test fun legacyHiddenLimitsMigrateOnceAndStoppedPlanStillNeedsFreshExplicitRetry() = runTest {
        val storage = InMemoryKeyValueStore()
        val journal = InMemoryEventJournal()
        val store = DefaultSessionOrganismStore(storage, journal) { 1_000L }
        ...
        val saved = store.adopt("project", root, listOf(child), OrganismLimits(activeSessions = 8, depth = 6, tokens = 1_000_000, recoveryTokens = 10_000, durationMillis = 3_600_000, retries = 3, queueSize = 128, contextCharacters = 64_000))
        ...
        val reloaded = DefaultSessionOrganismStore(storage, InMemoryEventJournal()) { 20_000_000L }
