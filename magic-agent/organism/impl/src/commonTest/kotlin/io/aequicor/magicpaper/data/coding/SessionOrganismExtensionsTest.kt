...
class SessionOrganismExtensionsTest {
    private class Fixture {
        ...
        var secrets: Set<String> = emptySet()
        val store = DefaultSessionOrganismStore(storage, journal, knownSecrets = { secrets }) { 1_000 }
    ...
    @Test fun committedOrganismIsDiscoverableWithoutLegacyProjectionAndModeChangeRevokesOldAuthority() = runTest {
        val f = Fixture(); f.init()
        val old = f.scope()
        assertEquals(listOf(f.organism.id), DefaultSessionOrganismStore(f.storage, f.journal).loadAll().map { it.id })
    ...
    @Test fun secretsAreMaskedInStoredTasksContextAndResultsWithoutCollapsingOperationIdentity() = runTest {
        val f = Fixture(); f.init()
        f.secrets = setOf("configured-secret-value")
