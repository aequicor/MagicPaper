 ...
 * `--no-optional-locks`, `GIT_OPTIONAL_LOCKS=0`, пустым `core.hooksPath` и закалёнными исполняемыми
 * ключами конфига (`SandboxCheckDriver.hardenedGitConfiguration`); системный и глобальный конфиг
 * пользователя остаются в силе, потому что решают форму данных репозитория.
 * Он не опирается на ограничение записи, которое доказывает probe,
