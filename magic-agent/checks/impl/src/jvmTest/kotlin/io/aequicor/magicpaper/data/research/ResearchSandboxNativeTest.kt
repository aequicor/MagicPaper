...
class ResearchSandboxNativeTest {
    ...
    @Test fun nativeRunnerReadsAndBuildsButCannotWriteSourcesOrGit() = runBlocking {
        ...
        try {
            ...
            val runner = NativeCheckFixture(root.resolve("runtime"))
    ...
    @Test fun repeatChecksPreserveNewIgnoredFilesAndReleaseTemporaryCaches() = runBlocking {
        ...
        try {
            ...
            val next = NativeCheckFixture(root.resolve("runtime"))
    ...
    @Test fun unixDetachedProcessesCannotEscapeCheckLifetime() = runBlocking {
        ...
        try {
            ...
            val result = NativeCheckFixture(root.resolve("runtime")).run(project, "s", listOf(binary.toString()))
            assertNull(result.blockedReason, result.toString())
    ...
    @Test fun applicationCrashStopsCommandWithoutInventingDurableCleanup() = runBlocking {
        assumeTrue(enabled)
...
private class NativeCheckFixture(root: Path, sandbox: () -> ResearchSandbox = { ResearchSandbox.current() }) {
    ...
    suspend fun run(project: Path, sessionId: String, arguments: List<String>): CheckResult = coroutineScope {
        ...
        val command = CheckCommand(CheckRef(CheckScope("fixture", sessionId, requestId, 0), "check"), project.toString(), arguments)
        val observer = launch(start = CoroutineStart.UNDISPATCHED) {
            owner.progress.collect { if (it.ref == command.ref) progress.value = CheckResult(it.output, null) }
        }
        try { owner.run(command) }
