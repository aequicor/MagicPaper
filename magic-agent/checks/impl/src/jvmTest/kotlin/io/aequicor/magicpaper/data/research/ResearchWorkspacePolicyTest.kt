...
class ResearchWorkspacePolicyTest {
    ...
    @Test fun gitWorktreeMetadataAndTrackedArtifactsStayProtected() {
        ...
        try {
            ...
            // Policy tests receive values; the owned native acquisition of them has separate integration coverage.
            val metadata = ResearchGitMetadata.decode(worktree, mapOf(
                io.aequicor.magicpaper.domain.checks.CheckGitMetadataQuery.TRACKED_FILES to "package.json\u0000dist/data.txt\u0000".encodeToByteArray(),
                io.aequicor.magicpaper.domain.checks.CheckGitMetadataQuery.GIT_DIRECTORY to git(worktree, "rev-parse", "--absolute-git-dir").encodeToByteArray(),
                io.aequicor.magicpaper.domain.checks.CheckGitMetadataQuery.COMMON_DIRECTORY to git(worktree, "rev-parse", "--git-common-dir").encodeToByteArray(),
            ))
            assertFailsWith<IllegalArgumentException> { ResearchWorkspacePolicy.inspect(worktree, scratch) }
            val policy = ResearchWorkspacePolicy.inspect(worktree, scratch, metadata = metadata)
