...
internal data class ResearchWorkspacePolicy(
    ...
    companion object {
        ...
        fun metadataOnly(project: Path, scratch: Path): ResearchWorkspacePolicy {
            val source = project.toRealPath()
            val temporary = scratch.toRealPath()
            require(!temporary.startsWith(source) && !source.startsWith(temporary)) { "Служебная папка должна быть вне проекта" }
            return ResearchWorkspacePolicy(source, listOf(temporary), listOf(source), emptyList())
        }

        fun inspect(projectPath: Path, scratch: Path, artifacts: Map<Path, String> = emptyMap(),
            metadata: ResearchGitMetadata? = null): ResearchWorkspacePolicy {
            ...
            // Metadata is supplied only by the journaled process owner; policy discovery starts no process.
            if (hasRepository(project)) {
                requireNotNull(metadata) { "Сведения Git для защиты файлов не подтверждены" }
                require(metadata.project == project) { "Сведения Git относятся к другой папке" }
                tracked.addAll(metadata.tracked)
                protected.addAll(metadata.directories)
            }
        ...
        fun hasRepository(project: Path): Boolean =
            generateSequence(project) { it.parent }.any { Files.exists(it.resolve(".git"), LinkOption.NOFOLLOW_LINKS) }
