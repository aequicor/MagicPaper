...
internal class ResearchCheckBridge(private val session: CodingSession, private val project: CodingProject,
    private val checks: CommandChecks, requestId: String) : AutoCloseable {
    private val scope = CheckScope(project.id, session.id, requestId, session.runtimeGeneration).also {
        require(project.id == session.projectId && requestId.isNotBlank())
    }
    ...
    private data class Request(val method: String?, val params: JsonObject, val response: CompletableDeferred<JsonObject>)
    private val results = ConcurrentHashMap<JsonElement, Request>()
    ...
    init {
        ...
        server.createContext("/mcp") { exchange ->
            try { handle(exchange) } catch (failure: Exception) {
                report("bridge.request.failed", failure)
                try { reply(exchange, 500) } catch (disconnected: Exception) { report("bridge.reply.failed", disconnected) }
            }
    ...
    private fun handle(exchange: HttpExchange) {
        ...
        val request = try { Json.parseToJsonElement(bytes.decodeToString()) as? JsonObject }
            catch (_: IllegalArgumentException) { null } // Invalid wire JSON is a rejected request, not an operational failure.
        ...
        val result = runBlocking {
            ...
            val previous = results.putIfAbsent(id, Request(method, params, response))
            if (previous != null) return@runBlocking if (previous.method == method && previous.params == params) previous.response.await()
                else error(id, "Идентификатор проверки уже использован с другим запросом")
            ...
            val value = try {
                ...
                val payload = when (method) {
                    ...
                    "tools/call" -> {
                        ...
                        // JSON spelling preserves string/numeric JSON-RPC identities without inventing a new call ID.
                        val result = checks.run(CheckCommand(CheckRef(scope, id.toString()), project.path, command,
                            args["cwd"]?.jsonPrimitive?.content ?: ".", CheckPolicy.PROTECTED_PROJECT))
                ...
                response.complete(error(id, "Проверка отменена"))
                throw cancelled
            } catch (failure: Exception) {
                report("bridge.check.failed", failure)
                error(id, if (failure is CheckOutcomeUnknown) "Исход проверки неизвестен. Повтор заблокирован."
                    else "Не удалось выполнить проверку. Проверьте команду и состояние сессии.")
    ...
    override fun close() {
        ...
        listOf<() -> Unit>({ checks.abort(session.id) }, { server.stop(0) }, { executor.shutdownNow(); Unit }).forEach { cleanup ->
    ...
    private fun error(id: JsonElement, message: String) = buildJsonObject {
        put("jsonrpc", "2.0"); put("id", id)
        putJsonObject("error") { put("code", -32602); put("message", message) }
    }
    private fun report(event: String, failure: Exception) {
        val fields = mapOf("projectId" to scope.projectId, "sessionId" to scope.sessionId,
            "requestId" to scope.requestId, "causeType" to failure::class.simpleName.orEmpty())
        if (failure is CancellationException) AppLog.info("research", "bridge.cancelled", fields)
        else AppLog.error("research", event, fields)
    }
