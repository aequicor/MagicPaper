...
internal class OwnedCodingProcess(private val directory: File) : io.aequicor.magicpaper.backend.NativeProcessRecovery,
    io.aequicor.magicpaper.backend.NativeProviderProcesses {
    ...
    override fun clear(id: String) {
        val receipt = file(id)
        check(receipt.delete() || !receipt.exists()) { "Не удалось очистить запись процесса" }
    }
    override fun reconcileOrphans() {
        if (!directory.exists()) return
        val receipts = checkNotNull(directory.listFiles()) { "Не удалось прочитать записи процессов" }
        receipts.filter { it.extension == "process" }.forEach { receipt ->
            val values = receipt.readLines().map { it.toLongOrNull() }
            require(values.size == 4 && values.all { it != null }) { "Не удалось прочитать владельца процесса" }
            val owner = ProcessHandle.of(checkNotNull(values[2])).orElse(null)?.takeIf {
                it.isAlive && it.info().startInstant().orElse(null)?.toEpochMilli() == values[3]
            }
            // Another live application/bundle still owns its attempts; restore never borrows its authority.
            if (owner == null) reconcile(receipt.nameWithoutExtension)
        }
    }
    override fun belongsTo(id: String, process: Process?): Boolean {
