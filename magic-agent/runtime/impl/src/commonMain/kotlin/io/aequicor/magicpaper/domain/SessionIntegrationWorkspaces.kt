...
class SessionIntegrationWorkspaces(
    ...
    private val retained = mutableMapOf<String, List<WorkspaceLease>>()
    ...
    suspend fun integrate(project: CodingProject, initial: SessionIntegration, inputs: List<SessionIntegrationInput>,
        sourceLease: WorkspaceLease? = null, checkpoint: suspend (SessionIntegration) -> Unit): SessionIntegration {
        ...
        val owned = mutableListOf<WorkspaceLease>()
        ...
        try {
            if (sourceLease == null) {
                ...
                withContext(NonCancellable) {
                    owned += checkNotNull(workspaces.acquire(owner, Id.new())) { "Исходная рабочая копия занята" }
                }
            }
            val sourceHandle = checkNotNull(sourceLease ?: owned.singleOrNull())
            fun sourceOperation(step: String) = WorkspaceOperation(sourceHandle, "integration:${request.id}:$step")
            currentCoroutineContext().ensureActive()
            require(workspaces.verificationSnapshot(project.path, sourceOperation("source-before")) == request.sourceSnapshot) { "Исходники изменились до интеграции" }
            ...
            val workspace = preparation.withLock { workspaces.prepare(project, "session-integration-${request.id}", sourceOperation("prepare")) }
            ...
            require(workspaces.verificationSnapshot(project.path, sourceOperation("source-prepared")) == request.sourceSnapshot) { "Исходники изменились при подготовке интеграции" }
            val owner = project.copy(id = "session-integration-${request.id}", path = workspace.integrationPath)
            withContext(NonCancellable) {
                owned += checkNotNull(workspaces.acquire(owner, Id.new())) { "Рабочая копия интеграции занята" }
            }
            val executionLease = owned.last()
            fun executionOperation(step: String) = WorkspaceOperation(executionLease, "integration:${request.id}:$step")
            ...
            for (input in inputs) {
                ...
                if (!workspaces.integrate(workspace, input.workspace.attempt, executionOperation("merge:${input.result.id}"))) {
            ...
            if (record.phase != SessionIntegrationPhase.CONFLICT) {
                workspaces.validateIntegration(workspace, executionOperation("validate-before"))
                val snapshot = workspaces.verificationSnapshot(workspace.integrationPath, executionOperation("snapshot-before")) ?: error("Снимок интеграции недоступен")
                ...
                workspaces.validateIntegration(workspace, executionOperation("validate-after"))
                val actual = workspaces.verificationSnapshot(workspace.integrationPath, executionOperation("snapshot-after"))
                ...
                require(workspaces.verificationSnapshot(project.path, sourceOperation("source-final")) == request.sourceSnapshot) { "Исходники изменились во время итоговой проверки" }
                if (!acceptance.permitsProgress) save(record.copy(phase = SessionIntegrationPhase.BLOCKED, error = safe(acceptance.summary())))
                    ...
                    val sha = workspaces.capture(StageAttempt("session-integration-${request.id}-final", request.actorSessionId,
                        ...
                        report = "Объединённый результат ${request.id}", verificationSnapshot = snapshot), executionOperation("capture"))
                    require(sha.isNotBlank() && workspaces.verificationSnapshot(workspace.integrationPath, executionOperation("snapshot-captured")) == snapshot) { "Результат изменился при фиксации интеграции" }
            ...
            withContext(NonCancellable) {
                ...
                val remaining = mutableListOf<WorkspaceLease>()
