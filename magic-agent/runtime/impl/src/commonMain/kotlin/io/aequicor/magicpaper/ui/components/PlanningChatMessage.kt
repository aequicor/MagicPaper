...
@Composable internal fun PlanningChatMessage(message: CodingMessage, session: CodingSession, history: List<CodingMessage>, service: PlanningChatService, snapshot: CodingPlanningState, onOpenSession: (String) -> Unit) {
    ...
    val run = snapshot.runs[source.id] ?: PlanningRunUi()
    ...
    Column(Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        display.selectedMilestones.forEach { stage ->
            Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                ...
                if (stage.description.isNotBlank()) PaperChatMarkdown(stage.description)
        ...
        FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (plan.confirmedRevision == null) {
                ...
                if (plan.phase != ExecutionPhase.COMPLETE) {
                    PaperButton(if (run.active) "Пауза" else "Продолжить", {
                        if (!run.active && plan.blockingIssues(history).isNotEmpty()) openQuestionnaire(InteractionKind.RECOVER_PLAN, plan.id)
                        else service.control(plan.id, if (run.active) "pause" else "resume")
                    }, enabled = run.active || run.canContinue, kind = PaperButtonKind.SECONDARY)
