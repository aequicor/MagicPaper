...
internal fun interactionCandidates(
    ...
    plans.forEach { plan ->
        ...
        history.filter { it.planning?.planId == plan.id && it.planning?.questions?.isNotEmpty() == true &&
        ...
        if (blockers.isNotEmpty()) {
            ...
            add(recoveryInteraction("blocker:${blockers.map { it.messageId }.sorted().joinToString(":")}", parent,
                ...
                .copy(planId = plan.id, affectedSessionIds = (affected + parent.id).toSet(),
                    verificationProofs = blockers.takeIf { all -> all.all { it.canSkipVerification } }
                        ?.map { checkNotNull(it.verificationProof) }?.toSet()))
        }
    ...
    ui.sessions.filter { !it.session.archived && !it.running && it.plan == null && !it.session.planningMode && it.session.stageId == null }.forEach { item ->
        ...
        if (item.runPhase != CodingMachine.Phase.UNKNOWN && checkpoint?.hasSuccessfulResponse(item.messages) == true && item.session.taskWorktree?.let { it.phase != TaskWorktreePhase.COMPLETE } != true) return@forEach
