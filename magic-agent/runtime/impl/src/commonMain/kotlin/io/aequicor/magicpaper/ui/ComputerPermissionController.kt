...
internal class ComputerPermissionController(private val port: ComputerPermissions, private val scope: CoroutineScope) {
    ...
    fun refresh(computer: ComputerAccess, application: ComputerAccess) {
        ...
        probe = scope.launch {
            try {
                ...
                val report = port.inspect(computer, application)
    ...
    fun open(permission: ComputerPermission) = action("open_settings") { port.openSettings(permission) }
    fun reveal(target: PermissionTarget) = action("reveal_target") { port.reveal(target) }
