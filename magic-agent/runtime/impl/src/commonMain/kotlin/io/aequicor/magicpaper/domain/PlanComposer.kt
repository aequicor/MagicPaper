...
class PlanComposer(
    ...
    suspend fun completePlanning(plan: Plan, profile: LlmProfile, messages: List<LlmMessage>, onActivity: (CodingStep) -> Unit): String {
        ...
        return withContext(UsageOwner(UsageScope(plan.parentSessionId.takeIf { it.isNotBlank() }?.let { "coding:$it" }, projectId = plan.projectId, planId = plan.id)) + PlanningRulesContext(plan.planningRulesSnapshot)) {
            if (toolSessions == null) return@withContext planningGateway.completeWithActivity(project, engine, plan.requestId.ifBlank { plan.id }, profile, messages, onActivity)
            ...
            supervisorScope {
                ...
                while (proposal == null) {
                    ...
                    val tools = toolSessions.session(turnContext, mapOf(
                        "questionnaire" to { ctx, id, arguments ->
                            val answer = requestScope.async { toolSessions.questionnaires.ask(ctx, id, arguments) }
        ...
        return withContext(tools + UsageOwner(UsageScope("coding:${tools.context.ownerSessionId}", projectId = plan.projectId, planId = plan.id)) + PlanningRulesContext(plan.planningRulesSnapshot)) {
            planningGateway.completeWithActivity(project, plan.engine ?: legacyCodingEngine(profile), tools.context.requestId, profile,
                messages + LlmMessage(LlmChatRole.SYSTEM, "Действия выполняй только через инструменты magicpaper_. " +
                    ...
                    "Не подтверждай и не возобновляй план без явного действия пользователя. " +
                    "Если пользователь поручил новый план взамен старого, вызови plan.refine с newPlan=true; обычный refine дорабатывает существующий. " +
                    "Проверяй status, planId, proposalId и confirmationReady в результате инструмента. unchanged означает, что предложение не создано. " +
                    "Рабочая копия создаётся после подтверждения: workspacePrepared=false у готового предложения не препятствует подтверждению. " +
                    "Явное согласие пользователя с готовым предложением, включая короткое «го», можно выполнить через plan.control action=confirm с актуальными revision и proposalId; не отправляй пользователя повторно подтверждать то же самое в интерфейсе."), onActivity)
        }
    }
