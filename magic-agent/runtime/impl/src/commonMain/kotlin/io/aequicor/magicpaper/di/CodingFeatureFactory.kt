...
fun codingFeature(deps: CodingFeatureDependencies): CodingFeature {
    ...
    val graph = CodingRuntimeGraph(deps.store, deps.json, deps.settings, deps.profiles, projects, deps.codingRuntime,
        ...
        sourceAccess = ResearchSourceAccess(deps.readResearchPage), mediaGeneration = deps.media,
        mediaAllowed = { context, kind -> mediaAllowed(deps, projects, context, kind) },
        orchestrationFactory = deps.orchestrationFactory, toolQuestions = deps.toolQuestionnaires,
