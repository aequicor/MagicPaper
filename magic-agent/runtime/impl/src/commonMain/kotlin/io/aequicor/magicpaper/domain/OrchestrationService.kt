...
/** A publication identity contains no exception message or provider payload. */
data class OrchestrationFailureEvent(val id: String)
...
class OrchestrationService(
    ...
    private val projects: CodingProjectOwner, private val profiles: LlmProfileRepository,
    ...
    private val toolSessions: ToolSessionFactory? = null,
    private val toolReceipts: ToolReceiptStore? = toolSessions?.receipts,
    ...
    val messageScheduler = MessageScheduler(store, this.scope, ::dispatchScheduledMessage,
        onFailure = ::publishError, now = clock,
        beforeTick = ::synchronizeQuestionEvents, hasReceipt = ::hasScheduledReceipt)
    ...
    private val liveDrafts = MutableStateFlow<Map<String, CodingDraft>>(emptyMap())
    val drafts: StateFlow<Map<String, CodingDraft>> = liveDrafts.asStateFlow()
    private var draftObserver: Job? = null
    private fun observeDrafts() {
        if (draftObserver?.isActive == true) return
        draftObserver = scope.launch {
            combine(_drafts, coordinatorActivity) { requests, coordinators ->
        ...
        combined
    }.flowOn(workerDispatcher).collect { liveDrafts.value = it }
        }
    }
    ...
    private val _failureEvents = MutableStateFlow<OrchestrationFailureEvent?>(null)
    val failureEvents: StateFlow<OrchestrationFailureEvent?> = _failureEvents.asStateFlow()
    private fun publishError(message: String?) {
        _error.value = message
        _failureEvents.value = message?.let { OrchestrationFailureEvent(Id.new()) }
        changed()
    }
    ...
    init {
        observeDrafts()
        organisms?.let { owner ->
            scope.launch { owner.projections.collect { changed() } }
            scope.launch { owner.failures.collect { failures ->
                failures.values.firstOrNull()?.let(::publishError)
            } }
        }
    }

    override fun knownSecrets(): Set<String> = toolSecrets
    override suspend fun checkScope(context: ToolExecutionContext, historical: Boolean) =
        withContext(scope.coroutineContext.minusKey(Job)) { checkToolScope(context, historical) }
    override suspend fun authorizeCommand(context: ToolExecutionContext, definition: ToolDefinition, arguments: JsonObject) =
        withContext(scope.coroutineContext.minusKey(Job)) { checkLegacyToolAuthority(context, definition, arguments) }
    override suspend fun reconcile(context: ToolExecutionContext, receipt: ToolReceipt): JsonElement? =
        withContext(scope.coroutineContext.minusKey(Job)) { reconcileTool(context, receipt) }
    override suspend fun execute(context: ToolExecutionContext, operation: String, tool: String, arguments: JsonObject): JsonElement =
        withContext(scope.coroutineContext.minusKey(Job)) { executeTool(context, operation, tool, arguments) }

    override suspend fun prepareWorker(session: CodingSession): ToolExecutionContext {
        toolProfiles()
        ...
        return when {
            plan != null && stage != null && attempt != null -> ToolExecutionContext(session.projectId, session.id, session.id,
                session.pendingRun?.runId?.takeIf { it.isNotBlank() } ?: "${attempt.id}-turn-${attempt.turnIndex}",
    ...
    private suspend fun reconcileTool(context: ToolExecutionContext, receipt: ToolReceipt): JsonElement? {
        if (receipt.toolId == "session.delete") {
            val args = json.decodeFromJsonElement<SessionDeleteArgs>(receipt.arguments)
            val organism = context.organismId?.let { organisms?.store?.get(it) }
            if (organism != null && args.sessionId in organism.historyDeletedIds &&
                projects.sessions(context.projectId).none { it.id == args.sessionId })
                return buildJsonObject { put("status", "deleted"); put("sessionId", args.sessionId) }
            return null
        }
        ...
        return when (receipt.toolId) {
            ...
            "plan.refine", "plan.recalculate" -> {
                ...
                if (saved.pendingRequest.isBlank() && saved.dialogue.any { it.id == "$operation-reply" }) {
                    if (saved.replacesPlanId == plan.id) updateState(saved.parentSessionId, saved.projectId, OrchestrationEvent.PlanSelected(saved.id))
    ...
    private suspend fun replacementDraft(context: ToolExecutionContext, previous: Plan, operation: String): Plan {
        ...
        store.planFor(id)?.let { saved ->
            ...
            updateState(previous.parentSessionId, previous.projectId, OrchestrationEvent.PlanSelected(saved.id))
            return saved
        }
        ...
        store.command(draft.id, PlanningMachine.Intent.Create(draft, PlanningMachine.Stamp(Id.new(), Id.now())))
        updateState(previous.parentSessionId, previous.projectId, OrchestrationEvent.PlanSelected(draft.id))
        return draft
    }
    ...
    private suspend fun executeTool(context: ToolExecutionContext, operation: String, tool: String, arguments: JsonObject): JsonElement {
        ...
        return when (tool) {
            "session.delete" -> {
                val args = json.decodeFromJsonElement<SessionDeleteArgs>(arguments)
                requireTool(context.mode == CodingInteractionMode.PLANNING && !context.auxiliaryExecution) { "Удаление доступно только планировщику" }
                val sessions = projects.sessions(context.projectId)
                val target = sessions.firstOrNull { it.id == args.sessionId }
                requireTool(target != null && target.parentSessionId == context.ownerSessionId) { "Можно удалить только непосредственную дочернюю сессию" }
                args.expectedVersion?.let { expected ->
                    val organism = context.organismId?.let { organisms?.store?.get(it) }
                    requireTool(organism?.version == expected) { "Состояние сессий изменилось" }
                }
                deleteSessionTree(context.projectId, args.sessionId)
                buildJsonObject { put("status", "deleted"); put("sessionId", args.sessionId) }
            }
            ...
            "receipt.get" -> {
                ...
                val receipt = toolReceipts?.get(args.receiptId) ?: error("Вызов не найден")
            ...
            "context.get" -> buildJsonObject {
                ...
                val previous = toolReceipts?.forRequest("${context.projectId}/${context.ownerSessionId}/${context.requestId}").orEmpty()
            ...
            "stage.pause" -> {
                ...
                val saved = updateState(context.ownerSessionId, context.projectId,
                    OrchestrationEvent.WorkPaused(key, OrchestrationPause(current.id, args.stageIds), mergeStages = true))
            ...
            "stage.handoff" -> {
                ...
                try {
                    updatePlan(current.id) { CoordinationEvent.HandoffSubmitted(CoordinationRecord(id,
                ...
                } catch (e: HandoffConflict) { throw ToolArgumentRejection(e.message!!) }
    ...
    private suspend inline fun <reified T> legacyTool(plan: Plan, request: String, call: String, name: String,
        ...
        val host = toolSessions ?: return action()
    ...
    private fun launch(block: suspend () -> Unit) = scope.launch {
        ...
        try { block() } catch (e: CancellationException) { throw e } catch (e: Exception) {
            AppLog.error("coding.orchestration", "command.failed", e)
            publishError((e as? SessionActionRejection)?.problem?.userMessage
                ?: "Не удалось выполнить действие. Проверьте состояние сессии и повторите.")
        }
    }
    ...
    suspend fun deleteProjectSessions(projectId: String) = historyDeletionLock.withLock {
        ...
        try {
            ...
            forms.remove(projectId)
            plans.forEach { store.deletePlan(it.id) }
            messageLock.withLock { projects.sessions(projectId).forEach { projects.dispatch(projectId, CodingMachine.Intent.DeleteSession(CodingMachine.ref(it))) } }
    ...
    suspend fun deleteSessionTree(projectId: String, sessionId: String): Set<String> = historyDeletionLock.withLock {
        ...
        try {
            ...
            forms.remove(projectId, ids)
            plans.forEach { store.deletePlan(it.id) }
            messageLock.withLock { ids.forEach { id -> projects.sessions(projectId).firstOrNull { it.id == id }?.let { projects.dispatch(projectId, CodingMachine.Intent.DeleteSession(CodingMachine.ref(it))) } } }
    ...
    suspend fun pauseForReset() {
        closing = true
        forms.revoke()
        messageScheduler.pauseForReset()
        val children = scope.coroutineContext[Job]?.children?.toList().orEmpty()
        children.forEach { it.cancel() }
        children.joinAll()
        sessionTree?.pauseForReset()
        organisms?.pauseForReset()
        jobs.clear()
        _states.value = emptyMap()
        _sessions.value = emptyList()
        _drafts.value = emptyMap()
        liveDrafts.value = emptyMap()
        coordinatorActivity.value = emptyMap()
        _unsavedInputs.value = emptyMap()
        _persistenceErrors.value = emptyMap()
        publishError(null)
        toolSecrets = emptySet()
        deletedPlans.clear()
        deletedSessions.clear()
        clearingProjects.clear()
        draftObserver = null
        ready = CompletableDeferred()
    }

    suspend fun resumeAfterReset() {
        sessionTree?.resumeAfterReset()
        closing = false
    ...
    suspend fun shutdown() {
        ...
        try { forms.flush() }
            ...
            forms.revoke()
            try { organisms?.shutdown() }
            finally { try { sessionTree?.shutdown() } finally { scope.coroutineContext[Job]?.cancelAndJoin() } }
        }
    }
    fun cancelRequest(sessionId: String) {
        val capturedPlan = store.plans.value.firstOrNull { it.parentSessionId == sessionId && it.pendingRequest.isNotBlank() }
        val capturedRefinement = capturedPlan?.let { store.machineStates.value[it.id]?.refinement?.ref }
        ...
        launch {
            ...
            val plan = capturedPlan ?: return@launch
            val input = capturedRefinement?.let { PlanningMachine.Intent.CancelRefinement(it, PlanningMachine.Stamp(Id.new(), Id.now())) }
                ?: PlanningMachine.Intent.DiscardLegacyRefinement(plan.requestId, PlanningMachine.Stamp(Id.new(), Id.now()))
            val cancelled = store.command(plan.id, input)
            if (cancelled.pendingRequest.isNotBlank()) return@launch
    ...
    suspend fun stopOwnedPlanRuns(rootId: String, caller: Job): Boolean {
        var confirmed = true
    ...
    private suspend fun publishQuestionEvents(saved: OrchestrationState) {
        store.plans.value.filter { it.parentSessionId == saved.sessionId }.forEach { p ->
            ...
            if (p.scheduleQuestionIds != questions || events.any { event -> p.messageEvents.none { it.id == event.id } }) updatePlan(p.id, CoordinationEvent.QuestionsObserved(events, questions))
        }
    }
    private suspend fun synchronizeQuestionEvents() {
        _states.value.values.toList().forEach { saved ->
            // Cached history can outlive its deleted parent while orphan workers remain visible.
            if (projects.sessions(saved.projectId).none { it.id == saved.sessionId }) return@forEach
            try {
                val next = updateState(saved.sessionId, saved.projectId) { OrchestrationEvent.ScheduledRunsObserved(store.plans.value) }
    ...
    private suspend fun dispatchScheduledMessage(plan: Plan, rule: ScheduledMessage): Boolean {
        ...
        if (plan.canDeliverScheduled(rule)) {
            require(projects.sessions(plan.projectId).none { it.id == rule.targetSessionId && it.archived }) { "Сессия адресата архивирована" }
            val queued = updatePlan(plan.id, CoordinationEvent.ScheduledDeliveryRequested(rule, parent.id))
        ...
        updateState(parent.id, plan.projectId, OrchestrationEvent.InputEnqueued(input))
    ...
    private suspend fun updateHandoff(planId: String, id: String, status: HandoffStatus, text: String) {
        val p = updatePlan(planId, CoordinationEvent.HandoffStatusRecorded(id, status, text))
    ...
    private suspend fun replaySessionCommands(session: CodingSession) {
        state(session.id, session.projectId).sessionCommands.filterNot { it.applied }.forEach { command ->
            store.planFor(command.planId)?.let { plan ->
                try { performSessionCommand(plan, command) }
                catch (cancelled: CancellationException) { throw cancelled }
                catch (failure: Exception) {
                    AppLog.error("coding.orchestration", "session-command.failed", fields = mapOf(
                        "planId" to plan.id, "sessionId" to session.id, "causeType" to failure::class.simpleName.orEmpty()))
                    publishError(failure.message ?: "Не удалось выполнить сохранённое действие сессии.")
                }
            }
        }
    }
    ...
    suspend fun append(projectId: String, sessionId: String, message: CodingMessage) = messageLock.withLock {
        ...
        if (next != history) { publishHistory(projectId, sessionId, next); changed() }
    }
    private suspend fun publishHistory(projectId: String, sessionId: String, messages: List<CodingMessage>) {
        ...
        projects.dispatch(projectId, CodingMachine.Fact.HistoryPublished(CodingMachine.ref(session), messages))
    }
    suspend fun configure(session: CodingSession, planning: Boolean = session.planningMode, search: SearchProvider = session.searchProvider) {
        val current = projects.sessions(session.projectId).single { it.id == session.id }
        require(!current.planningMode || planning) { "Режим планирования закреплён за сессией" }
        require(current.stageId == null || planning == current.planningMode) { "Режим сессии исполнителя менять нельзя" }
        if (planning && !current.planningMode) projects.dispatch(current.projectId,
            CodingMachine.Intent.ChangeMode(CodingMachine.ref(current), CodingInteractionMode.PLANNING))
        projects.dispatch(current.projectId, CodingMachine.Intent.SetSearchProvider(CodingMachine.ref(current), search))
        refreshSessions()
    }
    private suspend fun persistenceFailure(sessionId: String, projectId: String, error: Exception): Nothing {
        val message = "Не удалось сохранить состояние оркестратора. Проверьте хранилище и повторите действие."
        AppLog.error("coding.orchestration", "state.save.failed", mapOf("sessionId" to sessionId, "projectId" to projectId, "causeType" to error::class.simpleName.orEmpty()))
    ...
    /** Reducers own the mutation vocabulary; the store retains locking, revision and CAS authority. */
    private suspend fun updatePlan(id: String, vararg events: PlanEvent): Plan = store.command(id,
        PlanningMachine.Intent.Revise(events.toList(), PlanningMachine.Stamp(Id.new(), Id.now())))
    private suspend fun updatePlan(id: String, event: () -> PlanEvent): Plan = updatePlan(id, event())

    private suspend fun updateState(sessionId: String, projectId: String, event: OrchestrationEvent,
        onCommitted: suspend () -> Unit = {}): OrchestrationState =
        transitionState(sessionId, projectId, onCommitted) { event }.state

    private suspend fun updateState(sessionId: String, projectId: String, event: () -> OrchestrationEvent): OrchestrationState =
        transitionState(sessionId, projectId, event = event).state

    /** Read environment snapshots under the same lock as the transition and its durable commit. */
    private suspend fun transitionState(sessionId: String, projectId: String, onCommitted: suspend () -> Unit = {},
        event: () -> OrchestrationEvent): OrchestrationTransition = stateLock.withLock {
        val old = state(sessionId, projectId)
        val session = projects.sessions(projectId).first { it.id == sessionId }
        val transition = try {
            projects.dispatch(projectId, CodingMachine.Intent.Orchestrate(CodingMachine.ref(session), event())).effects
                .filterIsInstance<CodingMachine.Effect.Orchestrated>().single().transition
        } catch (e: CancellationException) { throw e }
        catch (e: Exception) { persistenceFailure(sessionId, projectId, e) }
        var next = transition.state
        val usedIds = next.messageEvents.map { it.id }.toMutableSet()
        val answerEvents = next.pendingAnswerEvents(clock()).map { draft ->
            draft.copy(id = uniqueSchedulingId(usedIds).also { usedIds += it })
        }
        if (answerEvents.isNotEmpty()) next = try {
            projects.dispatch(projectId, CodingMachine.Intent.Orchestrate(CodingMachine.ref(session),
                OrchestrationEvent.AnswerEventsRecorded(answerEvents))).state.orchestrations.getValue(sessionId)
        } catch (e: CancellationException) { throw e }
        catch (e: Exception) { persistenceFailure(sessionId, projectId, e) }
    ...
    private suspend fun recoverOrchestrationNow(sessionId: String) {
        ...
        stateLock.withLock {
            ...
            val saved = try { projects.dispatch(session.projectId, CodingMachine.Intent.Orchestrate(CodingMachine.ref(session),
                OrchestrationEvent.UnsavedInputsRecovered(pending))).state.orchestrations.getValue(session.id) }
            catch (cancelled: CancellationException) { throw cancelled }
            catch (e: Exception) { persistenceFailure(session.id, session.projectId, e) }
    ...
    fun send(session: CodingSession, text: String, answers: List<PlanningAnswer> = emptyList(), replyTo: String? = null, resumeAfter: Boolean = false,
        onAccepted: suspend () -> Unit = {}, inputId: String = Id.new(), sourceSessionId: String? = null, sourceText: String? = null): Job? {
        if (text.isBlank() && answers.isEmpty()) return null
        if (session.id in deletedSessions || session.projectId in clearingProjects || session.planId in deletedPlans) return null
        if (session.stageId != null && session.planId != null) {
            return launch {
                val plan = store.planFor(session.planId!!) ?: return@launch
                require(plan.projectId == session.projectId)
                // Receipt identity wins over a route that changed after the original acceptance.
                plan.deliveries.firstOrNull { it.id == inputId }?.let { delivery ->
                    require(delivery.sourceSessionId == session.id && delivery.text == text && delivery.replyTo == null)
                    onAccepted()
                    publishDelivery(plan, delivery)
                    return@launch
                }
                val forwardedText = "По этапу «${session.name}» (${session.subtitle()}): $text"
                state(plan.parentSessionId, session.projectId).inputs.firstOrNull { it.id == inputId }?.let { previous ->
                    require(previous.matchesSubmission(OrchestrationInput(inputId, forwardedText.trim(), 0,
                        sourceSessionId = session.id, sourceText = text)))
                    onAccepted()
                    projects.sessions(session.projectId).firstOrNull { it.id == plan.parentSessionId }?.let { parent ->
                        syncInputMessage(parent, inputId)
                        drainInputs(parent)
                    }
                    return@launch
                }
                if (plan.phase == ExecutionPhase.COMPLETE || plan.finalAttempt != null || session.archived) {
                    ...
                    send(parent, forwardedText, onAccepted = onAccepted, inputId = inputId,
                        sourceSessionId = session.id, sourceText = text)?.join()
                } else { queueWorker(session, text, inputId, onAccepted) }
            }
        }
        val input = OrchestrationInput(inputId, text.trim(), Id.now(), answers, replyTo, resumeAfter = resumeAfter,
            sourceSessionId = sourceSessionId, sourceText = sourceText)
        return launch {
            if (session.id in deletedSessions) return@launch
            val previous = state(session.id, session.projectId).inputs.firstOrNull { it.id == input.id }
            if (previous != null) {
                require(previous.matchesSubmission(input))
                onAccepted()
            ...
            sessionLock(session.id).withLock {
                ...
                projects.dispatch(latest.projectId, CodingMachine.Fact.PromptNamed(CodingMachine.ref(latest), text, false))
            }
            refreshSessions()
            try {
                updateState(session.id, session.projectId, OrchestrationEvent.InputSubmitted(input), onCommitted = onAccepted)
    ...
    suspend fun submitInteraction(request: UserInteractionRequest, answers: List<PlanningAnswer>) {
        ...
        when (request.kind) {
            InteractionKind.QUESTION -> {
                ...
                updateState(session.id, session.projectId, OrchestrationEvent.InputEnqueued(input))
                syncInputMessage(session, input.id)
                drainInputs(session)
            }
            InteractionKind.CONFIRM_PLAN -> {
                ...
                if ("yes" in answer.selected) confirmNow(plan.id, request.sourceId.ifBlank { null }, request.revision)
                    ...
                    if (plan.proposal != null) {
                        updatePlan(plan.id, PlanRevisionEvent.ProposalDeclined(plan.revision, plan.proposal!!.id))
                    }
            ...
            InteractionKind.RECOVER_PLAN -> {
                ...
                if (("review" in answer.selected || answer.text.isNotBlank() && blockers.any { it.issue.retryBlocked }) && !answer.skipped) {
                    ...
                    execution.continueWithoutVerification(plan.id, blockers.map { it.messageId }.toSet(),
                        requireNotNull(request.verificationProofs) { "Проверка изменилась. Ответьте на актуальный запрос." })
            ...
            InteractionKind.RECOVER_INPUT -> {
                ...
                if ("leave" !in answer.selected && !answer.skipped) {
                    updateState(session.id, session.projectId, OrchestrationEvent.InputRetried(input.id, answer.text))
                    syncInputMessage(session, input.id)
                    drainInputs(session)
                }
            }
    ...
    fun cancelQueuedInput(sessionId: String, inputId: String) = launch {
        ...
        updateState(session.id, session.projectId, OrchestrationEvent.InputWithdrawn(inputId))
        syncInputMessage(session, inputId)
    }

    fun retryInput(sessionId: String, inputId: String) = launch {
        val session = _sessions.value.firstOrNull { it.id == sessionId } ?: return@launch
        updateState(session.id, session.projectId, OrchestrationEvent.InputRetried(inputId))
        syncInputMessage(session, inputId)
        drainInputs(session)
    }
        ...
        val job = scope.launch(UsageOwner(UsageScope.coding(session)), start = CoroutineStart.LAZY) {
            try {
                while (true) {
                    ...
                    val claim = transitionState(session.id, session.projectId) { OrchestrationEvent.InputClaimRequested(store.plans.value) }
                    val input = claim.claimedInput ?: break
                    syncInputMessage(session, input.id)
                    try {
                ...
                publishError(e.message ?: "Не удалось сохранить состояние планирования.")
            } finally {
    ...
    private suspend fun drainInputsIfQueued(session: CodingSession) {
        ...
        if (state(session.id, session.projectId).hasQueuedInput(store.plans.value)) drainInputs(session)
    }

    private suspend fun setInputStatus(session: CodingSession, id: String, status: OrchestrationInputStatus, error: String = "") {
        updateState(session.id, session.projectId, OrchestrationEvent.InputStatusRecorded(id, status, error))
    ...
    private suspend fun processInputBody(session: CodingSession, input: OrchestrationInput) {
        ...
        if (plan == null) {
            ...
            store.command(plan.id, PlanningMachine.Intent.Create(plan, PlanningMachine.Stamp(Id.new(), Id.now())))
        }
        val current = plan
        ...
        updateState(session.id, session.projectId, OrchestrationEvent.PlanSelected(current.id))
        ...
        }.also { saved -> updateState(session.id, session.projectId, OrchestrationEvent.InputDecisionRecorded(input.id, saved)) }
        ...
        if (pauseIds.isNotEmpty() && !decision.toolsApplied) {
            val saved = updateState(session.id, session.projectId, OrchestrationEvent.WorkPaused(input.id, OrchestrationPause(current.id, pauseIds)))
        ...
        when (decision.intent) {
            ...
            UserTurnIntent.DISCUSS -> {
                ...
                val saved = updatePlan(current.id) { PlanRevisionEvent.DialogueAppended(listOf(
        ...
        updateState(session.id, session.projectId) { OrchestrationEvent.WorkPauseFinished(store.plans.value.first { it.id == current.id }, input.id) }
        val appliedMutation = decision.toolsApplied && toolReceipts
    ...
    private suspend fun interpretToolInput(session: CodingSession, plan: Plan, input: OrchestrationInput, profile: LlmProfile): UserTurnDecision {
        val tools = toolSessions!!.session(ToolExecutionContext(plan.projectId, session.id, session.id, input.id,
    ...
    private suspend fun interpretInput(session: CodingSession, plan: Plan, input: OrchestrationInput, savedDecisionProblem: String? = null): UserTurnDecision {
        ...
        if (toolSessions != null) return interpretToolInput(session, plan, input, profile)
    ...
    private suspend fun askToRefine(session: CodingSession, plan: Plan, input: OrchestrationInput, decision: UserTurnDecision) {
        ...
        updateState(session.id, session.projectId, OrchestrationEvent.QuestionRegistered(
            OrchestrationQuestion(id, plan.id, text, questions, session.id, scopeLabel = "Уточнение плана",
                refinementRequest = if (needsDetails) null else input.text, forDiscussion = needsDetails,
                pauseStageIds = decision.pauseStageIds, requirementContext = input.text)))
        val saved = updatePlan(plan.id, PlanRevisionEvent.DialogueAppended(listOf(
    ...
    private suspend fun answerQuestion(session: CodingSession, plan: Plan, input: OrchestrationInput, decision: UserTurnDecision) {
        val transition = transitionState(session.id, session.projectId) { OrchestrationEvent.QuestionAnswered(plan, input, decision, clock()) }
        val answer = transition.answer ?: return
        val (question, combined, complete, refinePlan) = answer
        val id = question.id
        ...
        suspend fun deliverAnswer() {
            ...
            if (question.forDiscussion && !question.pauseStageIds.isNullOrEmpty()) {
                ...
                updateState(session.id, session.projectId, OrchestrationEvent.RequirementsQueued(followup, OrchestrationPause(plan.id, question.pauseStageIds!!)))
                return
            }
            ...
            updatePlan(plan.id, PlanRevisionEvent.QuestionDelivered(targets))
            if (plan.intent == ExecutionIntent.RUN) execution.start(plan.id)
        }
        deliverAnswer()
        updateState(session.id, session.projectId) {
            val pause = store.plans.value.firstOrNull { it.id == plan.id }?.proposal?.let { proposal ->
                OrchestrationPause(plan.id, question.pauseStageIds ?: question.stageIds.ifEmpty { plan.selectedMilestones.map { it.id } }, proposal.id)
            }
            OrchestrationEvent.QuestionResolved(id, input.id, pause)
        }
    }
    ...
    private suspend fun refineLocked(id: String, requestId: String, text: String, requireApproval: Boolean, nodeId: String? = null) {
        val before = store.planFor(id) ?: error("План не найден")
        val session = projects.sessions(before.projectId).first { it.id == before.parentSessionId }
        val choice = session.modelSelection ?: before.plannerSelection
        val accepted = store.dispatch(id, PlanningMachine.Intent.BeginRefinement(
            PlanningMessage(requestId, "user", text), nodeId, requireApproval, choice, session.searchProvider,
        ...
        try {
            ...
            val preparation = pending.prepareRefinement(requireApproval, choice, session.searchProvider, session.engine)
            val continuation = refinement.mode == RefinementMode.PROPOSAL
            val effective = preparation.plan
            ...
            val result = withContext(workerDispatcher) {
                ...
                if (subtree == null) composer.refine(effective, request, profile, roster, modelDossiers.dossiers(), settings.load(), ::event) { event(CodingStep(CodingStepKind.INFO, it)) }
                else composer.recalculate(effective, subtree, profile, roster, modelDossiers.dossiers(), settings.load(), ::event) { event(CodingStep(CodingStepKind.INFO, it)) }
            }
            ...
            val saved = store.command(id, PlanningMachine.Fact.RefinementCompleted(refinement.ref,
                RefinementResult(PlanSpecification.from(result), assistant, result.wizardStep),
                PlanningMachine.Stamp(Id.new(), Id.now())))
            if (continuation && saved.proposal == null) {
                val source = currentCoroutineContext()[ToolSession]?.context
                if (source?.role == ToolRole.ORCHESTRATOR) updateState(sessionId, pending.projectId,
                    OrchestrationEvent.WorkPauseNeedsUser(source.sourceInput?.id ?: source.requestId))
            }
            publish(saved)
            ...
            store.command(id, PlanningMachine.Intent.CancelRefinement(refinement.ref, PlanningMachine.Stamp(Id.new(), Id.now())))
    ...
    private suspend fun confirmNow(id: String, proposalId: String? = null, expectedRevision: Long? = null) = confirmation.withLock {
        ...
        if (proposalId != null) {
            ...
            val openQuestions = state(plan.parentSessionId, plan.projectId).openQuestions(plan.id).isNotEmpty()
            plan = updatePlan(id) { PlanRevisionEvent.ProposalApproved(proposal, expectedRevision, openQuestions,
                paused, Id.new(), nextWorktreeMode, Id.now()) }
            ...
            val openQuestions = state(plan.parentSessionId, plan.projectId).openQuestions(plan.id).isNotEmpty()
            plan = updatePlan(id) { PlanRevisionEvent.InitialConfirmed(expectedRevision, openQuestions, Id.new(), Id.now()) }
        }
        updateState(plan.parentSessionId, plan.projectId, OrchestrationEvent.ProposalConfirmed(plan.id, proposalId))
    ...
    fun resume(session: CodingSession, text: String = "", onAccepted: suspend () -> Unit = {}) = launch {
        ...
        if (interrupted != null && session.stageId == null) {
            updateState(session.id, session.projectId, OrchestrationEvent.InputRetried(interrupted.id, text, clearResumeAfter = true))
        ...
        if (text.isNotBlank()) {
            if (session.stageId != null) { queueWorker(session, text, onAccepted = onAccepted) }
            else { send(session, text, onAccepted = onAccepted); return@launch }
        }
        resumePlan(plan.id)
    }
    ...
    private suspend fun resumePlan(id: String) {
        ...
        if (plan.proposal == null) updateState(plan.parentSessionId, plan.projectId, OrchestrationEvent.PlanResumed(plan))
    ...
    override suspend fun recoverAssignments(plan: Plan): Plan {
        val observed = cancelObsoletePeerCommands(plan)
        val captured = checkNotNull(store.machineStates.value[observed.id]) { "План не загружен" }
        val plan = checkNotNull(captured.plan)
        ...
        fun attemptChange(stageId: String?, before: StageAttempt, after: StageAttempt?): AttemptAssignmentChange? {
            ...
            return AttemptAssignmentChange(stageId, PlanningMachine.AttemptRef.from(before), before.assignment,
                after.assignment, before.mergeAssignment, after.mergeAssignment)
        }
        val changes = AssignmentRecovery(AssignmentRecoveryRef(plan.id, plan.projectId, plan.runId, captured.generation),
            planner = next.plannerSelection?.takeIf { it != plan.plannerSelection }?.let { PlannerSelectionChange(plan.plannerSelection, it) },
            stages = plan.milestones.mapNotNull { stage ->
                val before = stage.assignment ?: return@mapNotNull null
                val after = next.milestones.firstOrNull { it.id == stage.id }?.assignment ?: return@mapNotNull null
                if (before == after) null else StageAssignmentChange(stage.id, before, after)
            },
            attempts = plan.milestones.flatMap { stage ->
                val updated = next.milestones.first { it.id == stage.id }
                stage.attempts.mapNotNull { before -> attemptChange(stage.id, before, updated.attempts.firstOrNull { it.id == before.id }) }
            } + listOfNotNull(plan.finalAttempt?.let { attemptChange(null, it, next.finalAttempt) }))
        val accepted = store.dispatch(plan.id, PlanningMachine.Intent.RecoverAssignments(changes, PlanningMachine.Stamp(Id.new(), Id.now())))
        accepted.rejection?.let { error(it.reason) }
        val saved = checkNotNull(accepted.state.plan)
        val applied = accepted.state.generation == captured.generation && saved.runId == plan.runId &&
            (changes.stages.any { change -> saved.milestones.firstOrNull { it.id == change.stageId }?.assignment == change.after } ||
                changes.attempts.any { change ->
                    val attempt = if (change.stageId == null) saved.finalAttempt else
                        saved.milestones.firstOrNull { it.id == change.stageId }?.attempts?.firstOrNull { it.id == change.expected.id }
                    attempt != null && attempt.assignment == change.after && attempt.mergeAssignment == change.afterMerge
                })
        if (applied) append(plan.projectId, plan.parentSessionId, CodingMessage("${plan.id}-models-${saved.plannerSelection.hashCode()}-${saved.milestones.map { it.assignment }.hashCode()}",
            CodingRole.AGENT, "Назначения недоступных моделей восстановлены по текущему выбору сессии.", createdAt = Id.now()))
        return saved
    }
    ...
    private suspend fun numbered(plan: Plan): Plan {
        ...
        val saved = updateState(plan.parentSessionId, plan.projectId,
            OrchestrationEvent.StageNumbersRequested(plan.id, (plan.milestones + plan.proposal?.milestones.orEmpty()).map { it.id }))
        val next = plan.bindStageNumbers(saved.stageNumbers)
        if (next == plan) return plan
        updatePlan(plan.id, PlanRevisionEvent.StageNumbersBound(saved.stageNumbers))
        return next
    }
    ...
    internal fun saveRename(session: CodingSession, form: CodingTextForm) = form.submit(replacement = { it.trim() }) { name, operationId, baseline ->
        historyDeletionLock.withLock {
            check(!closing && form.state.value.available && session.id !in deletedSessions && session.projectId !in clearingProjects)
            val latest = projects.sessions(session.projectId).firstOrNull { it.id == session.id }
                ?: error("Сессия удалена")
            if (latest.organismId == null) require(latest.name == baseline || latest.name == name.trim()) { "Название сессии уже изменено" }
            changeSession(session.id, SessionCommandKind.RENAME, name, operationId,
                expectedName = if (latest.organismId == null) baseline ?: latest.name else null)
            projects.sessions(session.projectId).any { it.id == session.id && it.name == name.trim() }
        }
    }

    internal fun editScheduledMessage(session: CodingSession, plan: Plan, rule: ScheduledMessage, form: CodingTextForm) = form.submit { request, operationId, _ ->
        val text = "Измени правило ${rule.id}: ${request.trim()}"
        val recorded = state(session.id, session.projectId).inputs.firstOrNull { it.id == operationId }
        if (recorded != null) {
            check(recorded.text == text && recorded.answers.isEmpty() && recorded.replyTo == null && !recorded.resumeAfter)
            return@submit true
        }
        val current = store.planFor(plan.id)
        if (session.stageId != null && current?.projectId == session.projectId && session.planId == current.id) {
            current.deliveries.firstOrNull { it.id == operationId }?.let { delivery ->
                check(delivery.sourceSessionId == session.id && delivery.text == text && delivery.replyTo == null)
                return@submit true
            }
            state(current.parentSessionId, session.projectId).inputs.firstOrNull { it.id == operationId }?.let { previous ->
                check(previous.matchesSubmission(OrchestrationInput(operationId,
                    "По этапу «${session.name}» (${session.subtitle()}): $text", 0,
                    sourceSessionId = session.id, sourceText = text)))
                return@submit true
            }
        }
        check(current?.projectId == session.projectId && current.runId == plan.runId &&
            current.scheduledMessages.any { it.id == rule.id && it.runId == rule.runId &&
                it.status in setOf(ScheduledMessageStatus.WAITING, ScheduledMessageStatus.READY, ScheduledMessageStatus.ERROR) })
        val accepted = CompletableDeferred<Boolean>()
        val job = send(session, text, onAccepted = { accepted.complete(true) }, inputId = operationId)
    ...
    private suspend fun changeSession(sessionId: String, kind: SessionCommandKind, name: String = "", operationId: String = Id.new(), expectedName: String? = null) {
        ...
        if (session.organismId != null && kind == SessionCommandKind.RENAME) {
            (organisms ?: error("Организмы недоступны")).renameByUser(session, name, operationId)
            val savedName = projects.sessions(session.projectId).firstOrNull { it.id == sessionId }?.name ?: return
            session.planId?.let { planId -> store.planFor(planId)?.let {
                updatePlan(planId, PlanRevisionEvent.StageRenamed(session.stageId, savedName))
            } }
            refreshSessions()
            return
        }
        if (session.effectiveRole == CodingSessionRole.ORCHESTRATOR && kind == SessionCommandKind.RENAME) {
            ...
            projects.dispatch(session.projectId, CodingMachine.Intent.RenameSession(CodingMachine.ref(session), name, expectedName))
        ...
        if (!hasLiveOrchestrator(session)) {
            ...
            sessionLock(sessionId).withLock {
                if (projects.sessions(session.projectId).none { it.id == sessionId }) return@withLock
                when (kind) {
                    SessionCommandKind.ARCHIVE -> projects.dispatch(session.projectId, CodingMachine.Intent.ArchiveSession(CodingMachine.ref(session), true))
                    SessionCommandKind.RESTORE -> projects.dispatch(session.projectId, CodingMachine.Intent.ArchiveSession(CodingMachine.ref(session), false))
                    SessionCommandKind.RENAME -> projects.dispatch(session.projectId, CodingMachine.Intent.RenameSession(CodingMachine.ref(session), name, expectedName))
                    else -> Unit
                }
            }
            refreshSessions()
        ...
        if (plan == null) return
        performSessionCommand(plan, SessionCommand(operationId, kind, sessionId, plan.id, session.stageId.orEmpty(), name.trim()))
    }
    ...
    private suspend fun performSessionCommand(plan: Plan, command: SessionCommand, publishNotice: Boolean = true) = sessionLock(command.sessionId).withLock {
        ...
        val saved = updateState(plan.parentSessionId, plan.projectId, OrchestrationEvent.SessionCommandRegistered(command))
        ...
        if (rejection.isNotBlank()) {
            updateState(plan.parentSessionId, plan.projectId, OrchestrationEvent.SessionCommandRejected(command.id, rejection))
        ...
        try {
            ...
            when (command.kind) {
                SessionCommandKind.CREATE -> {
                    ...
                    if (session == null) projects.dispatch(plan.projectId, CodingMachine.Intent.CreateSession(CodingSession(command.sessionId, plan.projectId,
                ...
                SessionCommandKind.ARCHIVE -> {
                    ...
                    if (!(currentStage.completed || (currentStage.attempts.isEmpty() && currentStage.id !in latest.selectedMilestones.map { it.id })))
                        throw SessionActionRejection(SessionActionProblem.INCOMPLETE_STAGE)
                    if (!(latest.deliveries.none { it.targetStageId == stage.id && it.state in setOf(DeliveryState.QUEUED, DeliveryState.DELIVERED) } &&
                        ...
                        throw SessionActionRejection(SessionActionProblem.PENDING_INPUT)
                    projects.dispatch(session.projectId, CodingMachine.Intent.ArchiveSession(CodingMachine.ref(session), true))
                }
                SessionCommandKind.RESTORE -> {
                    requireTool(session != null) { "Сессия не найдена" }
                    projects.dispatch(session.projectId, CodingMachine.Intent.ArchiveSession(CodingMachine.ref(session), false))
                }
                SessionCommandKind.RENAME -> {
                    requireTool(session != null && command.name.isNotBlank()) { "Добавьте название сессии" }
                    projects.dispatch(session.projectId, CodingMachine.Intent.RenameSession(CodingMachine.ref(session), command.name))
                    updatePlan(plan.id, PlanRevisionEvent.StageRenamed(stage?.id, command.name))
                }
            }
        } catch (e: IllegalArgumentException) {
            // Rejected commands must not become delayed actions when the stage later completes.
            updateState(plan.parentSessionId, plan.projectId, OrchestrationEvent.SessionCommandDiscarded(command.id))
            throw e
        }
        updateState(plan.parentSessionId, plan.projectId, OrchestrationEvent.SessionCommandApplied(command.id))
    ...
    override suspend fun prepareSessions(plan: Plan) {
        ...
        if (late != null) {
            val updated = updatePlan(plan.id) { CoordinationEvent.LateDeliveryObserved(late.id, Id.uuid()) }
        ...
        if (parent != null) {
            ...
            if (parent != updated) projects.dispatch(parent.projectId, CodingMachine.Fact.PlanSessionBound(CodingMachine.ref(parent),
                CodingSessionRole.ORCHESTRATOR, orchestratorNumber = updated.orchestratorNumber, suggestedName = updated.name))
        }
        plan.selectedMilestones.forEach { stage ->
            ...
            if (existing.none { it.id == id }) performSessionCommand(plan,
                ...
                val next = current.copy(role = CodingSessionRole.WORKER, stageNumber = stage.displayNumber,
                    ...
                    continuationOfNumber = plan.milestones.firstOrNull { it.id == stage.continuationOf }?.displayNumber)
                if (next != current) projects.dispatch(current.projectId, CodingMachine.Fact.PlanSessionBound(CodingMachine.ref(current),
                    CodingSessionRole.WORKER, stageNumber = next.stageNumber, continuationOfNumber = next.continuationOfNumber,
                    suggestedName = next.name, legacyName = legacyName))
            }
    ...
    private suspend fun queueWorker(session: CodingSession, text: String, inputId: String = Id.new(), onAccepted: suspend () -> Unit = {}) {
        val id = enqueue(session.planId!!, session.id, session.stageId!!, text, deliveryId = inputId, onCommitted = onAccepted)
    ...
    private suspend fun enqueue(id: String, source: String, target: String, text: String, deliveryId: String = Id.new(), replyTo: String? = null,
        expectedRetryCheckpoint: Plan? = null, onCommitted: suspend () -> Unit = {}): String {
        ...
        existing.deliveries.firstOrNull { it.id == deliveryId }?.let {
            require(it.sourceSessionId == source && it.text == text && it.replyTo == replyTo)
            onCommitted()
            prepareSessions(existing)
            publishDelivery(store.planFor(id)!!, it)
            return deliveryId
        }
        val updated = updatePlan(id) { CoordinationEvent.DeliveryRequested(
            PlanDelivery(deliveryId, source, target, text, replyTo = replyTo), Id.uuid(), expectedRetryCheckpoint) }
        onCommitted()
    ...
    override suspend fun instructions(plan: Plan, stage: Milestone, attempt: StageAttempt): String {
        ...
        val p = updatePlan(plan.id, CoordinationEvent.InboxPrepared(stage.id, attempt.id, attempt.turnIndex))
        ...
        return """
            ...
            ${if (toolSessions != null) "Передай структурированный результат через magicpaper_stage_handoff; последний текст — краткое пояснение. Параметры инструмента:" else "Последнее сообщение верни строго JSON:"} {"kind":"RESULT|QUESTION|BLOCKED|WAIT","text":"результат, вопрос или причина блокировки","targetStageId":"id этапа или пустая строка","changedFiles":["относительный путь"]}.
    ...
    override suspend fun started(plan: Plan, stage: Milestone, attempt: StageAttempt) {
        if (plan.parentSessionId.isBlank()) return
        val saved = updatePlan(plan.id, CoordinationEvent.InboxDelivered(attempt.id, attempt.turnIndex))
    ...
    override suspend fun finished(plan: Plan, stage: Milestone, attempt: StageAttempt): StageTurnDecision {
        ...
        if (existing == null) {
            ...
            updatePlan(plan.id) { CoordinationEvent.HandoffRecorded(CoordinationRecord(transferId, stage.id, parsed,
        ...
        if (beforeTransfer.deliveries.any { it.attemptId == attempt.id && it.turnIndex == attempt.turnIndex && it.state == DeliveryState.DELIVERED }) {
            updatePlan(plan.id, CoordinationEvent.InboxAnswered(attempt.id, attempt.turnIndex))
        }
    ...
    private suspend fun coordinateTurn(plan: Plan, stage: Milestone, attempt: StageAttempt, activityId: String): StageTurnDecision {
        ...
        if (record == null) {
            ...
            updatePlan(plan.id, CoordinationEvent.HandoffRecorded(createdRecord), CoordinationEvent.InboxAnswered(attempt.id, attempt.turnIndex))
        }
        ...
        if (decision == null) {
            ...
            val updated = updatePlan(plan.id, CoordinationEvent.DecisionRecorded(eventId, savedDecision, activity, replacesResult = savedResultProblem != null))
        ...
        if (!decision.toolsApplied) decision.actions.forEachIndexed { index, action ->
            val latest = store.planFor(plan.id)!!
            val applied = latest.appliedCoordinatorDelivery(eventId, action)
        ...
        when (val continuation = store.planFor(plan.id)!!.coordinatorContinuation(stage.id, record, decision)) {
            is CoordinatorContinuation.Return -> return continuation.decision
            is CoordinatorContinuation.Ask -> {
                askUser(plan, stage, continuation.id, continuation.text)
                return StageTurnDecision(StageTurnAction.WAIT, continuation.report, continuation.id)
            }
            is CoordinatorContinuation.Deliver -> continuation.delivery.let { delivery ->
                enqueue(plan.id, delivery.sourceSessionId, delivery.targetStageId, delivery.text, delivery.id)
            }
            CoordinatorContinuation.InspectInbox -> Unit
        }
        return store.planFor(plan.id)!!.coordinatedTurn(stage.id, reply.text)
    }
    ...
    private suspend fun cancelObsoletePeerCommands(plan: Plan): Plan {
        ...
        val saved = updatePlan(current.id, PlanRevisionEvent.LegacyPeersObserved(peers, admitted))
        ...
        for (session in projects.sessions(saved.projectId)) {
            messageLock.withLock {
                ...
                if (next != history) publishHistory(saved.projectId, session.id, next)
            }
            if (session.planId == saved.id && session.stageId in removed) projects.dispatch(saved.projectId, CodingMachine.Intent.ArchiveSession(CodingMachine.ref(session), true))
        }
        return saved
    }
    ...
    private suspend fun coordinatorDecision(
        ...
        if (toolSessions != null) {
            ...
            val tools = toolSessions.session(ToolExecutionContext(plan.projectId, plan.parentSessionId, plan.parentSessionId, "$eventId-coordinator",
                ToolRole.ORCHESTRATOR, CodingInteractionMode.PLANNING, plan.id, plan.runId,
            ...
            updateState(plan.parentSessionId, plan.projectId) {
                OrchestrationEvent.WorkPauseFinished(store.plans.value.first { it.id == plan.id }, tools.context.requestId)
            }
    ...
    private suspend fun askUser(plan: Plan, stage: Milestone, id: String, text: String,
        ...
        updateState(plan.parentSessionId, plan.projectId, OrchestrationEvent.QuestionRegistered(
    ...
    private suspend fun publish(plan: Plan) {
        ...
        if (plan.id in deletedPlans || plan.projectId in clearingProjects) return
        if (projects.sessions(plan.projectId).none { it.id == plan.parentSessionId }) return
        ...
        updateState(plan.parentSessionId, plan.projectId, OrchestrationEvent.QuestionsImported(plan, questionMessages))
        ...
        if (plan.confirmedRevision != null) {
            plan.selectedMilestones.forEach { stage ->
                ...
                messageLock.withLock {
                    ...
                    if (messages != next) publishHistory(plan.projectId, sessionId, next)
                }
            }
        }
    }
        ...
        store.plans().forEach { plan ->
            try {
                if (plan.parentSessionId.isBlank()) {
                    ...
                    if (projects.sessions(plan.projectId).none { it.id == sessionId }) projects.dispatch(plan.projectId, CodingMachine.Intent.CreateSession(CodingSession(sessionId, plan.projectId, "План: ${plan.goal.take(40)}", plan.createdAt, planningMode = true, modelSelection = plan.plannerSelection,
                        engine = plan.engine ?: legacyCodingEngine(plan.plannerSelection?.let { ProfileResolver.selection(it, profiles.load()) }))))
                    val linked = updatePlan(plan.id) { PlanRevisionEvent.LegacyLinked(sessionId, Id.now()) }
                ...
                if (legacy.parentSessionId.isNotBlank()) {
                    ...
                    if (legacy.milestones.any { it.attempts.any { a -> a.error?.isPlannerAnswerWait == true } }) {
                        updatePlan(legacy.id, PlanRevisionEvent.LegacyQuestionsObserved(open))
                    }
                }
                ...
                if (parent != null) {
                    ...
                    projects.dispatch(parent.projectId, CodingMachine.Fact.PlanSessionBound(CodingMachine.ref(parent), CodingSessionRole.ORCHESTRATOR))
                    if (migratedPlan.engine == null) updatePlan(migratedPlan.id, PlanRevisionEvent.EngineRestored(engine))
                    updateState(parent.id, parent.projectId, OrchestrationEvent.LegacyRequestImported(plan))
                }
...
/** Only these application-owned validation messages are safe to expose from the async command boundary. */
private enum class SessionActionProblem(val userMessage: String) {
    INCOMPLETE_STAGE("Сессию незавершённого этапа нельзя архивировать"),
    PENDING_INPUT("У сессии есть ожидающие сообщения или вопросы"),
}
private class SessionActionRejection(val problem: SessionActionProblem) : IllegalArgumentException(problem.userMessage), RejectedToolCall
