...
@Composable
internal fun ScheduledMessages(plan: Plan, onCancel: (String) -> Unit,
    formFor: (ScheduledMessage) -> CodingTextForm, onEdit: (ScheduledMessage, CodingTextForm) -> Unit) {
    var editing by rememberSaveable(plan.id, plan.runId) { mutableStateOf<String?>(null) }
        ...
        PaperPanel(Modifier.fillMaxWidth(), kind = PaperSurfaceKind.PANEL) {
            Column(Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) {
                ...
                if (rule.status in setOf(ScheduledMessageStatus.WAITING, ScheduledMessageStatus.READY, ScheduledMessageStatus.ERROR)) {
                    ...
                    PaperAction({ editing = rule.id }) { PaperText("Изменить через чат", role = PaperTextRole.LABEL) }
                }
    ...
    editing?.let { id -> plan.scheduledMessages.firstOrNull { it.id == id }?.let { rule ->
        val form = remember(plan.id, rule.runId, rule.id) { formFor(rule) }
        ...
        LaunchedEffect(operation.completed) { if (operation.completed > initialCompletion) editing = null }
        PaperDialog(title = "Изменить через чат", onDismissRequest = { editing = null },
            onDismissAction = { form.discard("") }, dismissLabel = "Отмена", dismissEnabled = draft.loaded && operation.available && !operation.busy,
            confirmLabel = "Отправить", confirmEnabled = draft.loaded && operation.available && draft.value.text.isNotBlank() && !operation.busy,
            onConfirm = { onEdit(rule, form) }) {
            PaperComposerField(draft.value.text, form::update, enabled = draft.loaded && operation.available,
                label = { PaperText("Что изменить в будущем сообщении?") }, minLines = 3)
