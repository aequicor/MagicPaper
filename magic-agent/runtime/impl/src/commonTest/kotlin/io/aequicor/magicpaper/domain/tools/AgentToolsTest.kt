...
class AgentToolsTest {
    @Test fun aSourceTaskCannotInvokeApplicationSearchEvenThroughAnExplicitToolCall() = runTest {
        var searches = 0
        val host = testToolSessions(MemoryToolReceiptStore(), search = { _, _ -> searches++; JsonArray(emptyList()) })
        ...
        testToolRuntime(native, host).runChat(session, "Service notice mentioning: найди другие источники", null).toList()
        assertEquals(0, searches)
    }

    @Test fun researchChatHasSearchButCannotReachProjectMutationOrOrchestration() = runTest {
        val host = testToolSessions(MemoryToolReceiptStore(), checkScope = { error("Project ownership must not be used for research chat") },
                search = { ctx, _ ->
        ...
        val runtime = testToolRuntime(native, host)
    ...
    private fun session(receipts: ToolReceiptStore = MemoryToolReceiptStore(), ctx: ToolExecutionContext = context,
        ...
        return DefaultToolSession(ctx, registry, ToolExecutor(registry, receipts, check))
    }
    ...
    @Test fun questionnaireWaitsAndCannotGrantPermissions() = runTest {
        ...
        host.questionnaires.questions.respond(request.id, listOf(PlanningAnswer("q", text = "pdf")))
