...
class SessionAuthorityToolTest {
    @Test fun plannerCreatesResearchAndCodingChildrenWithInheritedRulesAndSeparateAuthority() = runTest {
        val f = SessionOrganismTestFixture(); f.initialize(CodingInteractionMode.PLANNING)
        f.ports.startChild = { _, _ -> }; f.ports.canCreateCodeChild = { true }
        val tools = host(f).session(f.context())
        for (mode in listOf(CodingInteractionMode.CODE, CodingInteractionMode.RESEARCH)) {
            val result = tools.call("create-$mode", "session.create", json.encodeToJsonElement(
                SessionCreateArgs("Child", "Investigate", "Verified", mode = mode)).jsonObject).jsonObject
            val id = result.getValue("sessionId").jsonPrimitive.content
            val aggregate = f.store.get(f.root.organismId!!)
            val child = aggregate.sessions.getValue(id)
            assertEquals(mode, child.mode)
            assertEquals(aggregate.sessions.getValue("root").rules, child.rules)
            assertEquals("root", child.authorityParentId)
            val childContext = f.context(id)
            assertTrue(SessionToolCatalog.definitions.filter { it.orchestration }.none { it.allowed(childContext) })
        }
    }
    ...
    private fun host(f: SessionOrganismTestFixture): ToolSessionFactory = testToolSessions(StoredToolReceipts(f.storage), receiver = { context, operation, name, args -> f.service.execute(context, operation, name, args) },
    ...
    @Test fun oneToolSessionCanApplySequentialCommandsWithoutFreezingAnObsoleteVersion() = runTest {
        val f = SessionOrganismTestFixture(); f.initialize(CodingInteractionMode.PLANNING); f.ports.startChild = { _, _ -> }
        val tools = host(f).session(f.context())
    ...
    @Test fun explicitVersionIsComparedAtomicallyAfterSuspendingSourceInspection() = runTest {
        ...
        val context = f.context()
        val before = f.store.get(f.root.organismId!!)
        val service = testOrganismService(f.store, f.projects, f.settings, ports = f.ports, sourceSnapshot = {
        ...
        f.ports.startChild = { _, _ -> fail("A stale create must not start a runtime") }
    ...
    @Test fun omittedVersionRevalidatesOnlyAPrecommitConflictWithoutRepeatingSourceInspection() = runTest {
        ...
        f.ports.startChild = { _, _ -> started++ }
    ...
    @Test fun auxiliaryRuntimeCanInspectButCannotBorrowItsHistoryOwnersLifecycleAuthority() = runTest {
        ...
        var mutations = 0
        val host = testToolSessions(MemoryToolReceiptStore(), receiver = { _, _, tool, _ ->
    ...
    @Test fun argumentAuthorityIsCheckedBeforeOverridesAndReplays() = runTest {
        ...
        val host = testToolSessions(MemoryToolReceiptStore(),
            authorizeCommand = { _, definition, args ->
            if (definition.id == "session.control") requireTool(allowed && args["sessionId"] == JsonPrimitive("child")) { "Foreign target" }
        })
        val context = ToolExecutionContext("p", "s", "s", "request", ToolRole.CHAT, CodingInteractionMode.PLANNING)
