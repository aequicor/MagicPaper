...
class ToolReceiptIntegrityTest {
    ...
    private fun tools(store: ToolReceiptStore, definitions: List<ToolDefinition> = listOf(definition),
        ...
        return DefaultToolSession(ctx, registry, ToolExecutor(registry, store, check, check, reconcile, secrets), secrets)
    }
    ...
    @Test fun mediaSettledBeforeAwaiterCancellationCannotBeDowngradedOrQuarantined() = runTest {
        for (terminal in listOf(MediaPhase.READY, MediaPhase.FAILED)) {
            ...
            var quarantines = 0
            val host = testToolSessions(receipts, unknownOutcome = { _, _ -> quarantines++ })
            ...
            val session = host.session(ctx, mapOf("image.generate" to { _, operation, _ ->
                host.media.reconcileCompletion(MediaGenerationOwner("s", "request", key, "p"), operation,
    ...
    @Test fun hostDefaultsFreezeGenerationOncePerToolSession() = runTest {
        ...
        val host = testToolSessions(MemoryToolReceiptStore(),
            contextDefaults = { resolutions++; it.copy(runtimeGeneration = generation, organismId = "organism") },
            checkScope = { require(it.runtimeGeneration == generation) { "Generation revoked" } },
            receiver = { _, _, _, _ -> JsonPrimitive("current") })
    ...
    @Test fun uncertainEffectsNotifyTheApplicationAndKeepImmunityChannelAvailable() = runTest {
        ...
        var effects = 0
        val host = testToolSessions(store, unknownOutcome = { _, receipt -> reports += receipt })
        val session = host.session(context.copy(mode = CodingInteractionMode.PLANNING), mapOf(
