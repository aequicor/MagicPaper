...
class OrchestrationToolsTest {
    private class Fixture(val scope: TestScope, receipts: ToolReceiptStore = MemoryToolReceiptStore(), managed: Boolean = false) {
        ...
        val host = testToolSessions(receipts, planningAccess = { service })
        val organisms = if (managed) testOrganismService(io.aequicor.magicpaper.data.coding.DefaultSessionOrganismStore(kv, InMemoryEventJournal(), dispatcher = StandardTestDispatcher(scope.testScheduler)), projects, settings) else null
        ...
        val composer = PlanComposer(gateway, planningGateway = RuntimePlanningGateway(runtime), projectLookup = { project }, toolSessions = host)
        val executionPorts = TestPlanningExecutionPorts(organisms)
        val execution = PlanningExecutionService(store, testToolRuntime(runtime, host), projects, profiles, settings,
        ...
        val service: OrchestrationService by lazy { OrchestrationService(store, execution, projects, profiles, settings, composer, gateway, scope.backgroundScope,
            workerDispatcher = StandardTestDispatcher(scope.testScheduler), toolSessions = host, organisms = organisms, modelDossiers = io.aequicor.magicpaper.domain.testModelDossiers()).also { executionPorts.chatHooks = it } }
        suspend fun init(workerParent: String? = null) {
            projects.createTestProject(project); profiles.save(profile); settings.save(AppSettings(activeLlmProfileId = profile.id))
            projects.createTestSession(parent); projects.createTestSession(if (workerParent != null) worker.copy(parentSessionId = workerParent) else worker)
    ...
    @Test fun proseWithoutStructureDoesNotProduceASavedProposalReceipt() = runTest {
        ...
        assertEquals(result, f.service.reconcile(f.orchestrator().context, receipt))
    }
    ...
    @Test fun contextExposesActualPendingProposalAndRejectsMissingConfirmation() = runTest {
        ...
        f.store.seed("plan") { it.copy(proposal = PlanProposal("proposal", it.runId, it.tree, it.milestones,
        ...
        f.store.seed("plan") { it.copy(proposal = null) }
    ...
    @Test fun pausedStageCriteriaCanBeApprovedWithoutExtraStagesOrLosingHistory() = runTest {
        val f = Fixture(this); f.init()
        val paused = f.store.seed("plan") { it.copy(intent = ExecutionIntent.PAUSE, phase = ExecutionPhase.WAITING,
    ...
    @Test fun replacementDraftIsDurableVisibleAndOnlyItsConfirmationStartsNewWork() = runTest {
        ...
        f.store.seed("plan") { it.copy(issue = it.issue!!.copy(retryBlocked = true)) }
        ...
        val reloaded = TestPlanningStore(JsonPlanningRepository(f.kv, f.json)).plans().single { it.id == id }
        ...
        assertTrue(f.workerCalls.isNotEmpty(), "${f.service.error.value}; ${f.store.plans.value.map { it.issue }}")
    ...
    @Test fun selectivePauseIncludesDependantsAndLeavesIndependentStagesAvailable() = runTest {
        val f = Fixture(this); f.init()
        f.store.seed("plan") { p -> p.copy(milestones = p.milestones + listOf(
    ...
    @Test fun legacyTargetsCannotReuseAStageWhoseSessionBelongsToAnotherParent() = runTest {
        val f = Fixture(this); f.init(workerParent = "other-parent")
        val tools = f.orchestrator()
    ...
    @Test fun mergeRuntimeKeepsHistoryOwnershipWithoutAcquiringNormalSessionControl() = runTest {
        val f = Fixture(this); f.init()
        val context = f.service.prepareWorker(f.worker.copy(id = "worker-merge"))
        ...
        assertFalse(f.service.prepareWorker(f.worker).auxiliaryExecution)
    }
    ...
    @Test fun managedGenerationIsCheckedAfterEffectAndBeforeNativeCompletionPersistence() = runTest {
        ...
        assertFailsWith<IllegalArgumentException> { tools.recordNative(event.copy(phase = ToolPhase.SUCCEEDED, result = "late content")) }
    ...
    @Test fun savedManagedLegacyCommandCannotMintCurrentGenerationAuthorityOnReplay() = runTest {
        ...
        f.projects.recordTestOrchestration(OrchestrationState("parent", "p", activePlanId = "plan",
    ...
    @Test fun workerContextCannotReadSiblingMutableReportOutsideTheParentRoute() = runTest {
        val f = Fixture(this); f.init()
        f.store.seed("plan") { plan -> plan.copy(milestones = plan.milestones +
        ...
        val workerTools = f.host.session(f.service.prepareWorker(f.worker))
        ...
        val mergeTools = f.host.session(f.service.prepareWorker(f.worker.copy(id = "worker-merge")))
    ...
    @Test fun savedLegacyDecisionUsesCommandRecoveryAndHistoricalMessagesAreNotExecuted() = runTest {
        ...
        f.projects.recordTestOrchestration(OrchestrationState("parent", "p", activePlanId = "plan", inputs = listOf(input)))
        f.projects.publishTestHistory("p", "parent", listOf(CodingMessage("old-text", CodingRole.AGENT,
            """{"intent":"INSTRUCT","stageId":"stage","reply":"Never execute this history"}""", createdAt = 0)))
        f.service.bootstrap(); runCurrent()
        assertTrue(f.store.planFor("plan")!!.deliveries.isEmpty())
        assertTrue(f.host.receipts.forRequest("p/parent/legacy").isEmpty())
        f.service.recoverOrchestration("parent"); runCurrent()
    ...
    @Test fun handoffIsDurableAndCannotTargetAnotherAttempt() = runTest {
        val f = Fixture(this); f.init()
        val context = f.service.prepareWorker(f.worker)
    ...
    @Test fun failedRevisionKeepsSelectivePauseUntilAnExplicitUserResume() = runTest {
        val f = Fixture(this); f.init()
        f.store.seed("plan") { it.copy(intent = ExecutionIntent.RUN) }
        f.planningRun = { tools ->
    ...
    @Test fun prematurePlannerFinalKeepsQuestionAliveAndContinuesAfterConfirmedAnswer() = runTest {
        ...
        f.planningRun = { tools ->
            ...
            if (contexts.size == 1) {
                coroutineScope {
                    ...
                    f.host.questionnaires.questions.requests.first { it.isNotEmpty() }
        ...
        runCurrent()
        val question = f.host.questionnaires.questions.requests.value.single()
        advanceTimeBy(3_600_000); runCurrent()
        assertTrue(result.isActive)
        assertEquals(question, f.host.questionnaires.questions.requests.value.single())
        assertEquals(1, contexts.size)
        f.host.questionnaires.questions.respond(question.id, listOf(PlanningAnswer("source", text = "Существующее подключение")))
        ...
        assertTrue(f.host.questionnaires.questions.requests.value.isEmpty())
    }
    ...
    @Test fun plannerCannotProposeBeforeAnswerAndNormalQuestionNeedsNoRestart() = runTest {
        ...
        val result = async { f.composer.completePlanning(f.store.planFor("plan")!!, f.profile,
            listOf(LlmMessage(LlmChatRole.USER, "Review"))) {} }
        runCurrent()
        f.host.questionnaires.questions.respond(f.host.questionnaires.questions.requests.value.single().id, listOf(PlanningAnswer("q", text = "Мой ответ")))
        assertContains(result.await(), "Ответ учтён")
    ...
    @Test fun stoppingPlanningStillCancelsQuestionAfterPrematureNativeFinal() = runTest {
        ...
        val result = launch { f.composer.completePlanning(f.store.planFor("plan")!!, f.profile,
            listOf(LlmMessage(LlmChatRole.USER, "Review"))) {} }
        runCurrent()
        assertEquals(1, f.host.questionnaires.questions.requests.value.size)
    ...
    @Test fun coordinatorContinueQueuesConcreteWorkAndVerifyCannotAcceptItEarly() = runTest {
        val f = Fixture(this); f.init()
        f.host.session(f.service.prepareWorker(f.worker)).call("result", "stage.handoff", f.args("""{"kind":"RESULT","text":"Partial work"}"""))
    ...
    @Test fun proposalConfirmationWorkerHandoffAndVerificationShareToolsAndSeparateHistories() = runTest {
        val f = Fixture(this); f.init()
        f.store.seed("plan") { it.copy(confirmedRevision = null,
