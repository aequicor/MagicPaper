...
class SessionPlanBudgetTest {
    ...
    @Test fun continuationRetainsItsRemainingAllocationAcrossStoreReload() = runTest {
        ...
        val reloaded = DefaultSessionOrganismStore(f.storage, f.journal) { 1_000 }
    ...
    @Test fun auxiliaryPlannerOverrunUsesTheSameConservativeTaskAccounting() = runTest {
        ...
        val auxiliary = f.store.beginAuxiliary((context).organismAdmission())
    ...
    @Test fun runningSiblingUsageIsRecordedAfterAnotherWorkerExhaustsItsGrant() = runTest {
        ...
        val expired = DefaultSessionOrganismStore(f.storage, f.journal) { 9_000_000 }
        ...
        expired.observe(running.id, sibling.sessionId, sibling.generation, SessionObservedState.STOPPED)
        assertFailsWith<IllegalArgumentException> { expired.charge(sibling, 1) }
        assertEquals(17L, expired.get(running.id).sessions.getValue("worker-2").spentTokens)
    }
    ...
    @Test fun liveAuxiliaryUsageIsRecordedAfterAnotherAliasExhaustsTheirOwner() = runTest {
        ...
        val first = f.store.beginAuxiliary((context("first")).organismAdmission())
        val second = f.store.beginAuxiliary((context("second")).organismAdmission())
        ...
        assertFailsWith<IllegalArgumentException> { f.store.beginAuxiliary((context("third")).organismAdmission()) }
