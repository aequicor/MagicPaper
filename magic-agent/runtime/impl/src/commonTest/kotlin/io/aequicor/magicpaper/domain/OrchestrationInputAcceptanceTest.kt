...
class OrchestrationInputAcceptanceTest {
    ...
    @Test fun retryOfAcceptedWorkerInputDoesNotBecomeANewParentInputAfterPlanCompletes() = runTest {
        ...
        assertEquals(1, accepted)
        first.store.seed(plan.id) { it.copy(phase = ExecutionPhase.COMPLETE) }
        first.close()
    ...
    @Test fun forwardedReceiptSurvivesWorkerRenameAndPlanResumptionWithoutChangingDestination() = runTest {
        ...
        first.store.seed(plan.id) { it.copy(phase = ExecutionPhase.EXECUTING) }
        val renamed = worker.copy(name = "Renamed after acceptance")
        first.projects.dispatch(renamed.projectId, CodingMachine.Intent.RenameSession(CodingMachine.ref(renamed), renamed.name))
        first.close()
    ...
    @Test fun restoredRenameCannotOverwriteANameChangedAfterItsBaseline() = runTest {
        ...
        form.draft.awaitSaved()
        fixture.projects.dispatch(parent.projectId, CodingMachine.Intent.RenameSession(CodingMachine.ref(parent), "Another accepted name"))
