...
class ImmunityInterventionTest {
    private class Fixture {
        ...
        var store = DefaultSessionOrganismStore(storage, journal) { now }
        val ports = TestOrganismPorts()
        var service = testOrganismService(store, projects, settings, ports) { source }
        ...
        suspend fun initialize(limits: OrganismLimits = OrganismLimits()) {
            ...
            ports.canRecreateAfterQuarantine = { true }
            ports.startChild = { child, _ -> starts++; store.beginRun(id, child.id); service.project(store.get(id)) }
            ports.stopSubtree = { ids ->
                stops++
    ...
    @Test fun complaintAloneNeverCreatesActionProposalAndEvidenceSurvivesRestart() = runTest {
        ...
        assertEquals(proposal, DefaultSessionOrganismStore(f.storage, f.journal).get(f.id).interventions.single())
    ...
    @Test fun deletionUsesConfirmedProposalIdentityAndRetainsAuditAfterTombstone() = runTest {
        ...
        f.service.approveImmunityIntervention(f.id, proposal.id, ImmunityAction.DELETE_HISTORY) { project, target ->
            ...
            f.ports.stopSubtree(accepted.affected)
    ...
    @Test fun lostExternalCompletionNeverRepeatsARecoveryLaunch() = runTest {
        val f = Fixture(); f.initialize(); val child = f.child(); f.fail(child)
        val proposal = assertNotNull(f.signal(child))
        f.ports.startChild = { session, _ -> f.starts++; f.store.beginRun(f.id, session.id); error("lost launch response") }
    ...
    @Test fun aggregateUnknownIntegrationBlocksRecoveryEvenWithoutAToolReceipt() = runTest {
        ...
        f.service.shutdown()
        f.store = DefaultSessionOrganismStore(f.storage, io.aequicor.magicpaper.data.storage.InMemoryEventJournal()) { f.now }
        f.service = testOrganismService(f.store, f.projects, f.settings, f.ports) { f.source }
        assertFailsWith<IllegalArgumentException> { f.service.approveImmunityIntervention(f.id, proposal.id, ImmunityAction.RECREATE) }
