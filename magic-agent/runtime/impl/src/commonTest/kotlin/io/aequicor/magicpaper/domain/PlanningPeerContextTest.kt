...
class PlanningPeerContextTest {
    ...
    private inner class Fixture(scope: TestScope) {
        ...
        val execution = PlanningExecutionService(store, NoopCodingRuntime, projects, profiles, settings,
            ...
            workspaces = LocalPlanningWorkspace(), scope = scope.backgroundScope,
        ...
        val service = OrchestrationService(store, execution, projects, profiles, settings, PlanComposer(gateway), gateway,
            scope.backgroundScope, workerDispatcher = StandardTestDispatcher(scope.testScheduler), modelDossiers = io.aequicor.magicpaper.domain.testModelDossiers()).also { executionPorts.chatHooks = it }
        suspend fun initialize(vararg plans: Plan) {
            projects.createTestProject(CodingProject("project", "Project", "/shared", 1)); profiles.save(profile)
            settings.save(AppSettings(activeLlmProfileId = profile.id))
            for (plan in plans) {
                projects.createTestSession(owner(plan)); store.save(plan)
                for (stage in plan.milestones) projects.createTestSession(CodingSession(stage.attempts.firstOrNull()?.sessionId ?: "plan-${plan.id}-stage-${stage.id}",
    ...
    @Test fun repeatedPeerResultsStayContextAndNeverReopenWorkOrMultiplyMessages() = runTest {
        ...
        repeat(5) { turn ->
            f.store.seed(source.id) { p -> p.copy(milestones = p.milestones.map { stage -> stage.copy(attempts = stage.attempts.map { a -> a.copy(
    ...
    @Test fun archivedSourceOrRecipientNeverSendsOrReceivesAutomaticContext() = runTest {
        for (archiveSource in listOf(false, true)) {
            val source = sample("source"); val peer = sample("peer")
            val f = Fixture(this); f.initialize(source, peer)
            owner(if (archiveSource) source else peer).let { f.projects.dispatch(it.projectId, CodingMachine.Intent.ArchiveSession(CodingMachine.ref(it), true)) }
            assertEquals(StageTurnAction.VERIFY, f.finish(source.id).action)
    ...
    @Test fun bootAndInstructionsClearSyntheticQueueWithoutErasingHistoricalCards() = runTest {
        ...
        owner(source).let { f.projects.dispatch(it.projectId, CodingMachine.Intent.ArchiveSession(CodingMachine.ref(it), true)) }
        ...
        f.projects.publishTestHistory(target.projectId, workerId, listOf(card))
