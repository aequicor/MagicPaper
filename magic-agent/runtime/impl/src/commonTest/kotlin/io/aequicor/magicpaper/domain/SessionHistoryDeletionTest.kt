...
class SessionHistoryDeletionTest {
    ...
    @Test fun deletionProjectsDurableTombstonesForEveryDescendantAndImmunity() = runTest {
        ...
        f.service.deleteHistory(f.project.id, f.root.id)
        val recovered = restarted(f)
        try { recovered.recover() } finally { recovered.shutdown() }
        assertTrue(f.projects.sessions(f.project.id).isEmpty())
        assertTrue(f.projects.states.value.getValue(f.project.id).removedSessions.containsAll(expected))
    }

    private suspend fun fixture(): SessionOrganismTestFixture = SessionOrganismTestFixture().also {
        it.initialize(CodingInteractionMode.PLANNING); it.ports.startChild = { _, _ -> }; installConfirmedStop(it)
    }

    private fun installConfirmedStop(f: SessionOrganismTestFixture, observed: (Set<String>) -> Unit = {}) {
        f.ports.stopSubtree = { ids ->
            observed(ids)
    ...
    private fun restarted(f: SessionOrganismTestFixture) = testOrganismService(DefaultSessionOrganismStore(f.storage, f.journal) { 1_000 }, f.projects, f.settings, ports = f.ports)
    ...
    @Test fun deletingRootAlsoDisposesImmunityAndProjectRecoveryCannotResurrectIt() = runTest {
        ...
        assertTrue(f.projects.sessions(f.project.id).isEmpty())
        f.projects.dispatch(f.project.id, CodingMachine.Intent.DeleteProject)
        val resumed = restarted(f)
    ...
    @Test fun missingLegacyProjectOrRootIsNotRecreatedByAggregateDiscovery() = runTest {
        for (deleteProject in listOf(false, true)) {
            val f = fixture()
            if (deleteProject) f.projects.dispatch(f.project.id, CodingMachine.Intent.DeleteProject) else f.projects.dispatch(f.project.id, CodingMachine.Intent.DeleteSession(CodingMachine.SessionRef(f.root.id, f.root.runtimeGeneration)))
    ...
    @Test fun deletionWaitsForActualChildCleanupAndRejectsNewProjectAdmission() = runTest { withContext(Dispatchers.Default) {
        ...
        f.ports.cancelQuestions = { error("question cleanup write failed") }
        ...
        try {
            ...
            assertEquals(f.store.get(f.root.organismId!!).sessions.keys, ids)
            assertTrue("session-child" in ids)
            assertTrue(f.projects.sessions(f.project.id).isEmpty())
