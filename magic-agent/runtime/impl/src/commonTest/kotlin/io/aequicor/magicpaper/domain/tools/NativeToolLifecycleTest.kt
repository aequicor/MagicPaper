...
class NativeToolLifecycleTest {
    ...
    private fun started() = CodingEvent.ToolStarted("exec_command", "./gradlew :feature:session:impl:check", "exec", isExec = true)
    ...
    @Test fun lateEventsCannotReopenConfirmedNativeCommandOrChangeItsTimelineResult() = runTest {
        terminalPhases.forEach { terminal ->
            ...
            val host = testToolSessions(store, unknownOutcome = { _, receipt -> uncertain += receipt })
            ...
            assertEquals(listOf(CodingEvent.Finished), flowOf(CodingEvent.Finished).withTools(session).toList())
    ...
    @Test fun restartedSessionDiscardsLateEventsAgainstPersistedNativeCompletion() = runTest {
        terminalPhases.forEach { terminal ->
            ...
            val first = testToolSessions(StoredToolReceipts(storage)).session(context)
            ...
            val restarted = testToolSessions(StoredToolReceipts(storage)).session(context)
            ...
            assertEquals(listOf(CodingEvent.Finished), flowOf(CodingEvent.Finished).withTools(restarted).toList())
        }
    }
    ...
    @Test fun uncertainCommandNeedsNativeTerminalEvidenceAndAllowsHistoricalScopeRecovery() = runTest {
        terminalPhases.forEach { terminal ->
            ...
            val uncertain = mutableListOf<ToolReceipt>()
            val host = testToolSessions(store, checkScope = { require(live) { "Execution revoked" } },
                checkReplayScope = { scope -> assertEquals(context, scope); historicalChecks++ },
                unknownOutcome = { _, receipt -> uncertain += receipt })
            val session = host.session(context)
    ...
    @Test fun separatelyReconciledTerminalReceiptPreventsSyntheticUnknownAtStreamEnd() = runTest {
        terminalPhases.forEach { terminal ->
            val store = MemoryToolReceiptStore()
            val uncertain = mutableListOf<ToolReceipt>()
            val session = testToolSessions(store, unknownOutcome = { _, receipt -> uncertain += receipt }).session(context)
            val timeline = flow {
                emit(started())
    ...
    @Test fun oldGenerationCannotResolveNativeUncertainty() = runTest {
        val store = MemoryToolReceiptStore()
        val host = testToolSessions(store)
        ...
        val revoked = testToolSessions(store, checkReplayScope = { error("Historical generation revoked") }).session(context)
