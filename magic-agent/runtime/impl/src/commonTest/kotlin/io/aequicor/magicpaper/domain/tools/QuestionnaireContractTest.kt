...
class QuestionnaireContractTest {
    ...
    @Test fun rejectedCallReturnsTheConcreteProblemToTheAgent() = runTest {
        ...
        assertTrue(host.questionnaires.questions.requests.value.isEmpty(), "A rejected questionnaire must not reach the user")
        ...
        assertTrue(host.questionnaires.questions.requests.value.isEmpty())
    }
    ...
    @Test fun acceptedCallInRussianWaitsAndReturnsConfirmedAnswers() = runTest {
        ...
        host.questionnaires.questions.respond(request.id, listOf(PlanningAnswer("q0", selected = listOf("o0"))))
