...
class PlanningCheckpointReloadTest {
    @Test fun verificationRestoredWhilePreparingInstructionsNeverReplaysTheWorker() = runTest {
        ...
        val projects = journalCodingProjects(kv, json).also { it.createTestProject(CodingProject("project", "Project", "/fake", 1)) }
        ...
        val service = PlanningExecutionService(store, runtime, projects, profiles, JsonSettingsRepository(kv, json), verifier, workspace, backgroundScope,
            attemptAuthority = ports, chatHooksProvider = { ports.chatHooks })
        var admissions = 0
        ports.onPrepareAttempt = { _, _, attempt -> admissions++; attempt }
        ports.chatHooks = object : PlanningExecutionHooks {
            ...
            override suspend fun instructions(plan: Plan, stage: Milestone, attempt: StageAttempt): String {
                store.command(plan.id, PlanningMachine.Fact.StageTransitioned(checkNotNull(store.currentAdmission(plan.id)),
                    stage.id, PlanningMachine.AttemptRef.from(attempt), StageEvent.PlannerDecided(null), stamp = PlanningMachine.Stamp("restored-verification", 10)))
