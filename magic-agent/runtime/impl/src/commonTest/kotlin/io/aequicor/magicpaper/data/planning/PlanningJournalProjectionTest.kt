...
class PlanningJournalProjectionTest {
    ...
    @Test fun missingCheckpointsAreRebuiltWithExactlyTheAcceptedMessageIdsAndTimes() = runTest {
        ...
        val accepted = store.seed(initial.id) { it.copy(phase = ExecutionPhase.COMPLETE) }
    ...
    @Test fun newPlanIsDiscoverableWhenItsFirstCheckpointWasNeverWritten() = runTest {
        ...
        assertEquals(DecisionCompiler.migrate(initial), TestPlanningStore(repo, events).planFor(initial.id))
    }
    ...
    @Test fun staleSnapshotCannotOverwriteAnAcceptedChangeOrConsumeARevision() = runTest {
        ...
        val first = TestPlanningStore(repo, events)
        first.save(initial)
        val second = TestPlanningStore(repo, events)
        val stale = second.planFor(initial.id)!!
        val accepted = first.edit(initial.id, stale.revision) { it.copy(goal = "Winner") }
        val records = events.read(initial.id)
        assertFailsWith<PlanningRevisionConflictException> {
            second.edit(initial.id, stale.revision) { it.copy(goal = "Stale") }
        }
    ...
    @Test fun deletionFenceWinsOverOldCheckpointsAndRejectsStaleWriters() = runTest {
        ...
        val events = InMemoryEventJournal()
        var failDelete = true
        val owner = TestPlanningStore(object : PlanningCheckpointStore by repo {
        ...
        owner.save(initial)
        val stale = TestPlanningStore(repo, events)
        stale.plans()
        ...
        assertFailsWith<PlanningRevisionConflictException> { stale.edit(initial.id) { it.copy(goal = "resurrect") } }
        failDelete = false
        owner.recover()
        assertNull(owner.planFor(initial.id))
        assertNull(repo.planFor(initial.id))
        assertFailsWith<IllegalArgumentException> { owner.save(initial) }
        assertTrue(events.read(initial.id).isEmpty())
    }

    @Test fun exactLostAppendAcknowledgementIsRecoveredWithoutRepeatingTheCommand() = runTest {
        val repo = repository()
        ...
        store.save(initial)
        loseAck = true
        store.edit(initial.id) { it.copy(goal = "Accepted") }
    ...
    @Test fun recoveryErasesFencedPayloadBeforeDroppingItsLastCheckpoint() = runTest {
        ...
        val store = TestPlanningStore(repo, DurableEventJournal(backend))
        store.save(initial)
        failCleanup = true
        ...
        failCleanup = false
        val reopened = TestPlanningStore(repo, DurableEventJournal(backend))
        reopened.recover()
    ...
    @Test fun outcomePreservesAnotherWritersAcceptedChanges() = runTest {
        ...
        val events = InMemoryEventJournal()
        val first = TestPlanningStore(repo, events)
        first.save(initial.copy(milestones = listOf(Milestone("stage", "Stage", description = "Work"))))
        first.command(initial.id, PlanningMachine.Intent.Start("run", PlanningRulesSettings().snapshot(), PlanningMachine.Stamp("start", 1)))
        first.withJournaledIntent(initial.id, PlanJournalOperation.AGENT_INTENT) {
            val second = TestPlanningStore(repo, events)
            second.edit(initial.id) { it.copy(goal = "Changed concurrently") }
            complete()
        }
    ...
    @Test fun operationAndStateCannotBeSeparatedByACheckpointFailure() = runTest {
        ...
        assertFailsWith<PlanningPersistenceException> {
            store.command(initial.id, PlanningMachine.Intent.Stop(PlanningMachine.Stamp("stop", 9)))
        }
        val reopened = TestPlanningStore(repo, events)
        assertTrue(reopened.planFor(initial.id)!!.stopping)
        assertEquals(1, reopened.unsettled(initial.id).size)
        assertEquals(2, events.read(initial.id).count { it.operation != PLAN_INPUT_OPERATION || PlanInputCommit.from(it)?.input is PlanningMachine.Intent.Create }, "Seed plus one atomic operation/state record")
    }
}
