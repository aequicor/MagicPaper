...
class PlanningChatServiceTest {
    ...
    private inner class Fixture(scope: TestScope, val kv: KeyValueStore = InMemoryKeyValueStore(),
        ...
        val profiles = JsonLlmProfileRepository(kv, json, secrets = InMemorySecretStore())
        val settings = JsonSettingsRepository(kv, json, secrets = InMemorySecretStore())
        val gateway = Gateway()
        ...
        val execution = PlanningExecutionService(store, runtime, projects, profiles, settings, object : MilestoneVerifier {
            ...
            private val stageRoots = mutableMapOf<String, PlanWorkspace>()
            override suspend fun prepare(project: CodingProject, runId: String, operation: WorkspaceOperation): PlanWorkspace {
                val direct = localWorkspace.prepare(project, runId, operation)
                return if (isolatedWorkspaces)
                    PlanWorkspace(project.path, "${project.path}/.test-worktrees/integration-$runId", git = true)
                else direct
            }
            override suspend fun stage(project: CodingProject, workspace: PlanWorkspace, attempt: StageAttempt, operation: WorkspaceOperation): StageAttempt {
                if (!isolatedWorkspaces) return localWorkspace.stage(project, workspace, attempt, operation)
                localWorkspace.validateIntegration(workspace, operation)
                require(stageRoots[attempt.id]?.let { it == workspace } != false)
                stageRoots[attempt.id] = workspace
                return attempt.copy(path = "${workspace.integrationPath}/stage-${attempt.id}")
            }
            private suspend fun requireStage(attempt: StageAttempt, operation: WorkspaceOperation): PlanWorkspace {
                val workspace = checkNotNull(stageRoots[attempt.id])
                require(attempt.path == "${workspace.integrationPath}/stage-${attempt.id}")
                // A stage copy borrows its own integration root's actual live handle.
                localWorkspace.validateIntegration(workspace, operation)
                return workspace
            }
            override suspend fun capture(attempt: StageAttempt, operation: WorkspaceOperation): String {
                if (!isolatedWorkspaces) return localWorkspace.capture(attempt, operation)
                requireStage(attempt, operation)
                return ""
            }
            override suspend fun integrate(workspace: PlanWorkspace, attempt: StageAttempt, operation: WorkspaceOperation): Boolean {
                if (isolatedWorkspaces) require(requireStage(attempt, operation) == workspace)
                return localWorkspace.integrate(workspace, attempt, operation)
            }
            override suspend fun verificationSnapshot(path: String, operation: WorkspaceOperation?) = "fixture-snapshot"
        }, scope = scope.backgroundScope, attemptAuthority = executionPorts, chatHooksProvider = { executionPorts.chatHooks })
        val service = PlanningChatService(store, execution, projects, profiles, settings, composer, gateway, scope.backgroundScope,
            workerDispatcher = StandardTestDispatcher(scope.testScheduler), modelDossiers = testModelDossiers()).also { executionPorts.chatHooks = it }
        suspend fun initialize() {
            if (projects.all().none { it.id == project.id }) projects.createTestProject(project); profiles.save(profile); settings.save(AppSettings(activeLlmProfileId = profile.id, agentLimits = OrganismLimits(retries = retryLimit)))
            service.bootstrap()
        }
        suspend fun session(id: String, engine: CodingEngine? = null): CodingSession = CodingSession(id, project.id, id, 1, planningMode = true,
            modelSelection = ModelSelection(profile.id, "m"), engine = engine).also { projects.createTestSession(it) }
    ...
    private suspend fun Fixture.acknowledgeStoppedWorkers(planId: String) {
        val pending = store.unsettled(planId)
        assertTrue(pending.isNotEmpty(), "A dispatched worker needs an explicit outcome decision")
        val plan = checkNotNull(store.planFor(planId))
        for (session in projects.sessions(plan.projectId)) {
            if (execution.reconcileJournalQuarantine(session, confirmed = false) == QuarantineRecoveryOutcome.NEEDS_CONFIRMATION)
                assertEquals(QuarantineRecoveryOutcome.RESOLVED, execution.reconcileJournalQuarantine(session, confirmed = true))
        }
        assertTrue(store.unsettled(planId).isEmpty())
    }

    /** Explicit owner admission without launching a worker, for scheduler-only fixtures. */
    private suspend fun Fixture.admitScheduler(planId: String) {
        store.command(planId, PlanningMachine.Intent.Start("run", PlanningRulesSettings().snapshot(),
            PlanningMachine.Stamp("scheduler-start", 1)))
    }
    ...
    @Test fun verificationSkipMustConfirmTheAcceptanceSnapshotShownToTheUser() = runTest {
        val f = Fixture(this); f.initialize()
        val parent = f.session("parent")
        val plan = f.readyPlan("plan", parent)
        val criteria = plan.milestones.single().criteria()
        val record = AcceptanceRecord("run", "attempt", "shown-snapshot", criteria,
            criteria.map { AcceptanceFinding(it.id, CheckStatus.NOT_RUN, it.description, "Missing check") }, status = AcceptanceStatus.PARTIAL)
        val issue = PlanningIssue(IssueKind.VERIFICATION, record.summary(), requiresUser = true)
        val attempt = StageAttempt("attempt", "worker", StageAssignment(profile.id, "m"), phase = AttemptPhase.VERIFYING,
            report = "Implementation is ready", error = issue, acceptanceRecord = record, path = "/shared")
        val blocked = plan.copy(runId = "run", phase = ExecutionPhase.WAITING, issue = issue, confirmedRevision = 1,
            milestones = plan.milestones.map { it.copy(attempts = listOf(attempt)) })
        f.store.save(blocked)
        val shown = f.interactions(parent).single { it.kind == InteractionKind.RECOVER_PLAN }
        assertEquals(record, assertNotNull(shown.verificationProofs).single().acceptance)
        f.store.seed(plan.id) { latest ->
            latest.copy(milestones = latest.milestones.map { stage -> stage.copy(attempts = stage.attempts.map {
                assertEquals(record, it.acceptanceRecord)
                it.copy(acceptanceRecord = record.copy(snapshotId = "new-snapshot", reviewId = "new-review"))
            }) })
        }
        val current = f.interactions(parent).single { it.kind == InteractionKind.RECOVER_PLAN }
        assertEquals(shown.id, current.id, "Issue and binding tokens remain unchanged when only acceptance changes")
        assertNotEquals(shown.verificationProofs, current.verificationProofs)
        val answers = listOf(PlanningAnswer("decision", listOf("skip_verification")))
        assertFailsWith<IllegalArgumentException> { f.service.submitInteraction(shown, answers) }
        assertFailsWith<IllegalArgumentException> { f.service.submitInteraction(current.copy(verificationProofs = null), answers) }
        assertTrue(f.store.planFor(plan.id)!!.acceptanceWaivers.isEmpty())
        assertTrue(f.projects.messages(project.id, parent.id).none { it.id == shown.id + "-verification-skipped" })
        assertTrue(f.runtime.calls.isEmpty()); assertTrue(f.gateway.requests.isEmpty())
        f.service.submitInteraction(current, answers)
        assertEquals("new-snapshot", f.store.planFor(plan.id)!!.acceptanceWaivers.single().snapshotId)
    }
    ...
    @Test fun confirmationRejectsStaleRevisionAndNoKeepsPlanWithoutStarting() = runTest {
        ...
        f.store.seed(plan.id) { it.copy(goal = "Уточнённая цель") }
    ...
    @Test fun decliningProposalAfterCompletionClearsItSoStatusTurnsIdle() = runTest {
        ...
        f.store.seed(base.id) { it.copy(phase = ExecutionPhase.COMPLETE, status = PlanStatus.DONE,
    ...
    @Test fun restartRecoversUnavailableAssignmentsAndCoordinatesCheckpointBeforeStartingAnotherWorkerTurn() = runTest {
        ...
        assertTrue(restored.verificationReports.isEmpty())
        restored.service.control(base.id, "resume"); runCurrent()
    ...
    @Test fun shutdownRetainsQueuedInputWithoutExecutingItOnRestore() = runTest {
        for (stop in listOf(false, true)) {
            ...
            assertTrue(restored.runtime.calls.isEmpty())
            assertEquals(if (stop) OrchestrationInputStatus.CANCELLED else OrchestrationInputStatus.QUEUED,
    ...
    @Test fun resumeRestartsStoppedPlanInSameRunAndDoesNothingAfterCompletion() = runTest {
        ...
        val beforeResume = f.runtime.calls.size
        f.service.resume(parent); runCurrent()
        assertEquals(beforeResume, f.runtime.calls.size)
        assertTrue(f.store.unsettled(base.id).isNotEmpty())
        f.acknowledgeStoppedWorkers(base.id)
    ...
    @Test fun orphanWorkerCanBeArchivedRestoredAndRenamedWithoutPlan() = runTest {
        ...
        f.projects.createTestSession(worker)
    ...
    @Test fun orphanWorkerWithStalePlanCanBeArchivedAndDeleted() = runTest {
        ...
        f.projects.dispatch(project.id, CodingMachine.Intent.DeleteSession(CodingMachine.ref(parent)))
        assertFalse(f.service.hasLiveOrchestrator(worker))
        f.service.archiveSession(worker.id); runCurrent()
        assertTrue(f.projects.sessions(project.id).first { it.id == worker.id }.archived)
        assertFailsWith<IllegalArgumentException> { f.service.deleteSessionTree(project.id, worker.id) }
        assertNotNull(f.store.planFor(plan.id))
        assertTrue(f.projects.sessions(project.id).any { it.id == worker.id })
        f.acknowledgeStoppedWorkers(plan.id)
    ...
    @Test fun deletingOrchestratorStopsWorkersAndPreservesSiblingSessions() = runTest {
        ...
        assertFailsWith<IllegalArgumentException> { f.service.deleteSessionTree(project.id, parent.id) }
        assertNotNull(f.store.planFor(plan.id))
        assertTrue(f.projects.sessions(project.id).any { it.id == parent.id })
        f.acknowledgeStoppedWorkers(plan.id)
    ...
    @Test fun firstDiscussionLeavesTheNameToTheTitleServiceAndKeepsTheRequest() = runTest {
        ...
        f.projects.dispatch(project.id, CodingMachine.Intent.RenameSession(CodingMachine.ref(parent), parent.name))
    ...
    @Test fun deletingAllSessionsStopsPlansAndPreservesOtherProjects() = runTest {
        ...
        f.projects.createTestProject(other)
        f.projects.createTestSession(CodingSession("other-session", other.id, "Keep", 1))
        ...
        assertFailsWith<IllegalArgumentException> { f.service.deleteProjectSessions(project.id) }
        assertNotNull(f.store.planFor(plan.id))
        assertTrue(f.projects.sessions(project.id).isNotEmpty())
        f.acknowledgeStoppedWorkers(plan.id)
    ...
    @Test fun planningModeCannotBeRevertedEvenWithStaleSession() = runTest {
        ...
        f.projects.createTestSession(ordinary)
    ...
    @Test fun exhaustedVerificationPublishesReasonWithoutInventingQuestionAndRetryRunsRepair() = runTest {
        ...
        val worker = f.projects.sessions(project.id).single { it.stageId != null }
        assertEquals(2, f.runtime.calls.size, "${f.service.error.value}; ${f.store.plans.value.map { it.issue }}")
        ...
        f.store.seed(plan.id) { it }; runCurrent()
    ...
    @Test fun restartVerifiesOriginalEvidenceInsteadOfRerunningWorkerForShortAcknowledgement() = runTest {
        ...
        assertTrue(restored.runtime.calls.isEmpty())
        assertTrue(restored.verificationReports.isEmpty())
        restored.service.control(plan.id, "resume"); runCurrent()
        restored.runtime.gate.complete(Unit); advanceTimeBy(1000); runCurrent()
    ...
    @Test fun repeatedHandoffKeepsEachReplyAfterItsIncomingMessageAndSurvivesReload() = runTest {
        ...
        f.store.seed(plan.id) { it.copy(updatedAt = it.updatedAt + 1) }; runCurrent()
    ...
    @Test fun legacyCombinedReplyIsSplitAroundTheUserAnswerWithoutLosingActivity() = runTest {
        ...
        val worker = f.projects.sessions(project.id).single { it.id == workerId }
        f.projects.dispatch(project.id, CodingMachine.Intent.ReplaceHistory(CodingMachine.ref(worker),
            f.projects.messages(project.id, workerId), listOf(
        ...
        f.projects.publishTestHistory(project.id, parent.id, listOf(
        ...
        f.store.seed(plan.id) { it.copy(confirmedRevision = plan.confirmedRevision, milestones = plan.milestones) }; runCurrent()
    ...
    @Test fun questionsUseHistoryAndAnswersAreStoredOnce() = runTest {
        ...
        f.projects.publishTestHistory(project.id, session.id, listOf(CodingMessage("history", CodingRole.USER, "Existing context", createdAt = 1)))
    ...
    @Test fun answeredRecoveryLimitReturnsToCoordinatorInsteadOfRepeatingHelp() = runTest {
        ...
        f.store.seed(plan.id) { it.copy(milestones = listOf(stage.copy(attempts = listOf(nextAttempt)))) }
    ...
    @Test fun answerArrivingBeforeWorkerSavesWaitingStateResumesTheStage() = runTest {
        ...
        f.gateway.coordinator = """{"reply":"Уточните формат","askUser":true}"""
        var answered = false
        f.executionPorts.chatHooks = object : PlanningExecutionHooks by f.service {
    ...
    @Test fun legacyMigrationDoesNotStartRuntimeOrDuplicateMessages() = runTest {
        val f = Fixture(this)
        f.projects.createTestProject(project); f.profiles.save(profile)
    ...
    @Test fun sessionNamesAndNumbersSurviveReorderingArchivalAndRenaming() = runTest {
        ...
        f.store.seed("p") { it.copy(milestones = it.milestones.reversed()) }
        f.service.prepareSessions(f.store.planFor("p")!!)
        ...
        f.store.seed("p") { it.copy(milestones = it.milestones.map { stage -> stage.copy(status = MilestoneStatus.DONE) }) }
    ...
    @Test fun activePlanProposalHoldsWorkerUntilApprovedRequirementsAreDurable() = runTest {
        ...
        assertEquals(1, f.runtime.calls.size, "Approval does not prove the interrupted worker outcome")
        assertTrue(f.store.unsettled("p").isNotEmpty())
        f.execution.stopAndJoin("p")
        f.acknowledgeStoppedWorkers("p")
        f.service.control("p", "resume"); advanceTimeBy(200); runCurrent()
        assertEquals(2, f.runtime.calls.size, f.store.planFor("p")?.issue?.message)
    ...
    @Test fun designClarificationPausesDesignWhileResearchContinuesAndDeliversBeforeResume() = runTest {
        ...
        assertEquals(2, f.runtime.calls.size, "Answering the question does not resolve the interrupted worker outcome")
        assertEquals(1, f.runtime.calls.count { it.first.id.endsWith("research") })
        assertFalse(f.store.planFor("p")!!.milestones.single { it.id == "research" }.attempts.single().interrupted)
        f.execution.stopAndJoin("p")
        f.acknowledgeStoppedWorkers("p")
        f.service.control("p", "resume"); advanceTimeBy(300); runCurrent()
        assertEquals(4, f.runtime.calls.size, f.store.planFor("p")?.issue?.message)
        val resumed = f.runtime.calls.last { it.first.id.endsWith("stage") }
    ...
    @Test fun withdrawnInputSurvivesRestartAndIsExcludedFromTheFirstPlanContext() = runTest {
        val f = Fixture(this)
        f.projects.createTestProject(project)
        val parent = f.session("parent")
        ...
        f.projects.recordTestOrchestration(OrchestrationState(parent.id, project.id, inputs = listOf(input)))
        // Simulate stopping after the inbox was saved but before its chat projection was updated.
        f.projects.publishTestHistory(project.id, parent.id, listOf(CodingMessage(input.id, CodingRole.USER,
    ...
    @Test fun failedWithdrawalCheckpointKeepsDurableCancellationAndExposesStorageNotice() = runTest {
        val backing = InMemoryKeyValueStore()
        var fail = false
        val disk = object : KeyValueStore by backing {
        ...
        assertNotNull(f.projects.failures.value[project.id])
    ...
    @Test fun completedPlanCanBeDiscussedAndExtendedOnlyAfterConfirmingTheProposal() = runTest {
        ...
        assertEquals(MilestoneStatus.DONE, verifying.milestones.single { it.title == "Обработка ошибок" }.status, "${f.service.error.value}; ${verifying.issue}")
    ...
    @Test fun systemNoticesStayInStoredHistoryButDoNotEnterModelContext() = runTest {
        ...
        f.projects.publishTestHistory(project.id, parent.id, listOf(notice, answer))
    ...
    @Test fun bootRestoresStoredInboxWithoutExecutingSavedDecisionsOrRepeatingReplies() = runTest {
        val f = Fixture(this)
        f.projects.createTestProject(project); f.profiles.save(profile); f.settings.save(AppSettings(activeLlmProfileId = profile.id, agentLimits = OrganismLimits(retries = 2)))
        val parent = f.session("parent")
        val base = f.readyPlan("p", parent)
        f.store.save(base.copy(dialogue = listOf(PlanningMessage("applied-reply", "assistant", "Уже сохранено"))))
        f.projects.recordTestOrchestration(OrchestrationState(parent.id, project.id, activePlanId = "p", inputs = listOf(
        ...
        f.service.bootstrap(); runCurrent()
        assertEquals(listOf(OrchestrationInputStatus.PROCESSING, OrchestrationInputStatus.QUEUED), f.projects.orchestration(parent.id)!!.inputs.map { it.status })
        assertTrue(f.gateway.lastMessages.isEmpty())
        assertEquals(0, f.projects.messages(project.id, parent.id).count { it.id == "queued-reply" })
    ...
    @Test fun fullAnswerForwardsEverySavedPartToTheAffectedWorker() = runTest {
        ...
        f.projects.recordTestOrchestration(OrchestrationState(parent.id, project.id, activePlanId = "p", questions = listOf(
    ...
    @Test fun bootReconstructsMissingDeliveryCardsOnlyOnce() = runTest {
        ...
        f.store.seed("p") { it.copy(goal = "Новое имя цели") }; runCurrent()
        assertEquals(routed, f.projects.messages(project.id, parent.id).single { it.id == "saved-routed" })
    }

    @Test fun failedInitialCheckpointRetainsAcceptedInputAndObservableStorageFailure() = runTest {
        val backing = InMemoryKeyValueStore()
        var fail = false
        val disk = object : KeyValueStore by backing {
            override fun write(key: String, value: String) {
        ...
        f.service.send(parent, "Сделай редактор"); runCurrent()
        assertNotNull(f.projects.failures.value[project.id])
        assertEquals("Сделай редактор", f.projects.orchestration(parent.id)!!.inputs.single().text)
        assertEquals(OrchestrationInputStatus.DONE, f.projects.orchestration(parent.id)!!.inputs.single().status)
        val calls = f.gateway.requests.size
        fail = false
        f.service.recoverOrchestration(parent.id); runCurrent()
        assertEquals(calls, f.gateway.requests.size)
        assertTrue(f.service.persistenceErrors.value.isEmpty())
    ...
    @Test fun damagedCompatibilityCacheIsRebuiltFromJournalWithoutRunningEitherInbox() = runTest {
        val f = Fixture(this)
        f.projects.createTestProject(project); f.profiles.save(profile); f.settings.save(AppSettings(activeLlmProfileId = profile.id, agentLimits = OrganismLimits(retries = 2)))
        ...
        f.projects.recordTestOrchestration(OrchestrationState(healthy.id, project.id, activePlanId = "good", inputs = listOf(
            OrchestrationInput("queued", "Расскажи", 2, decision = UserTurnDecision(UserTurnIntent.DISCUSS, "Ответ")))))
        f.service.bootstrap(); runCurrent()
        assertTrue(f.service.persistenceErrors.value.isEmpty())
        assertEquals(OrchestrationInputStatus.QUEUED, f.projects.orchestration(healthy.id)!!.inputs.single().status)
        assertEquals(0, f.projects.messages(project.id, healthy.id).count { it.id == "queued-reply" })
        assertTrue(f.gateway.requests.isEmpty()); assertTrue(f.runtime.calls.isEmpty())
    }
    ...
    @Test fun retryAfterRestartReconsidersInvalidCachedScheduleWithoutRerunningWorker() = runTest {
        ...
        f.projects.recordTestOrchestration(OrchestrationState(parent.id, project.id, activePlanId = base.id, inputs = listOf(input)))
        f.execution.shutdown(); runCurrent()
    ...
    @Test fun restartedCoordinatorRepairsCachedCancellationOfDeliveredRule() = runTest {
        ...
        f.projects.recordTestOrchestration(OrchestrationState(parent.id, project.id, activePlanId = base.id,
    ...
    @Test fun fullAnswerProducesOneDurableEventAndPartialAnswerDoesNot() = runTest {
        ...
        f.projects.recordTestOrchestration(OrchestrationState(parent.id, project.id, activePlanId = p.id, questions = listOf(q)))
        f.service.recoverOrchestration(parent.id); runCurrent()
        ...
        assertEquals(ScheduledMessageStatus.WAITING, f.store.planFor(p.id)!!.scheduledMessages.single().status,
            "Restored paused plans retain the event without authorizing scheduled work")
