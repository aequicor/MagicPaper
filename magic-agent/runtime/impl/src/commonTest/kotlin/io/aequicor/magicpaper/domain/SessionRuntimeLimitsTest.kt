...
class SessionRuntimeLimitsTest {
    private fun TestScope.runtime(f: SessionOrganismTestFixture, startedAt: Long = testScheduler.currentTime) =
        testSessionTree(f.service, f.projects, f.profiles, f.settings, f.ports, clock = { 1_000 + testScheduler.currentTime - startedAt }).also { f.ports.tree = it }
    ...
    @Test fun aReceivedUsageObservationSurvivesCancellationWhileWaitingForANewPolicy() = runTest {
        for (auxiliary in listOf(false, true)) {
            ...
            var gateNextApplication = false
            val gatedStore = object : SessionOrganismStore by f.store {
                override suspend fun loadAll(): List<SessionOrganism> {
                    if (gateNextApplication) {
                        gateNextApplication = false
            ...
            val service = testOrganismService(gatedStore, f.projects, f.settings, ports = f.ports)
            val tree = testSessionTree(service, f.projects, f.profiles, f.settings, f.ports, clock = { 1_000 }).also { f.ports.tree = it }
            ...
            f.settings.save(settings) // The settings owner committed; the runtime now applies that snapshot.
            gateNextApplication = true
            val save = async { service.applySettingsLimits(settings) }
            saving.await()
