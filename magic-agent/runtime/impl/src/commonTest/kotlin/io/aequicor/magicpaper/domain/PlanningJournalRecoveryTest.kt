...
class PlanningJournalRecoveryTest {
    private class Fixture {
        ...
        val store = TestPlanningStore(JsonPlanningRepository(InMemoryKeyValueStore(), Json), events)
    ...
    @Test fun openJournalProjectsIntoQuarantineAndCannotBeSettledByEmptyNativeEvidence() = runTest {
        ...
        f.sessions.ports.reconcileUnknownOutcomes = native::reconcile
    ...
    @Test fun confirmationRequiresTheSamePlanAndJournalThatWereInspected() = runTest {
        for (changeJournal in listOf(false, true)) {
            ...
            else f.store.seed("plan") { it.copy(goal = "New goal") }
    ...
    @Test fun aSessionSharedByTwoPlansResolvesItsExactQuarantineBatchBeforeClosingEitherPlan() = runTest {
        val f = Fixture(); f.initialize()
        f.store.save(Plan("second-plan", f.sessions.project.id, "Second goal", parentSessionId = f.sessions.root.id,
            phase = ExecutionPhase.WAITING, issue = PlanningJournalRecovery.uncertainty))
        f.events.append("second-plan", PlanJournalOperation.PREPARE_INTENT.wire, 2)
        assertEquals(QuarantineRecoveryOutcome.NEEDS_CONFIRMATION, f.recovery.reconcile(f.sessions.root, false))
        val before = f.sessions.store.get(f.sessions.root.organismId!!).unresolvedQuarantines(f.sessions.root.id)
        assertEquals(2, before.size)
        assertEquals(QuarantineRecoveryOutcome.RESOLVED, f.recovery.reconcile(f.sessions.root, true))
        assertTrue(f.store.unsettled("plan").isEmpty())
        assertTrue(f.store.unsettled("second-plan").isEmpty())
        val after = f.sessions.store.get(f.sessions.root.organismId!!)
        assertTrue(after.unresolvedQuarantines(f.sessions.root.id).isEmpty())
        assertEquals(2, after.audit.count { it.action == QUARANTINE_RESOLVED_ACTION })
        assertTrue(f.store.machineStates.value.values.all { it.run?.phase != PlanningMachine.RunPhase.RUNNING })
    }
}
