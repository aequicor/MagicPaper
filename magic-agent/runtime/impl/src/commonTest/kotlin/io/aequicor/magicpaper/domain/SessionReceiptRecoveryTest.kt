...
class SessionReceiptRecoveryTest {
    ...
    @Test fun acceptedCreationIsNotSuccessUntilDurableRuntimeObservationProvesOwnership() = runTest {
        val f = SessionOrganismTestFixture(); f.initialize(); f.ports.startChild = { _, _ -> }
    ...
    @Test fun acceptedSendRequiresDeliveryAcknowledgementAndNeverSendsDuringRecovery() = runTest {
        val f = SessionOrganismTestFixture(); f.initialize(); f.ports.startChild = { _, _ -> }; f.create("child")
    ...
    @Test fun unknownReceiptCanBeProvedWhileQuarantinedWithoutRestoringMutationAuthority() = runTest {
        val f = SessionOrganismTestFixture(); f.initialize(CodingInteractionMode.PLANNING); f.ports.startChild = { _, _ -> }; f.create("child")
        ...
        var executions = 0
        val authorizeReceipt: suspend (ToolExecutionContext, ToolDefinition) -> Unit = { context, _ ->
        ...
        val host = testToolSessions(receipts,
            receiver = { context, id, tool, args -> executions++; f.service.execute(context, id, tool, args) },
            authorizeReceipt = authorizeReceipt,
            ...
            reconcile = { context, intent -> restarted(f).reconcile(context, intent) })
        val context = f.context()
