...
class SessionTitleServiceTest {
    ...
    private inner class Fixture(answer: suspend (List<LlmMessage>) -> String) {
        ...
        val cache = JsonCodingProjectRepository(kv, json)
        val projects = journalCodingProjects(kv, json, checkpoints = cache)
        ...
        fun sync(session: CodingSession, vararg messages: CodingMessage, saved: Boolean = true) {
            runBlocking {
                if (!initialized) {
                    cache.save(project)
                    cache.saveSession(session.copy(engine = CodingEngine.PI))
                    if (saved) cache.saveMessages(project.id, session.id, messages.toList())
                    projects.start()
                    initialized = true
                } else if (saved) {
                    val current = projects.sessions(project.id).single { it.id == session.id }
                    projects.dispatch(project.id, CodingMachine.Fact.HistoryPublished(CodingMachine.SessionRef(current.id, current.runtimeGeneration), messages.toList()))
                }
            }
