...
class MessageSchedulerTest {
    ...
    @Test fun validationChecksInboxReceiptBeforeRuleStatusAndPreservesCommittedCommands() = runTest {
        val store = TestPlanningStore(JsonPlanningRepository(InMemoryKeyValueStore(), Json { encodeDefaults = true }))
        ...
        var delivered = true
        var dispatches = 0
        val scheduler = MessageScheduler(store, backgroundScope, { _, _ -> dispatches++; true }, { fail(it) }, { 300 },
        ...
        delivered = true
        val committed = store.seed(original.id) { it.copy(scheduledMessages = it.scheduledMessages.map { r -> r.copy(status = ScheduledMessageStatus.QUEUED) }) }
        assertNull(scheduler.validationProblem(original.id, edit, "edit", "orchestrator", emptySet()))
    ...
    @Test fun eventsCommitWithSourceStateAndSkippedTaskDoesNotSucceed() = runTest {
        ...
        store.save(plan())
        store.seed("plan") { p -> p.copy(milestones = p.milestones.map { if (it.id == "a") it.copy(status = MilestoneStatus.SKIPPED) else it }) }
        assertTrue(store.planFor("plan")!!.messageEvents.isEmpty())
        store.seed("plan") { p -> p.copy(milestones = p.milestones.map { if (it.id == "b") it.copy(status = MilestoneStatus.DONE, updatedAt = 200, report = "done") else it }) }
        val reopened = TestPlanningStore(JsonPlanningRepository(kv, json)).planFor("plan")!!
    ...
    @Test fun pauseRestartAndInterruptedDispatchKeepOneDurableDelivery() = runTest {
        ...
        store.save(register(trigger = MessageTrigger(MessageTriggerKind.AT_TIME, at = 200)))
        store.command("plan", PlanningMachine.Intent.Start("run", PlanningRulesSettings().snapshot(), PlanningMachine.Stamp("start", 100)))
        store.command("plan", PlanningMachine.Intent.Pause(PlanningMachine.Stamp("pause", 150)))
        ...
        val receipt: suspend (Plan, ScheduledMessage) -> Boolean = { _, rule -> rule.deliveryId in inbox }
        var scheduler = MessageScheduler(store, backgroundScope, dispatch, { fail(it) }, { 300 }, hasReceipt = receipt)
        scheduler.tick()
        assertEquals(0, calls)
        assertEquals(ScheduledMessageStatus.WAITING, store.planFor("plan")!!.scheduledMessages.single().status)
        store.command("plan", PlanningMachine.Intent.Start("run", PlanningRulesSettings().snapshot(), PlanningMachine.Stamp("resume", 300)))
        ...
        store = TestPlanningStore(JsonPlanningRepository(kv, json), journal)
        store.plans()
        assertNull(store.currentAdmission("plan"))
        scheduler = MessageScheduler(store, backgroundScope, dispatch, { fail(it) }, { 300 }, hasReceipt = receipt)
        scheduler.tick(); scheduler.tick()
        assertEquals(1, calls)
        assertEquals(ScheduledMessageStatus.READY, store.planFor("plan")!!.scheduledMessages.single().status)
        store.command("plan", PlanningMachine.Intent.Start("run", PlanningRulesSettings().snapshot(), PlanningMachine.Stamp("restart", 400)))
        scheduler.tick(); scheduler.tick()
    ...
    @Test fun restoredRunningTimerNeedsFreshAdmissionBeforeDelivery() = runTest {
        val kv = InMemoryKeyValueStore()
        val journal = InMemoryEventJournal()
        val json = Json { encodeDefaults = true }
        val original = TestPlanningStore(JsonPlanningRepository(kv, json), journal)
        original.save(register(trigger = MessageTrigger(MessageTriggerKind.AT_TIME, at = 200)))
        original.command("plan", PlanningMachine.Intent.Start("run", PlanningRulesSettings().snapshot(), PlanningMachine.Stamp("start", 100)))
        val restored = TestPlanningStore(JsonPlanningRepository(kv, json), journal)
        restored.plans()
        var calls = 0
        val scheduler = MessageScheduler(restored, backgroundScope, { _, _ -> calls++; true }, { fail(it) }, { 300 })
        scheduler.bootstrap()
        advanceTimeBy(120_000); runCurrent()
        assertNull(restored.currentAdmission("plan"))
        assertEquals(0, calls)
        assertEquals(ScheduledMessageStatus.WAITING, restored.planFor("plan")!!.scheduledMessages.single().status)
        restored.command("plan", PlanningMachine.Intent.Start("run", PlanningRulesSettings().snapshot(), PlanningMachine.Stamp("restart", 300)))
        scheduler.tick(); scheduler.tick()
        assertEquals(1, calls)
        assertEquals(ScheduledMessageStatus.QUEUED, restored.planFor("plan")!!.scheduledMessages.single().status)
    }
    ...
    @Test fun waitingLoopDoesNotDispatchOrInventEvents() = runTest {
        ...
        store.save(register())
        var calls = 0
        val scheduler = MessageScheduler(store, backgroundScope, { _, _ -> calls++; true }, { fail(it) }, { testScheduler.currentTime })
        scheduler.bootstrap()
    ...
    @Test fun staleRunCommandsAreRejectedBeforeCreatingAnyReceipt() = runTest {
        val store = TestPlanningStore(JsonPlanningRepository(InMemoryKeyValueStore(), Json { encodeDefaults = true }))
        store.save(plan())
        val scheduler = MessageScheduler(store, backgroundScope, { _, _ -> true }, { fail(it) })
    ...
    @Test fun backgroundTickFailureRemainsVisibleWithoutLeakingCauseOrDispatching() = runTest {
        val store = TestPlanningStore(JsonPlanningRepository(InMemoryKeyValueStore(), Json { encodeDefaults = true }))
        val errors = mutableListOf<String>()
        var calls = 0
        var firstTick = true
        val scheduler = MessageScheduler(store, backgroundScope, { _, _ -> calls++; true }, errors::add,
            beforeTick = {
                if (firstTick) { firstTick = false; error("sensitive storage detail") }
            })
        scheduler.bootstrap()
        runCurrent()
        assertEquals(0, calls)
        assertEquals(1, errors.size)
        assertFalse(errors.single().contains("sensitive"))
        assertContains(errors.single(), "Проверьте состояние плана")
        scheduler.pauseForReset()
    }
