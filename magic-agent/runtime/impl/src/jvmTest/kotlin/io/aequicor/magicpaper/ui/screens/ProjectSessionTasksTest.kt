...
class ProjectSessionTasksTest {
    ...
    @Test fun ordinaryConversationKeepsItsRowAfterAutomaticLifecycleAdoption() {
        ...
        assertEquals(setOf("root"), CodingUi(sessions = listOf(root, withDiagnostic))
            .projectSessionTasks("project").single().ids().toSet())
        assertEquals(setOf("child"), CodingUi(sessions = listOf(root, immunity, session("child", parent = "root")))
            .projectSessionTasks("project").single().ids().toSet())
    }
    ...
    @Test fun zygoteAndImmunityShareATaskWithoutChangingTheirRuntimeParents() {
        ...
        assertEquals(listOf("early" to 0, "grandchild" to 1, "late" to 0),
            task.visibleRows(emptySet()).map { it.item.session.id to it.depth })
        assertEquals(listOf(1, 0, 0), task.visibleRows(emptySet()).map { it.childCount })
    ...
    @Test fun nestedDisclosurePreservesHeaderOwnedRootAndImmunity() {
        ...
        assertEquals("zygote", task.rootId)
        assertEquals("immunity", task.immunityId)
        assertEquals(listOf("child"), task.ids(setOf("child")))
        assertEquals(listOf("child", "grandchild"), task.ids())
    }
    ...
    @Test fun metadataCanGroupLegacyProjectionsBeforeSessionIdentityReloads() {
        ...
        assertEquals(listOf("child" to 0),
    ...
    @Test fun missingKindMetadataUsesParentlessWorkSessionWhenTimestampsMatch() {
        ...
        assertEquals(listOf("child"), task.ids())
    }
    ...
    @Test fun archivedRootStillNamesAndOwnsItsTaskAndLegacyDescendants() {
        ...
        assertEquals("zygote", task.rootId)
        assertEquals(listOf("orphan"), task.ids())
        assertEquals(listOf(0), task.visibleRows(emptySet()).map { it.depth })
    }
    ...
    @Test fun missingRootUsesAggregateTitleAndTimestampInsteadOfImmunity() {
        ...
        assertEquals(emptyList(), tasks.last().ids())
    ...
    @Test fun missingParentsAndCyclesKeepEveryVisibleSessionExactlyOnce() {
        ...
        assertEquals(ui.sessions.filterNot { it.session.archived || it.session.id in setOf("zygote", "immunity") }.map { it.session.id }.toSet(),
            rows.map { it.item.session.id }.toSet())
        assertEquals(6, rows.size)
        assertEquals("immunity", tasks.single { it.organismId != null }.immunityId)
        assertEquals(2, tasks.size)
        assertEquals(listOf("orphan", "cycle-a", "cycle-b", "self"),
