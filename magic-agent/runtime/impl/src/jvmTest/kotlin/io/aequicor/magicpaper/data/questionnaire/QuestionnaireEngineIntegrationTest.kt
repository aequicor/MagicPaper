...
class QuestionnaireEngineIntegrationTest {
    ...
    @Test fun installedCodexCallsQuestionnaireOnNewAndResumedSessions() = runBlocking {
        ...
        try {
            repeat(2) {
                val service = CodexAppServerOpenAiSubscription(Json { ignoreUnknownKeys = true }, root.resolve("home"), binary, browser = io.aequicor.magicpaper.data.coding.testBrowserSessions, checks = io.aequicor.magicpaper.data.coding.testCommandChecks, journal = io.aequicor.magicpaper.data.storage.InMemoryEventJournal(), questionnaireFactory = io.aequicor.magicpaper.domain.testQuestionnaireFactory())
                try {
                    val observed = CopyOnWriteArrayList<CodingEvent>()
                    val events = async { service.runFixtureCoding(CodingProject("p", "Project", root.toString(), 0),
