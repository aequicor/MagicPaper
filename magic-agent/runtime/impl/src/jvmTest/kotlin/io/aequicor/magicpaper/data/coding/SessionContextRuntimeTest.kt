...
class SessionContextRuntimeTest {
    @Test fun snapshotUsesPinnedSkillsAndTheSamePromptBuilderAsTheAdapters() = runBlocking {
        ...
        val client = CodexAppServerOpenAiSubscription(Json, root.resolve("codex"), browser = io.aequicor.magicpaper.data.coding.testBrowserSessions, checks = io.aequicor.magicpaper.data.coding.testCommandChecks, journal = io.aequicor.magicpaper.data.storage.InMemoryEventJournal(), questionnaireFactory = io.aequicor.magicpaper.domain.testQuestionnaireFactory())
        try {
            var reads = 0
            val runtime = DesktopCodingRuntime(PiCodingRuntime(root.resolve("pi").toFile(), browser = io.aequicor.magicpaper.data.coding.testBrowserSessions, checks = io.aequicor.magicpaper.data.coding.testCommandChecks, journal = io.aequicor.magicpaper.data.storage.InMemoryEventJournal(), questionnaireFactory = io.aequicor.magicpaper.domain.testQuestionnaireFactory()), client,
                skillSelection = {
                    reads++
            ...
            for (engine in CodingEngine.entries) {
                ...
                assertTrue("EXACT SKILL TEXT" in planning)
