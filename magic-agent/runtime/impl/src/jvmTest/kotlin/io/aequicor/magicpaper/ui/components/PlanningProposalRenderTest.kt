...
class PlanningProposalRenderTest {
    @Test fun collapsedSummaryKeepsProposalReachableWhileShowingActualWork() = runTest {
        ...
        try {
            ...
            val checkpoints = JsonCodingProjectRepository(kv, json)
            val projects = journalCodingProjects(kv, json, checkpoints = checkpoints)
            ...
            val execution = PlanningExecutionService(store, NoopCodingRuntime, projects, profiles, settings,
                ...
                }, workspaces = LocalPlanningWorkspace(), scope = backgroundScope,
                attemptAuthority = executionPorts, chatHooksProvider = { executionPorts.chatHooks })
            val service = OrchestrationService(store, execution, projects, profiles, settings, textPlanComposer(gateway), gateway, backgroundScope, modelDossiers = io.aequicor.magicpaper.domain.testModelDossiers()).also { executionPorts.chatHooks = it }
            ...
            checkpoints.save(project); checkpoints.saveSession(parent)
            ...
            checkpoints.saveOrchestration(OrchestrationState(parent.id, project.id, activePlanId = base.id))
            ...
            for (phase in listOf(ExecutionPhase.EXECUTING, ExecutionPhase.COMPLETE)) {
                val plan = store.seed(base.id) { it.copy(proposal = proposal, phase = phase, milestones = listOf(
