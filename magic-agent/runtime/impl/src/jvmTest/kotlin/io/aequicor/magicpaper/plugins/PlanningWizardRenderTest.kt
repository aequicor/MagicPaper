    ...
    @Test fun failedLoadOffersRetryAndPreservesThePanelOnNarrowWindows() = runTest {
        ...
        try {
            ...
            val plugin = CodingPlanningPlugin(store, textPlanComposer(gateway), DossierResearcher(gateway, search),
                execution, NoopCodingRuntime, null, profiles, settings, InMemoryDraftRepository(), backgroundScope, modelDossiers = io.aequicor.magicpaper.domain.testModelDossiers())
    ...
    @Test fun rendersFourStepsOnDesktopAndPhone() = runTest {
        ...
        try {
            ...
            for (step in PlanningStep.entries) for (width in listOf(1000, 390)) {
                ...
                val executionPorts = TestPlanningExecutionPorts()
                val service = PlanningExecutionService(store, NoopCodingRuntime, null, profiles, settings, verifier, workspaces = LocalPlanningWorkspace(), scope = backgroundScope,
                    attemptAuthority = executionPorts, chatHooksProvider = { executionPorts.chatHooks })
                val drafts = InMemoryDraftRepository()
                val plugin = CodingPlanningPlugin(store, textPlanComposer(gateway), DossierResearcher(gateway, search), service, NoopCodingRuntime, null, profiles, settings, drafts, backgroundScope, modelDossiers = io.aequicor.magicpaper.domain.testModelDossiers())
