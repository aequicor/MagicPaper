...
class PlanningRuntimeIntegrationTest {
    ...
    private suspend fun verifyEngine(engine: CodingEngine, root: File) {
        ...
        dir.resolve("AGENTS.md").writeText("Instruction marker: PLANNING_AGENTS_FIXTURE. Answer briefly.")
        ...
        server.start()
        val codex = CodexAppServerOpenAiSubscription(Json { ignoreUnknownKeys = true }, root.resolve("codex").toPath(), browser = io.aequicor.magicpaper.data.coding.testBrowserSessions, checks = io.aequicor.magicpaper.data.coding.testCommandChecks, journal = io.aequicor.magicpaper.data.storage.InMemoryEventJournal(), questionnaireFactory = io.aequicor.magicpaper.domain.testQuestionnaireFactory())
        ...
        val runtime = MeteredCodingRuntime(DesktopCodingRuntime(isolatedPiRuntime(root), codex, skillSelection = {
            CodingSkillSelection(listOf(SkillInstruction("fixture-skill", "1.0", "fixture-checksum", "Fixture", "Planning guidance",
                emptySet(), "PLANNING_SKILL_FIXTURE: verify your conclusions.")), trustedText = true, freshSession = true)
        }), ledger)
        try {
            ...
            assertContains(requests.first().toString(), "PLANNING_AGENTS_FIXTURE")
            assertContains(requests.first().toString(), "PLANNING_SKILL_FIXTURE")
