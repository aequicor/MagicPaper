            ...
            try {
                ...
                onUi {
                    ...
                    assertTrue(attach.boundsInRoot.width >= 28 && attach.boundsInRoot.height >= 28,
                        "Direct attachment retains its hit target at ${case.scale} text scale")
                    assertTrue(scene.nodes().none { it.config.getOrNull(SemanticsProperties.ContentDescription) == listOf("Показать параметры") },
                        "A provider chat preview without option content must not offer an empty native-engine menu")
