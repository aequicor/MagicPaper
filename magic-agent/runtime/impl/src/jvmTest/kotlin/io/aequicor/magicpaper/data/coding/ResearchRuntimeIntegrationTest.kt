...
class ResearchRuntimeIntegrationTest {
    ...
    private suspend fun verifyEngine(engine: CodingEngine, root: File) {
        ...
        server.start()
        val checks = io.aequicor.magicpaper.data.checks.createCommandChecks(
            io.aequicor.magicpaper.data.storage.InMemoryEventJournal(), io.aequicor.magicpaper.data.storage.InMemoryKeyValueStore(),
            root.toPath().resolve("checks"))
        val codex = CodexAppServerOpenAiSubscription(Json { ignoreUnknownKeys = true }, root.resolve("codex").toPath(), browser = io.aequicor.magicpaper.data.coding.testBrowserSessions, checks = checks, journal = io.aequicor.magicpaper.data.storage.InMemoryEventJournal(), questionnaireFactory = io.aequicor.magicpaper.domain.testQuestionnaireFactory())
        val runtime = DesktopCodingRuntime(isolatedPiRuntime(root, checks), codex, checks = checks)
        try {
        ...
        } finally { runtime.abortAll(); checks.close(); codex.close(); server.stop(0) }
    }
}
