    ...
    @Test fun namedRoutesAndQuestionContextRenderAtDesktopAndPhoneWidths() = runTest {
        ...
        try {
            ...
            val service = OrchestrationService(store, execution, projects, profiles, settings, textPlanComposer(gateway), gateway, backgroundScope,
                workerDispatcher = kotlinx.coroutines.test.StandardTestDispatcher(testScheduler), modelDossiers = io.aequicor.magicpaper.domain.testModelDossiers()).also { executionPorts.chatHooks = it }
            ...
            store.save(plan)
            checkpoints.saveOrchestration(OrchestrationState(parent.id, project.id, activePlanId = plan.id,
