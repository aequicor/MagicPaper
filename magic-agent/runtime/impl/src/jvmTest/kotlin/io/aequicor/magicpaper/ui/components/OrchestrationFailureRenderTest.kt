    ...
    @Test fun failedInputUsesQuestionnaireWhileAWorkerContinues() = runTest {
        ...
        try {
            ...
            val service = OrchestrationService(store, execution, projects, profiles, settings, textPlanComposer(gateway),
                gateway, backgroundScope, workerDispatcher = StandardTestDispatcher(testScheduler), modelDossiers = io.aequicor.magicpaper.domain.testModelDossiers()).also { executionPorts.chatHooks = it }
            ...
            checkpoints.saveOrchestration(OrchestrationState(parent.id, project.id, activePlanId = plan.id, inputs = listOf(input)))
