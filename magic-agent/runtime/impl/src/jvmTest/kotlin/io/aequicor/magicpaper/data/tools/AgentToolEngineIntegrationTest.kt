...
import io.aequicor.magicpaper.data.coding.isolatedPiRuntime
...
class AgentToolEngineIntegrationTest {
    ...
    private fun tools(): ToolSession {
        val host = testToolSessions(MemoryToolReceiptStore(),
            receiver = { _, _, name, _ -> check(name == "context.get") { "Forbidden receiver reached: $name" }; JsonPrimitive("CONTEXT_OK") })
    ...
    @Test fun piPlanningInvokesToolsReceivesResultsAndRejectsForbiddenRole() = runBlocking {
        ...
        val runtime = isolatedPiRuntime(root.toFile())
    ...
    @Test fun codexPlanningInvokesMcpToolsReceivesResultsAndRejectsForbiddenRole() = runBlocking {
        ...
        Fixture(true).use { fixture ->
            val runtime = CodexAppServerOpenAiSubscription(Json { ignoreUnknownKeys = true }, root.resolve("home"), browser = io.aequicor.magicpaper.data.coding.testBrowserSessions, checks = io.aequicor.magicpaper.data.coding.testCommandChecks, journal = io.aequicor.magicpaper.data.storage.InMemoryEventJournal(), questionnaireFactory = io.aequicor.magicpaper.domain.testQuestionnaireFactory())
            try {
                val tools = tools()
                ...
                val events = withTimeout(60_000) { runtime.runFixtureCoding(CodingProject("p", "Project", root.toString(), 0),
    ...
    private suspend fun quietPlanning(engine: CodingEngine) {
        ...
        val pi = if (engine == CodingEngine.PI) isolatedPiRuntime(root.toFile()) else null
        val codex = if (engine == CodingEngine.CODEX) CodexAppServerOpenAiSubscription(Json { ignoreUnknownKeys = true }, root.resolve("home"), browser = io.aequicor.magicpaper.data.coding.testBrowserSessions, checks = io.aequicor.magicpaper.data.coding.testCommandChecks, journal = io.aequicor.magicpaper.data.storage.InMemoryEventJournal(), questionnaireFactory = io.aequicor.magicpaper.domain.testQuestionnaireFactory()) else null
            ...
            try {
                ...
                val driver = pi ?: object : CodingRuntime by NoopCodingRuntime {
                    ...
                    override fun runPlanning(project: CodingProject, session: CodingSession, prompt: String, profile: LlmProfile) =
                        kotlinx.coroutines.flow.channelFlow<CodingEvent> {
                            val requestId = java.util.UUID.randomUUID().toString()
                            val registry = io.aequicor.magicpaper.domain.testQuestionnaireFactory().create("fixture", null)
                            io.aequicor.magicpaper.data.coding.AgentRunResources.prepare(project, session, true, null, registry,
                                io.aequicor.magicpaper.data.coding.testBrowserSessions, io.aequicor.magicpaper.data.coding.testCommandChecks,
                                kotlinx.coroutines.currentCoroutineContext()[io.aequicor.magicpaper.domain.tools.ToolSession],
                                requestId = requestId).use { resources ->
                                val agent = (codex!!.binding.runtime as io.aequicor.magicpaper.data.coding.GenericNativeRuntime).agent
                                agent.run(io.aequicor.magicpaper.backend.NativeAgentRequest(requestId, session, project.path,
                                    prompt, "Use application tools and submit the plan", null, profile,
                                    kotlinx.serialization.json.JsonObject(emptyMap()), emptyList(), CodingInteractionMode.PLANNING,
                                    resources.nativeTools(), io.aequicor.magicpaper.backend.NativeModelConnection("quiet_model_test", fixture.url, "")))
                                    .collect { send(it) }
                            }
                        }
