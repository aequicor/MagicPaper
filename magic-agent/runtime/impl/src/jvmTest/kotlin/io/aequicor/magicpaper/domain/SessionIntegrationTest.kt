...
class SessionIntegrationTest {
    ...
    private class Fixture {
        ...
        val port = testGitPlanningWorkspace(Files.createTempDirectory("session-integration-work").toFile(), checks = testGitChecks())
        ...
        suspend fun input(id: String, file: String = "$id.txt", content: String = id): SessionIntegrationInput {
            ...
            val heldWorkspace1 = assertNotNull(port.acquire(owner, "lease-request-1"))
            val workspace = try { port.prepare(project, id, WorkspaceOperation(heldWorkspace1, "prepare-$id")) }
                finally { port.release(heldWorkspace1) }
        ...
        suspend fun run(inputs: List<SessionIntegrationInput>, initial: SessionIntegration? = null, sourceLease: WorkspaceLease? = null) =
            helper.integrate(project, initial ?: request(inputs), inputs, sourceLease, checkpoint)
    }
    ...
    @Test fun combinesAcceptedCommitsChecksTheCombinedFilesAndPreservesUserCheckout() = runTest { withContext(Dispatchers.Default) {
        ...
        f.checks.runBody = { path, _, command ->
            ...
            assertNull(f.port.acquire(f.project.copy(id = "competing-check", path = path), "lease-request-2"))
    ...
    @Test fun cancellationKeepsWriterUntilCheckReconciliationCompletes() = runTest { withContext(Dispatchers.Default) {
        ...
        try {
            ...
            assertNull(f.port.acquire(f.project.copy(id = "early", path = f.saved.workspace!!.integrationPath), "lease-request-3"))
            ...
            val heldWorkspace4 = assertNotNull(f.port.acquire(owner, "lease-request-4")); f.port.release(heldWorkspace4)
    ...
    @Test fun existingParentWriterLeaseCanBeBorrowedWithoutReleasingIt() = runTest { withContext(Dispatchers.Default) {
        ...
        val heldWorkspace5 = assertNotNull(f.port.acquire(parent, "lease-request-5"))
        try {
            assertEquals(SessionIntegrationPhase.VERIFIED, f.run(inputs, sourceLease = heldWorkspace5).phase)
            assertNull(f.port.acquire(f.project.copy(id = "other-root"), "lease-request-6"))
        } finally { f.port.release(heldWorkspace5) }
    } }
