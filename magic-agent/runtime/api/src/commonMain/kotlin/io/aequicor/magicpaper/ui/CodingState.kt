...
data class CodingSessionUi(
    ...
    val runPhase: io.aequicor.magicpaper.domain.CodingMachine.Phase? = null,
    val plan: Plan? = null,
    ...
    val canChangeHistory: Boolean get() = runPhase == null && !running && !draft.active && interactions.isEmpty() && !awaitingUser &&
    ...
    val canResume: Boolean
        get() {
            ...
            if (session.queuedPrompts.isNotEmpty()) return true
            return session.pendingRun?.let { runPhase == io.aequicor.magicpaper.domain.CodingMachine.Phase.UNKNOWN || session.taskWorktree != null || !it.hasSuccessfulResponse(messages) }
...
data class PlanningRunUi(val active: Boolean = false, val canContinue: Boolean = false, val requiresRecovery: Boolean = false)
