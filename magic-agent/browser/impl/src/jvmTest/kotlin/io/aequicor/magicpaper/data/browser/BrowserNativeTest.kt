...
class BrowserNativeTest {
    ...
    @Test fun realBrowserNavigatesInteractsValidatesAndCloses() = runBlocking {
        ...
        val browser = testBrowser(launch = { it.chromium().launch().also { instance -> native = instance } },
            createPlaywright = { Playwright.create(Playwright.CreateOptions().setEnv(mapOf("PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD" to "1"))) })
        val session = testBrowserTools(MemoryToolReceiptStore())
        var call = 0
        ...
        try {
            attachBrowser(session, browser).use {
                ...
                val shot = execute("screenshot", "tabId" to tab, "format" to "png")
