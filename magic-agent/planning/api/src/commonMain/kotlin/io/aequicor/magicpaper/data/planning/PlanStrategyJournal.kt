...
data class JournalPlanSnapshot(val plan: Plan, val journal: JournalSnapshot)
data class StrategyIntent(val record: JournalRecord, val runId: String, val stageId: String)
data class PlanStrategyFacts(
...
fun planStrategyFacts(records: List<JournalRecord>): PlanStrategyFacts {
    unsettledPlanIntents(records)
    var state = records.firstOrNull()?.let { PlanningMachine.initial(it.stream) }
    ...
    records.forEach { stored ->
        state = projectPlanningRecord(checkNotNull(state), stored)
...
fun PlanStrategyFacts.strategyFor(intent: JournalRecord): Long? {
