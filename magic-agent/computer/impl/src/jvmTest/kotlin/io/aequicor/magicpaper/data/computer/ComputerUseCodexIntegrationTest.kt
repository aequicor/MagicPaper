            ...
            computer.testBridge("a")!!.use { bridge ->
                ...
                try {
                    ...
                    val result = call("thread/start", buildJsonObject {
                        ...
                        put("config", io.aequicor.magicpaper.domain.computerCodexConfig(buildJsonObject {}, bridge.descriptor))
                    })
                    ...
                    bridge.close()
                    computer.enableForTest("a", ComputerAccess.SCREEN)
                    computer.testBridge("a", "next-run")!!.use { renewed ->
                        call("thread/unsubscribe", buildJsonObject { put("threadId", threadId) })
                        call("thread/resume", buildJsonObject {
                            put("threadId", threadId)
                            put("config", io.aequicor.magicpaper.domain.computerCodexConfig(buildJsonObject {}, renewed.descriptor))
                        })
