...
internal class FakeComputerDesktop : ComputerDesktop {
    ...
    override fun checkPermissions(access: ComputerAccess, request: Boolean) { if (denied) throw ComputerPermissionException(ComputerPermissionFailure.SCREEN_RECORDING) }
...
class DesktopComputerUseTest {
    @Test fun noAccessUntilUserEnablesIt(): Unit = runBlocking {
        ...
        assertTrue(computer.execute("a", io.aequicor.magicpaper.domain.ComputerLease("a", "0"), request("screenshot")).failed())
    ...
    @Test fun permissionsAndSessionOwnershipAreNotInherited(): Unit = runBlocking {
        ...
        computer.enableForTest("a", ComputerAccess.SCREEN)
        val epoch = computer.grant("a")!!
        computer.enableForTest("worker", ComputerAccess.CONTROL)
    ...
    @Test fun osDenialIsVisibleAndDoesNotGrantAccess(): Unit = runBlocking {
        ...
        assertContains(computer.state.value.detail, "запись экрана")
        assertNull(computer.grant("a"))
    }
