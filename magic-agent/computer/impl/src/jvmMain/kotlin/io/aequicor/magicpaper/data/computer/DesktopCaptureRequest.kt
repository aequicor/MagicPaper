...
internal data class DesktopImageSize(val width: Int, val height: Int) {
    companion object {
        fun parse(value: JsonElement): DesktopImageSize {
            ...
            computerRequire(args.keys == setOf("width", "height")) { "image_size: требуются width и height" }
            val width = args.requiredInt("width")
            val height = args.requiredInt("height")
            computerRequire(width > 0 && height > 0) { "image_size: ширина и высота должны быть положительными" }
...
internal data class DesktopRegion(val x: Int, val y: Int, val width: Int, val height: Int) {
    fun validate(display: ComputerDisplay) {
        computerRequire(x >= 0 && y >= 0 && width > 0 && height > 0 &&
    ...
    fun crop(args: JsonObject, imageWidth: Int, imageHeight: Int): DesktopRegion {
        computerRequire(args.keys == setOf("x", "y", "width", "height")) { "region: требуются x, y, width, height" }
        val left = args.requiredInt("x"); val top = args.requiredInt("y")
        val w = args.requiredInt("width"); val h = args.requiredInt("height")
        computerRequire(left >= 0 && top >= 0 && w > 0 && h > 0 && left.toLong() + w <= imageWidth && top.toLong() + h <= imageHeight) {
