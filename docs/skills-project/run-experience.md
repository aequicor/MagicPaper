...
> Исторический отчёт указанного этапа. В целевой миграции 20 сентября неиспользуемый
> `PlanRunner` и его отдельные тесты удалены; исполнение плана и выбор модели проверяются
> через `PlanningExecutionService`. Команды и пути ниже относятся к записанной версии.
