...
| `:backend-agents:api` / `factory` | Нейтральная к движку конфигурация (дескриптор, способности, окружение), контракты адаптера, жизненного цикла и портов хоста; типы Pi/Codex сюда не попадают. Единственная фабрика адаптеров; потребитель — `:magic-agent:runtime:impl` |
| `:backend-agents:pi` / `codex` | Собственные типы движка (`Pi*`, `Codex*`), протокол, конфигурация, чтение событий и запуск native-процессов; реализации видит только factory |
...
Как объявить машину, что харнесс не видит и каталог всех машин — в
[STATE-SPACES.md](STATE-SPACES.md); верификатор архитектуры не пропускает `reduce` без
объявления, теста с `verifyStateSpace` и строки в этом каталоге.
