...
`designSystem` is a Compose Multiplatform module with JVM, Android, JS and Wasm targets. Application assembly in `:app` and feature implementations consume it; it has no dependency on these owners or platform hosts. Its public namespace is `io.aequicor.magicpaper.designsystem` and contains no Material types in public parameters, returns or composition locals.
...
The parchment palette, `PaperBackground` with its animation policy and platform renderers, and Cormorant Garamond, Literata and JetBrains Mono font resources are owned by this module. Material 3 is private implementation only. The application assembles feature renderer providers around `PaperTheme`; these providers do not belong to the design system.
...
Keyboard and accessibility behavior is covered by `PaperSemanticsTest.contextIndicatorExposesProgressAndSupportsKeyboard`; `app`'s `UsageUiTest` covers 390/1000 px layouts, menu filters, unknown/estimated context and expandable context reports. Native Windows interaction: **NOT_RUN** on the macOS verification host; follow up with the same controls on Windows using Tab, Enter, Space and Escape.
