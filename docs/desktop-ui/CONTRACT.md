...
Текущая сборка приложения принадлежит `:app`; feature-модули содержат экраны и их контракты. Общие визуальные компоненты, включая `PaperBackground`, принадлежат `:designSystem`. Упоминания прежнего модуля и старых путей ниже описывают исходный этап; действующие владельцы и команды находятся в [MODULES.md](../MODULES.md) и [DESIGN-SYSTEM.md](DESIGN-SYSTEM.md).
