...
Текущий маршрут: **Настройки → Управление компьютером** (при первоначальной
приёмке ниже: Движки → Компьютер и приложения). Новая страница и онбординг:
[отдельная приёмка](COMPUTER-PERMISSIONS-ACCEPTANCE.md).
