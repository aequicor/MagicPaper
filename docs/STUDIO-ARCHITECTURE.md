...
2. `EventJournal` в `:core:storage:api`, `DurableEventJournal` в `impl` (имя по соседям —
   ...
   **Хранилище готово; писателя ещё нет.** Область `StorageArea.EVENTS` очищается тем же
   ...
   Следующий шаг — писатель: `PlanningExecutionService.journal(...)` пишет туда же, куда
   сегодня пишет в `Plan.journal`. Затем читатель: какие намерения остались без исхода.
   **Запись намерения эффекта до исполнения** — `effect-requested` и `effect-settled(outcome)`
   двумя записями — появляется вместе с писателем.
