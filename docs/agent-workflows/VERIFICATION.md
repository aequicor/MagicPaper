...
| Поведение одного модуля | `./gradlew :magic-chat:impl:jvmTest --tests '*ИмяТеста'` — подставить владельца и реальный класс |
| Границы или Gradle dependencies | `python3 docs/verify-module-architecture.py --self-test` |
| Кодинг-сессии, планирование, оркестрация, движки | `./gradlew :magic-agent:runtime:impl:jvmTest --tests '*ИмяТеста'`; протокол движков — `:backend-agents:pi:nodeProtocolTest` |
| Машины и журналы нативного запуска | `./gradlew :backend-agents:api:jvmTest :backend-agents:lifecycle:impl:jvmTest :backend-agents:factory:jvmTest`; адаптер журнала хоста — runtime `*NativeLifecycleJournal*Test` |
| Организм сессий, receipts и карантин | `./gradlew :magic-agent:organism:api:jvmTest :magic-agent:organism:impl:jvmTest`; интеграция — runtime `*Session*Test`, `*CodingRuntimeGraphTest` |
| Рабочие копии, Git и восстановление доставки | `./gradlew :magic-agent:workspace:api:jvmTest :magic-agent:workspace:impl:jvmTest`; runtime `*GitTaskWorkspaceTest`, `*GitPlanningWorkspaceTest`, `*PlanningWorkspaceOwnershipTest`, `*SessionCodingWorkspaceTest`, `*SessionIntegrationTest` и app `*CodingWorktreeTest`. Ошибки записи/потеря ACK/restore без эффектов и точная identity lease проверяются отдельно от реальных временных Git-репозиториев |
| Проверки команд и процессные доказательства | `./gradlew :magic-agent:checks:api:jvmTest :magic-agent:checks:impl:jvmTest`; runtime adapters `*TaskWorktreeIntegrationChecksTest`, `*ResearchCheckBridgeTest`. Настоящие sandbox/containment проверки ОС — отдельный opt-in `-Pmagicpaper.research.native=true`, без включения live model интеграций. На Windows `ResearchSandboxNativeTest` даёт 6 записанных отказов из-за неработающего `WRITE_RESTRICTED`-токена; база и требуемое решение — в разделе «Долги» [STUDIO-ARCHITECTURE.md](../STUDIO-ARCHITECTURE.md). `WindowsCheckAuthorityNativeTest` и `ReadOnlyGitWithoutSandboxProbeNativeTest` — реальные проверки ACL и read-only Git: первая входит в обычный прогон на Windows, вторая требует того же opt-in флага |
| Общая презентация сессий (транскрипт, вложения, закрепления) | `./gradlew :magic-chat:impl:jvmTest :magic-agent:runtime:impl:jvmTest` — у `:magic-common:transcript` пока нет своих тестов, его код покрывают потребители |
...
| Браузер, ресурсный lifecycle и журнал | `./gradlew :magic-agent:browser:api:jvmTest :magic-agent:browser:impl:jvmTest`; MCP consumer — runtime `*BrowserToolBridgeTest`, `*AgentToolBridgeTest`; Chromium opt-in отдельно в [AGENT-BROWSER.md](../AGENT-BROWSER.md) |
| Native wire resources | `./gradlew :backend-agents:pi:nodeProtocolTest` |
| Computer/application-use | `:magic-agent:computer:api:jvmTest :magic-agent:computer:impl:jvmTest --tests '*ApplicationUseTest' --tests '*NativeApplicationDesktopTest' --tests '*ComputerUse*Test'`; политики/очереди — `:app:jvmTest --tests '*AutomationPolicyTest' --tests '*SessionInputQueueTest' --tests '*ChatInputQueueTest'`. Реальное окно отдельно: [opt-in macOS-сценарий](../COMPUTER-USE.md#проверка); на интерактивном Windows desktop — `tools/check-computer-use-windows.ps1` ([границы проверки](../desktop-ui/COMPUTER-USE-FEEDBACK.md#проверка-windows)) |
| Онбординг разрешений компьютера | Runtime `*ComputerSettings*Test`, `*ComputerPermissionControllerTest`; computer impl `*DesktopComputerPermissionsTest`; DS `*PaperFileTransferTest`. Реальный drop/Finder/TCC и Windows проверяются отдельно: [сценарии](../desktop-ui/COMPUTER-PERMISSIONS-ACCEPTANCE.md) |
...
| Макеты из чата | `:magic-agent:runtime:impl:jvmTest --tests '*Layout*Test'`, `:app:jvmTest --tests '*ChatLayoutWorkflowTest' --tests '*ChatServiceLifecycleTest'`; модель в тестах заменена фикстурой |
| Чат → native Paper Editor | Сначала `./gradlew -PpaperEditor=true :desktopApp:createDistributable`, затем `./gradlew -PpaperEditor=true -Pmagicpaper.paperEditor.it=true :magic-agent:runtime:impl:jvmTest --tests '*DesktopLayoutEditorIntegrationTest'`; отдельный временный проект, настоящий редактор, без LLM-запросов; PNG `magic-agent/runtime/impl/build/reports/layout-chat/generated.png`. `-Pmagicpaper.paperEditor.testExecutable=…` проверяет конкретный вложенный executable |
...
DMG/Windows-инсталлятор Inno Setup/DEB проверяются на соответствующей ОС:
холодная/повторная ссылка, одновременный запуск, свёрнутое окно, upgrade и uninstall. Для отсутствующей ОС
