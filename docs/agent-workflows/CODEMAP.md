...
| Computer/application-use, доступ и native UI | [session automation](../../feature/session/impl/src/jvmMain/kotlin/io/aequicor/magicpaper/data/computer) | API `domain/ComputerUse.kt`; `DesktopComputerUse`, `ApplicationUse`, `NativeApplicationDesktop`; [протокол и проверки](../COMPUTER-USE.md) | `:feature:session:impl:jvmTest --tests '*ApplicationUseTest' --tests '*NativeApplicationDesktopTest' --tests '*ComputerUse*Test'`; policy/restore: `:app:jvmTest --tests '*AutomationPolicyTest' --tests '*SessionInputQueueTest' --tests '*ChatInputQueueTest'` |
