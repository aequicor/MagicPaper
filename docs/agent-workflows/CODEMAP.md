...
| Вопросы и источники исследования | [session](../../feature/session), [контракт](../desktop-ui/RESEARCH-CHAT.md) | `api/.../ui/ChatState.kt`, `ChatService.kt`; `impl/.../ui/screens/ResearchWorkspace.kt`, `domain/ChatResearch.kt`; модели `ChatSession`, `ResearchResource` | `:app:jvmTest --tests '*ChatResearchTest'`; owner `jvmTest --tests '*ResearchWorkspaceRenderTest' --tests '*ChatResearchContextTest' --tests '*ResearchPageReaderTest'` |
...
| Computer/application-use, доступ и native UI | [session automation](../../feature/session/impl/src/jvmMain/kotlin/io/aequicor/magicpaper/data/computer) | API `domain/ComputerUse.kt`; `DesktopComputerUse`, `DesktopCaptureRequest`, `ApplicationUse`, `NativeApplicationDesktop`; [протокол и проверки](../COMPUTER-USE.md) | `:feature:session:impl:jvmTest --tests '*ApplicationUseTest' --tests '*NativeApplicationDesktopTest' --tests '*ComputerUse*Test' --tests '*DesktopScreenshotDetailTest'`; policy/restore: `:app:jvmTest --tests '*AutomationPolicyTest' --tests '*SessionInputQueueTest' --tests '*ChatInputQueueTest'` |
