...
| Исполнение инструментов и эффекты | [session](../../feature/session) | `impl` → `domain/tools/` (`AgentTools`, `ToolHost`, `ToolReceiptIntegrity`, `ToolEnabledCodingRuntime`); эффекты: `domain/OrchestrationService.kt`; native-мост: JVM `data/tools/AgentToolBridge.kt` | `:feature:session:impl:jvmTest --tests '*SessionAuthorityToolTest'` |
