...
class DefaultMediaGenerationService(
    ...
    private val profiles: suspend () -> List<LlmProfile>,
    ...
    private val onTerminal: suspend (MediaGenerationOwner, String, GeneratedMedia) -> Unit = { _, _, _ -> },
    private val authorizeSubmission: suspend (MediaGenerationOwner, MediaKind) -> Boolean = { _, _ -> true },
) : MediaGenerationService {
    private val json = Json { ignoreUnknownKeys = true; encodeDefaults = true }
    private val operationJournal = MediaOperationJournal(journal, mediaStore)
    ...
    private suspend fun selected(kind: MediaKind): Pair<MediaModelSelection, LlmProfile>? {
        ...
        val selection = settings().media.selection(kind) ?: return null
        val profile = profiles().firstOrNull { it.id == selection.profileId && it.enabled }
            ?: return null
    ...
    override suspend fun check(kind: MediaKind, selection: MediaModelSelection, profile: LlmProfile): MediaConnectionStatus {
        ...
        val id = Id.uuid()
        val operation = MediaOperationData(id, MediaGenerationOwner("connection-check", id, id), request.copy(prompt = ""), selection, fingerprint,
            ...
            requestFingerprint = mediaStore.fingerprint(json.encodeToString(request)), createdAt = Id.now())
        ...
        val status = try {
            val result = runOperation(operation, profile, allowSubmit = true, submissionRequest = request) { preview ->
    ...
    private suspend fun publishCheckResult(operation: MediaOperationData, status: MediaConnectionStatus) = connectionLock.withLock {
    ...
    override suspend fun generate(owner: MediaGenerationOwner, operationId: String, request: MediaGenerationRequest,
        ...
        load(operationId)?.let { old ->
            require(old.owner == owner && old.requestFingerprint == mediaStore.fingerprint(json.encodeToString(mediaRequestDefaults(old.selection, request)))) { "Идентификатор генерации уже использован" }
        ...
        val operation = MediaOperationData(operationId, owner, effective.copy(prompt = ""), current.first, fp,
            ...
            requestFingerprint = mediaStore.fingerprint(json.encodeToString(effective)), createdAt = Id.now())
        runOperation(operation, current.second, allowSubmit = true, submissionRequest = effective, authorize = authorize, onUpdate = onUpdate)
    }
    ...
    override suspend fun recoverMedia(mediaId: String): GeneratedMedia? {
        val operation = operationJournal.all().mapNotNull { it.operation }.firstOrNull { it.media.id == mediaId } ?: return null
        return recover(operation.id)
    }

    private suspend fun resume(saved: MediaOperationData, onUpdate: suspend (GeneratedMedia) -> Unit): GeneratedMedia {
        pendingRuns.value[saved.id]?.takeIf { it.isActive }?.let { return it.await().getOrThrow().also { onUpdate(it) } }
        val restored = change(saved.id, MediaGenerationMachine.Fact.Restored).state
        if (restored.persistenceUnknown) throw MediaJournalUnknown()
        val operation = checkNotNull(restored.operation)
        ...
        val profile = profiles().firstOrNull { it.id == operation.selection.profileId }
            ?: throw MediaUnknownFailure()
        return runOperation(operation, profile, allowSubmit = false, onUpdate = onUpdate)
    }

    private suspend fun runOperation(initial: MediaOperationData, profile: LlmProfile, allowSubmit: Boolean,
        submissionRequest: MediaGenerationRequest? = null,
        authorize: suspend () -> Unit = {},
        ...
        val pending = lifecycleLock.withLock {
            ...
            pendingRuns.value[initial.id] ?: applicationScope.async(start = CoroutineStart.LAZY) {
                ...
                try {
                    val result = withTimeout(waitTimeoutMillis) { runAttempt(initial, profile, allowSubmit, submissionRequest, authorize, notify) }
    ...
    private suspend fun runAttempt(initial: MediaOperationData, profile: LlmProfile, allowSubmit: Boolean,
        submissionRequest: MediaGenerationRequest?, authorize: suspend () -> Unit,
        onUpdate: suspend (GeneratedMedia) -> Unit): GeneratedMedia {
        val usageObservation = usage.captureObservation()
        var operation = initial
        var ephemeralOutput: MediaRemoteOutput? = null
        var ownsAttempt = false
        suspend fun input(value: MediaGenerationMachine.Input): List<MediaGenerationMachine.Effect> {
            val transition = change(operation.id, value)
            operation = checkNotNull(transition.state.operation)
            return transition.effects
        }
        suspend fun notify() { if (live(operation.owner)) onUpdate(operation.media) }
        try {
            if (allowSubmit) {
                input(MediaGenerationMachine.Intent.Create(operation))
                val effect = input(MediaGenerationMachine.Intent.Submit).single() as MediaGenerationMachine.Effect.Submit
                check(effect.operationId == operation.id)
                ownsAttempt = true
                notify()
                ...
                val payload = checkNotNull(submissionRequest) { "Повторная отправка сохранённого запроса запрещена" }
                check(mediaStore.fingerprint(json.encodeToString(payload)) == operation.requestFingerprint) { "Запрос генерации изменился" }
                when (val submission = gateway.submit(profile, operation.selection, payload)) {
                    is MediaSubmission.Accepted -> input(MediaGenerationMachine.Fact.Accepted(submission.jobId))
                    is MediaSubmission.Completed -> {
                        ephemeralOutput = submission.output
                        input(MediaGenerationMachine.Fact.Output(submission.output.copy(dataBase64 = "")))
                    }
                }
            }
            var failedPolls = 0
            while (operation.output == null) {
                val effect = input(MediaGenerationMachine.Intent.Poll).single() as MediaGenerationMachine.Effect.Poll
                ownsAttempt = true
                delay(pollDelayMillis)
                val result = try { gateway.poll(profile, operation.selection, effect.jobId, operation.request.kind) }
                catch (failure: MediaGatewayException) {
                    if (failure.kind == MediaFailureKind.TRANSIENT && ++failedPolls < 4) {
                        input(MediaGenerationMachine.Fact.Failure(safeMessage(failure), confirmed = false))
                ...
                failedPolls = 0
                when (result) {
                    MediaPollResult.Pending -> input(MediaGenerationMachine.Fact.Pending)
                    is MediaPollResult.Completed -> {
                        ephemeralOutput = result.output
                        input(MediaGenerationMachine.Fact.Output(result.output.copy(dataBase64 = "")))
                    }
            ...
            val download = input(MediaGenerationMachine.Intent.Download(ephemeralOutput?.dataBase64?.isNotBlank() == true))
                .single() as MediaGenerationMachine.Effect.Download
            ownsAttempt = true
            notify()
            val downloaded = gateway.download(ephemeralOutput ?: download.output)
            ...
            input(MediaGenerationMachine.Fact.Asset(asset))
            recordUsage(usageObservation, operation, profile, completed = true)
            ...
            if (ownsAttempt && operation.media.phase != MediaPhase.READY) withContext(NonCancellable) {
                cleanup(operation, "outcome.save") { input(MediaGenerationMachine.Fact.Failure(message, confirmed)) }
                cleanup(operation, "outcome.state") {
                    // A failed append can leave only an unknown in-memory projection; never invent success.
                    operationJournal.read(operation.id)?.operation?.let { operation = it }
                    if (live(operation.owner)) mutableOperations.update { it + (operation.media.id to operation.media) }
                }
                cleanup(operation, "outcome.publish") { notify() }
                cleanup(operation, "usage.record") { recordUsage(usageObservation, operation, profile, completed = false) }
            ...
            if (failure !is CancellationException) AppLog.error("media", "generation.failed", fields = mapOf("operationId" to operation.id,
    ...
    private suspend fun cleanup(operation: MediaOperationData, event: String, action: suspend () -> Unit) {
        try { action() }
        catch (failure: Exception) {
            if (failure is CancellationException) throw failure
    ...
    private suspend fun recordUsage(observation: UsageObservation, operation: MediaOperationData, profile: LlmProfile, completed: Boolean) = usage.record(observation,
    ...
    private suspend fun invalidate(operation: MediaOperationData, message: String) {
    ...
    override suspend fun recoverPending() {
        ...
        for (state in operationJournal.all()) {
            val operation = state.operation ?: continue
            ...
            if (operation.jobId == null && operation.output == null) {
                change(operation.id, MediaGenerationMachine.Fact.Restored)
                continue
            }
    ...
    override suspend fun deleteSession(sessionId: String) {
        ...
        for (state in operationJournal.all()) {
            ...
            if (operation.owner.sessionId == sessionId) {
                ...
                operationLock(operation.id).withLock { load(operation.id)?.let { change(it.id, MediaGenerationMachine.Intent.Delete) } }
    ...
    private suspend fun load(id: String): MediaOperationData? = operationJournal.read(id)?.operation
    private suspend fun change(id: String, input: MediaGenerationMachine.Input): MediaGenerationMachine.Transition {
        val transition = try { operationJournal.dispatch(id, input) }
        catch (failure: Exception) {
            operationJournal.unconfirmed(id)?.operation?.let { operation ->
                mutableOperations.update { it + (operation.media.id to operation.media) }
            }
            throw failure
        }
        transition.state.operation?.let { operation ->
            if (operation.deleted) mutableOperations.update { it - operation.media.id }
            else if (live(operation.owner)) mutableOperations.update { it + (operation.media.id to operation.media) }
        }
        return transition
    }
...
fun mediaRequestDefaults(selection: MediaModelSelection, request: MediaGenerationRequest): MediaGenerationRequest {
