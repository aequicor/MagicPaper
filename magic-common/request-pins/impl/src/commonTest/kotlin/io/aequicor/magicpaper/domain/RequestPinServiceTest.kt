...
class RequestPinServiceTest {
    ...
    private class Fixture(scope: CoroutineScope, val answer: suspend (String) -> String) {
        ...
        val service = RequestPinService(repo, gateway, scope, journal,
            storageDispatcher = scope.coroutineContext[ContinuationInterceptor] as CoroutineDispatcher)
    }
    ...
    @Test fun groupsRequestsAndClarificationsInOrderAndCachesAcrossRestart() = runTest {
        ...
        val resumed = RequestPinService(f.repo, f.gateway, this, f.journal,
            storageDispatcher = coroutineContext[ContinuationInterceptor] as CoroutineDispatcher)
    ...
    @Test fun planningQuestionsNeverFlashAsPinsAndClarificationsStayWithTheirRequest() = runTest {
        ...
        val restored = RequestPinService(f.repo, f.gateway, this, f.journal,
            storageDispatcher = coroutineContext[ContinuationInterceptor] as CoroutineDispatcher)
    ...
    @Test fun loadingPlanningDecisionsRemovesOldQuestionPinsEvenWithoutAModel() = runTest {
        ...
        val restored = RequestPinService(f.repo, null, this, f.journal,
            storageDispatcher = coroutineContext[ContinuationInterceptor] as CoroutineDispatcher)
