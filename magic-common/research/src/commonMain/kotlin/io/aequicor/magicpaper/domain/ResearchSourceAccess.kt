...
data class ResearchSourceCheck(val resource: ResearchResource, val problem: String? = null)
...
class ResearchSourceAccess(private val readPage: (suspend (String) -> String)? = null) {
    ...
    suspend fun check(resources: List<ResearchResource>): List<ResearchSourceCheck> = coroutineScope {
        resources.distinctBy { it.key }.map { resource -> async { check(resource) } }.awaitAll()
    }

    suspend fun check(resource: ResearchResource): ResearchSourceCheck {
...
fun List<ResearchSourceCheck>.readableSources(): List<ResearchResource> = filter { it.problem == null }.map { it.resource }

fun List<ResearchSourceCheck>.unavailableSourceContext(allowSearch: Boolean = true): String = filter { it.problem != null }.takeIf { it.isNotEmpty() }
