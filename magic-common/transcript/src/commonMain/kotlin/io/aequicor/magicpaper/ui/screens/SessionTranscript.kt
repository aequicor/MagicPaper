...
/** Transcript and composer are shared by an ordinary chat and a project session. */
@Composable
fun MessagesList(session: ChatSession?, busy: Boolean, modifier: Modifier = Modifier,
                ...
                items(fragments, key = { it.key }, contentType = { it.contentType }) { fragment ->
                    ...
                    PaperChatScrollItem(scroll, fragment.key) {
                        MessageBubble(message,
                            ...
                            onPause = onPause, onResume = onResume, resumeAction = resumeAction, actions = { content ->
                ...
                Column(modifier = Modifier.fillMaxWidth()
                    ...
                    if (fragment.first && !isUser && (message.researchActivity.isNotEmpty() || working || paused)) {
                        ResearchActivity(message.researchActivity, working, paused, failed, onPause, onResume,
                            answering = message.text.isNotBlank(), resumeAction = resumeAction)
                    }
...
fun Composer(
    ...
    ResearchComposer(
        state = draft, enabled = enabled, busy = busy, paused = paused, resumeAction = resumeAction,
