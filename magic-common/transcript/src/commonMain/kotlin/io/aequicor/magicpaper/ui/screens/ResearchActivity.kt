...
fun ResearchActivity(steps: List<CodingStep>, busy: Boolean, paused: Boolean = false,
    ...
    val activity = remember(steps, busy, paused, answering, failed, resumeAction) { researchActivityEntries(steps, busy, paused, answering, failed, resumeAction.label) }
    val state = when {
    ...
    PaperResearchActivityPanel(label, state, expanded, { expanded = it },
        actionLabel = if (busy) "Остановить" else resumeAction.label, onAction = action) {
...
fun researchActivityEntries(steps: List<CodingStep>, busy: Boolean, paused: Boolean,
    answering: Boolean = false, failed: Boolean = false, recoveryLabel: String = "Продолжить"): List<ResearchActivityEntry> {
        ...
        .distinctBy { it.id.ifBlank { "${it.kind}:${it.title}" } }.map { step ->
            ...
            val detail = when {
                step.kind == CodingStepKind.ERROR && text == RESEARCH_MODEL_FAILURE ->
                    "Проверьте подключение к модели и нажмите «$recoveryLabel»."
