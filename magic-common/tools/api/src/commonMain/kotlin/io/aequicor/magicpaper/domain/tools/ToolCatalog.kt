...
object ToolCatalog {
    ...
    private val everyMode = CodingInteractionMode.entries.toSet()
    private val code = setOf(CodingInteractionMode.CODE)
    private val research = setOf(CodingInteractionMode.RESEARCH)
    private val codeOrResearch = setOf(CodingInteractionMode.CODE, CodingInteractionMode.RESEARCH)
    ...
    val definitions: List<ToolDefinition> = listOf(
        ...
        ToolDefinition("research_check", "Защищённая проверка", empty, ToolCategory.EXEC, workers, native = true, modes = research),
        ...
        app<MediaToolArgs>("image.generate", "Создать изображение по описанию. Дождись готового результата: он содержит сохранённый файл, который можно использовать в проекте. Не вызывай повторно для проверки готовности.", workers, mutating = true, modes = codeOrResearch),
        app<MediaToolArgs>("video.generate", "Создать видео по описанию. Генерация занимает несколько минут; вызов возвращает готовый сохранённый файл. Не вызывай повторно для проверки готовности.", workers, mutating = true, modes = codeOrResearch),
        ToolDefinition("questionnaire", QuestionnaireContract.description, QuestionnaireContract.schema, ToolCategory.ACTION),
        app<TaskHandoff>("task.handoff", "Обязательная передача завершённой worktree-задачи: outcome=RESULT; при реальной блокировке — BLOCKED. checks — команды проверок как массивы аргументов без shell. После успешного RESULT закончи ответ: приложение проверит результат и автоматически выполнит слияние. Подтверждение слияния через опросник не требуется", setOf(ToolRole.CHAT), true, modes = code, worktree = true),
        ...
        app<StageReply>("stage.handoff", "Передать результат RESULT, вопрос QUESTION, блокировку BLOCKED или запрос ожидания WAIT. После успеха завершить ответ; статус этапа определяет приложение", setOf(ToolRole.WORKER), true, true, modes = code),
