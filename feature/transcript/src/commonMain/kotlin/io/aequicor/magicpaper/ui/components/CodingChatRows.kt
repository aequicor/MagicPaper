...
data class CodingHistoryItem(val row: CodingChatRow, val stepIndex: Int? = null) {
...
fun codingHistoryItems(rows: List<CodingChatRow>): List<CodingHistoryItem> = buildList {
...
fun codingChatRows(messages: List<CodingMessage>, hideSystemSteps: Boolean = true,
...
fun CodingStep.isVisibleInChat(hideSystemSteps: Boolean): Boolean = when (kind) {
...
fun CodingDraft.visibleChatContent(hideSystemSteps: Boolean): CodingDraft = copy(
    steps = steps.filter { it.isVisibleInChat(hideSystemSteps) }.toMutableList().apply {
        if (!failedMessage.isNullOrBlank() && none { it.kind == CodingStepKind.ERROR })
            add(CodingStep(CodingStepKind.ERROR, failedMessage.orEmpty(), ok = false))
...
fun codingDraftRow(draft: CodingDraft, messages: List<CodingMessage>, fallbackId: String,
    ...
    val steps = pending.toMutableList().apply {
        if (!saved && !draft.failedMessage.isNullOrBlank() && none { it.kind == CodingStepKind.ERROR })
            add(CodingStep(CodingStepKind.ERROR, draft.failedMessage.orEmpty(), ok = false, id = "draft-error"))
