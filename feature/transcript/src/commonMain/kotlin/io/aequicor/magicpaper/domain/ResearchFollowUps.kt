...
data class ResearchReply(val text: String, val followUps: List<String> = emptyList())

const val RESEARCH_FOLLOW_UPS_MARKER = "<!-- magicpaper:follow-ups"

/** Optional answer metadata, shared by completion, old-history presentation and click validation. */
fun researchReply(text: String, streaming: Boolean = false): ResearchReply {
...
fun ChatMessage.researchReply(): ResearchReply =
...
fun List<TranscriptBlock>.researchContent(streaming: Boolean = false): List<TranscriptBlock> = mapNotNull { block ->
