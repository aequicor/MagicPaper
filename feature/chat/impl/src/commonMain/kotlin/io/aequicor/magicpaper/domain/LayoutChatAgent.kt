...
internal class LayoutChatAgent(private val gateway: LlmGateway, private val editor: LayoutEditor) {
    suspend fun answer(project: CodingProject?, conversationId: String, requestId: String, text: String,
        history: List<ChatMessage>, profile: LlmProfile?, attachments: List<Attachment>): MagicAgent.Answer {
        if (project == null) return MagicAgent.Answer("Выберите проект в разделе «Проекты и код», затем повторите запрос макета.")
        if (profile == null || !profile.configured) return MagicAgent.Answer("Подключите модель в настройках и повторите запрос макета.")
        ...
        try {
            ...
            repeat(3) { index ->
                ...
                if (render.valid && preview != null) {
                    ...
                    return MagicAgent.Answer("Макет сохранён и открыт в Paper Editor для проекта «${project.name}». Можно править его в редакторе или попросить изменения здесь.", attachments = listOf(preview))
                }
            ...
            return MagicAgent.Answer("Не удалось получить корректный макет за три попытки. Последняя сохранённая версия не изменена. Уточните запрос и повторите попытку.")
        } catch (cancelled: CancellationException) {
            AppLog.info("layout-chat", "authoring.cancelled", fields)
            throw cancelled
        } catch (failure: Exception) {
            AppLog.error("layout-chat", "authoring.failed", failure, fields)
            return MagicAgent.Answer((failure as? LayoutEditorException)?.message
