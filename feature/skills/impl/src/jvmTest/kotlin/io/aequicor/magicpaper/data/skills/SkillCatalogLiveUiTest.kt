    ...
    @Test fun searchAndEnteredLinkThroughProjectCatalog() = runTest(timeout = 180.seconds) {
        ...
        try {
            LocalSkillRepository(root, GithubSkillCatalog.HOST).use { repo ->
                val forms = io.aequicor.magicpaper.plugins.builtin.SkillsFormDrafts(io.aequicor.magicpaper.data.storage.InMemoryDraftRepository(), backgroundScope)
                ImageComposeScene(1000, 760) { MagicPaperTheme { Surface { ProjectSkillsPanel(forms) { repo }.Content("catalog-live-project") } } }.use { scene ->
                    var frame = 0L
                    suspend fun pump() {
