...
class ProjectSkillsTest {
    ...
    @Test fun realPanelRendersForProjectRoundTrip() = test { root ->
        ...
        val formScope = CoroutineScope(SupervisorJob() + Dispatchers.Main.immediate)
        try {
            LocalSkillRepository(root, host).use { r ->
                val v = install(r, "1.0.0")
                bind(r, "A", v)
                val project = androidx.compose.runtime.mutableStateOf("A")
                val forms = io.aequicor.magicpaper.plugins.builtin.SkillsFormDrafts(io.aequicor.magicpaper.data.storage.InMemoryDraftRepository(), formScope)
                val panel = io.aequicor.magicpaper.plugins.builtin.ProjectSkillsPanel(forms) { r }
        ...
        } finally { formScope.cancel(); kotlinx.coroutines.Dispatchers.resetMain() }
    }
