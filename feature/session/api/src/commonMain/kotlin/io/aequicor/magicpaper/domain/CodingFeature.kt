 ...
 * platform that can actually run it. Hosts without a native agent bind
 * [UnavailableCodingFeature]; the shell must not reach past this contract, so coding
 * implementation types stay invisible to common code.
 */
interface CodingFeature {
    ...
    /** Answers ordinary chat when this platform's engine hosts it; null leaves chat on HTTP. */
    val chatBackend: ChatBackend? get() = null

    /**
     * A terminal media outcome must reach the tool host that recorded the call.
     * The shell owns the media service, so it forwards the outcome here.
     */
    val onMediaTerminal: suspend (MediaGenerationOwner, String, GeneratedMedia) -> Unit
        get() = { _, _, _ -> }

    /**
     * Orchestration tools are checked twice: once when the catalog is filtered and once at
     * invocation. The default checker lives in a sibling implementation module that a feature
     * implementation may not depend on, so the application binds it through these two members.
     */
    val orchestrationActions: OrchestrationActions get() = OrchestrationActions { _, _, _, _ -> error("Оркестрация недоступна") }
    fun bindOrchestration(orchestration: CustomOrchestration) = Unit
