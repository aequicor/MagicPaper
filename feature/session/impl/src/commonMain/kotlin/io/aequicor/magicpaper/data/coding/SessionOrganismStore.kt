...
class SessionOrganismStore(private val storage: KeyValueStore, private val clock: () -> Long = Id::now) {
    ...
    /** All producers must be quiescent before removing their durable authority records. */
    ...
    suspend fun reconcileInterruptedRun(id: String, sessionId: String, generation: Long, version: Long): SessionOrganism = lock.withLock {
        val old = read(id); val node = old.sessions.getValue(sessionId)
        ...
        require(old.unresolvedQuarantines(sessionId).isEmpty()) { "Сначала подтвердите исход инструментов" }
