...
class ResearchWorkspaceRenderTest {
    ...
    @Test fun galleryKeepsReadingAndActionsVisibleAtNarrowAndLargeTextSizes() {
        ...
        for (case in listOf(Case("reading", 1280, 850), Case("empty", 1280, 850),
            ...
            try {
                ...
                onUi {
                    for (label in listOf("Новый вопрос", "Вопросы (${if (empty) 1 else 2})", "Источники (${if (empty) 0 else 2})",
                        if (case.name == "working") "Пауза" else "Отправить")) {
                        val bounds = scene.text(label).boundsInRoot
                    ...
                    if (case.width >= 980) assertTrue(scene.text("Источники").boundsInRoot.right < input.boundsInRoot.left)
                }
    ...
    @Test fun sourceMenuOffersValidationAndClosingWithoutLosingComposerDraft() {
        ...
        try {
            ...
            onUi { scene.action("Закрыть").config[SemanticsActions.OnClick].action!!.invoke() }
            render()
            onUi {
