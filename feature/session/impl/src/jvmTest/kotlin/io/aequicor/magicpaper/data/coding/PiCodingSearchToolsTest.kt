...
class PiCodingSearchToolsTest {
    ...
    fun `bundled binaries land in the shared bin dir`() {
        ...
        try {
            ...
            assertEquals(
                listOf("fd", "rg"),
                runtime.installBundledSearchTools { name -> bundle[name].asResource(root, name) },
            )
    ...
    fun `reinstall refreshes only changed content`() {
        ...
        try {
            ...
            runtime.installBundledSearchTools { name -> first[name].asResource(root, name) }
            val second = mapOf(fd to "fd-2", rg to "rg-1").mapValues { it.value.toByteArray() }
            assertEquals(
                listOf("fd", "rg"),
                runtime.installBundledSearchTools { name -> second[name].asResource(root, "second-$name") },
            )
    ...
    fun `pi process PATH starts with the shared bin dir`() {
        ...
        try {
            val runtime = PiCodingRuntime(rootDir = root)
            runtime.installBundledSearchTools { name -> "bytes".toByteArray().asResource(root, name) }
    ...
    @Test
    fun `runtime reads the bundle produced by the build`() {
        val root = createTempDirectory("coding-tools").toFile()
        try {
            val runtime = PiCodingRuntime(rootDir = root)
            val bundled = runtime.javaClass.getResource("/coding/tools/$TOOLS_TARGET") != null
            val installed = runtime.installBundledSearchTools()
            if (!bundled) return // Сборка без загрузки бинарников: остаётся системный PATH.
            // Имя каталога и суффикс бинарника должны совпадать с тем, что написал Gradle.
            assertEquals(listOf("fd", "rg"), installed)
            assertTrue(File(root, "bin/${runtime.searchToolFileName("fd")}").length() > 1_000_000)
            assertTrue(File(root, "bin/${runtime.searchToolFileName("rg")}").length() > 1_000_000)
            assertTrue(File(root, "bin/$NOTICES").isFile)
        } finally {
            root.deleteRecursively()
        }
    ...
    /** Ресурс дистрибутива — файл на диске; `null` означает «в сборке этого нет». */
    private fun ByteArray?.asResource(root: File, name: String): URL? {
        if (this == null) return null
        val file = File(root, "bundle/$name")
