...
class ToolCallDiagnosticsTest {
    ...
    @Test fun successfulCallAndNativeEngineCallReportTheirOutcome() = runTest {
        ...
        try {
            ...
            host.questions.respond(request.id, listOf(PlanningAnswer("a", selected = listOf("x"))))
            call.await()
            ...
            assertTrue(tools.executor.recordNative(context, native))
            assertTrue(tools.executor.recordNative(context, native.copy(phase = ToolPhase.FAILED, result = "exit 1")))
