...
class SessionHistoryDeletionTest {
    @Test fun deletionCleanupFailureIsReplayedFromTheTombstoneForEveryDescendantAndImmunity() = runTest {
        val f = fixture(); f.create("child")
        val expected = f.store.get(f.root.organismId!!).sessions.keys
        f.service.beforeDeleteSession = { _, _ -> error("Reference cleanup interrupted") }
        assertFailsWith<IllegalStateException> { f.service.deleteHistory(f.project.id, f.root.id) }
        assertTrue(f.projects.sessions(f.project.id).isNotEmpty())
        val cleaned = mutableSetOf<String>()
        val recovered = restarted(f)
        recovered.beforeDeleteSession = { project, id -> assertEquals(f.project.id, project); cleaned += id }
    ...
    private suspend fun fixture(): SessionOrganismTestFixture = SessionOrganismTestFixture().also {
        it.initialize(CodingInteractionMode.PLANNING); it.service.startChild = { _, _ -> }; installConfirmedStop(it)
    }
