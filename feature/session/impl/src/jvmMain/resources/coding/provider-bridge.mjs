...
if (process.argv[1] && import.meta.url === pathToFileURL(realpathSync(process.argv[1])).href) main().catch(error => {
  // Only fixed categories cross the process boundary; configuration and error messages may contain secrets.
  const code = ['ERR_MODULE_NOT_FOUND', 'ERR_PACKAGE_PATH_NOT_EXPORTED', 'EADDRINUSE', 'EACCES'].includes(error?.code)
    ? error.code : error instanceof SyntaxError ? 'INVALID_CONFIGURATION' : 'STARTUP_FAILED';
