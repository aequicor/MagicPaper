...
fun NewCodingSessionDialog(engine: CodingEngine, onEngineChange: (CodingEngine) -> Unit,
    ...
    PaperDialog("Новая сессия", onDismiss, confirmLabel = "Создать сессию", onConfirm = onCreate,
        ...
        Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            EngineChoices(engine, enabled = loaded && !busy, onSelect = onEngineChange)
...
@Composable
internal fun EngineChoices(selected: CodingEngine, enabled: Boolean = true, onSelect: (CodingEngine) -> Unit) {
    FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        CodingEngine.entries.forEach { engine -> PaperChoice(selected == engine, { onSelect(engine) }, engine.title, enabled = enabled) }
    }
}
