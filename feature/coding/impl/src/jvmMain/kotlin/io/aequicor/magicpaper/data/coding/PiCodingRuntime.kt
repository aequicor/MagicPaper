...
class PiCodingRuntime(
    ...
    override suspend fun reconcile(sessionId: String) = withContext(Dispatchers.IO) {
        ownedProcesses.reconcile(sessionId)
        migrateGeneratedModelCredentials(sessionHome(sessionId))
        // Other active/owned processes can still be using their old configuration.
        if (File(root, "owned-processes").listFiles().orEmpty().none { it.extension == "process" }) {
            migrateGeneratedModelCredentials(pihome)
            File(root, "session-configs").listFiles().orEmpty().filter { it.isDirectory }
                .forEach(::migrateGeneratedModelCredentials)
        }
    }
        ...
        try {
            ...
            process = ProcessBuilder(args)
                ...
                .apply {
                    ...
                    environment().remove(PiModelsConfig.API_KEY_ENV)
                    environment().putAll(PiModelsConfig.environment(profile))
    ...
    /** Only called after process ownership was reconciled; never touches external ~/.pi. */
    private fun migrateGeneratedModelCredentials(home: File) {
        val file = File(home, "models.json")
        if (!file.isFile) return
        val document = kotlinx.serialization.json.Json.parseToJsonElement(file.readText()) as kotlinx.serialization.json.JsonObject
        val providers = document["providers"] as? kotlinx.serialization.json.JsonObject ?: return
        val provider = providers[PiModelsConfig.PROVIDER_ID] as? kotlinx.serialization.json.JsonObject ?: return
        val reference = if ((provider["api"] as? kotlinx.serialization.json.JsonPrimitive)?.content == "openai-codex-responses")
            "managed-by-magicpaper" else PiModelsConfig.API_KEY_REFERENCE
        if ((provider["apiKey"] as? kotlinx.serialization.json.JsonPrimitive)?.content == reference) return
        val replaced = kotlinx.serialization.json.JsonObject(provider + ("apiKey" to kotlinx.serialization.json.JsonPrimitive(reference)))
        writeAtomically(file, kotlinx.serialization.json.JsonObject(document + ("providers" to
