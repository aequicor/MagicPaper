        ...
        suspend fun start(node: File, script: File, library: File, profile: LlmProfile): CodexProviderBridge = withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val ready = withTimeout(20_000) {
                    while (!stdout.ready()) { check(child.isAlive) {
                        val code = child.errorStream.bufferedReader().readLine()?.takeIf {
                            it in setOf("ERR_MODULE_NOT_FOUND", "ERR_PACKAGE_PATH_NOT_EXPORTED", "EADDRINUSE", "EACCES", "INVALID_CONFIGURATION", "STARTUP_FAILED")
                        } ?: "STARTUP_FAILED"
                        "Адаптер провайдера не запустился ($code)"
                    }; delay(25) }
