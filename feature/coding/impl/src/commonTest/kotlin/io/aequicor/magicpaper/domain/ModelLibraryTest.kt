...
class ModelLibraryTest {
    ...
    @Test fun forkKeepsIdentityButSendsProviderModelAndCustomOptions() {
        ...
        assertEquals(16384, PiModelsConfig.maxTokens(request), "Registry advertises provider capacity; per-request options carry the user limit")
    ...
    @Test fun missingProviderContextRetainsConfiguredLimit() {
        // An omitted provider fact is unknown; a family name is not evidence of capacity.
        ...
        assertEquals(profile.advanced.safeContextLimit, options.contextLimit)
    }

    @Test fun forModelDoesNotInventAnUndeclaredFamilyContext() {
        val dashFact = ProviderModel("qwen3.7-plus", "Qwen3.7 Plus", contextWindow = null, maxOutputTokens = null)
        val profile = LlmProfile("d", "DashScope", baseUrl = "https://dashscope.aliyuncs.com/compatible-mode/v1",
            modelId = "qwen3.7-plus", provider = ProviderType.OPENAI_COMPATIBLE,
            modelCatalog = listOf(dashFact), modelLibraryVersion = 1)
        val request = profile.forModel("qwen3.7-plus")
        assertEquals(profile.advanced.safeContextLimit, request.advanced.contextLimit)
    }
