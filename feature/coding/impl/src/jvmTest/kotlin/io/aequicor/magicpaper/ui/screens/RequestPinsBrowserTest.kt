...
class RequestPinsBrowserTest {
    ...
    @Test fun messageButtonOpensItsOwnPinAndSelectingAnotherPinRevealsTheSourceInBothChats() {
        for (coding in listOf(true)) Chat(coding).use { chat ->
    ...
    @Test fun manyPinsFitOnNarrowScreensWithLargeTextAndKeepTheListLazy() {
        for (coding in listOf(true)) Chat(coding, 360, 1.4f, 202).use { chat ->
    ...
    @Test fun pinsReserveTextSpaceWithoutAddingAButtonRowAndRestoreTheSameLayout() {
        for (coding in listOf(true)) for (fontScale in listOf(1f, 1.4f)) Chat(coding, 360, fontScale).use { chat ->
