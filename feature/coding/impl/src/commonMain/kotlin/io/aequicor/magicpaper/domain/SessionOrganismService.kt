...
class SessionOrganismService(
    ...
    /** Application-owned references (pins, drafts, unread state) share tombstone replay. */
    var beforeDeleteSession: suspend (projectId: String, sessionId: String) -> Unit = { _, _ -> }

    private suspend fun deleteProjection(projectId: String, sessionId: String) {
        beforeDeleteSession(projectId, sessionId)
        projects.deleteSession(projectId, sessionId)
    }
    ...
    suspend fun execute(context: ToolExecutionContext, operation: String, tool: String, arguments: JsonObject): JsonElement {
        ...
        val command = when (tool) {
            "session.create" -> json.decodeFromJsonElement<SessionCreateArgs>(arguments).let { args ->
                ...
                requireTool((args.mode ?: context.mode) != CodingInteractionMode.CODE || canCreateCodeChild(session)) { "Дочерняя разработка требует выделенной рабочей копии" }
