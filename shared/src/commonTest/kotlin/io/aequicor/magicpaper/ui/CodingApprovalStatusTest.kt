...
class CodingApprovalStatusTest {
    @Test fun pendingApprovalTakesPriorityOverRunningSessionAndProject() {
        ...
        val ui = CodingUi(sessions = listOf(CodingSessionUi(session, running = true),
            CodingSessionUi(session.copy(id = "other"), running = true)), approvals = listOf(request))
        ...
        assertEquals(CodingSessionStatus.WORKING, ui.copy(approvals = emptyList()).sessionsOf("p").first().status)
    ...
    @Test fun backgroundMergeIsVisibleInWorkerAndProjectStatus() {
        ...
        val ui = CodingUi(sessions = listOf(CodingSessionUi(session)), approvals = listOf(request))
