...
class SkillReleaseStore(
    ...
    suspend fun forget(keys: Set<String>): SkillReleaseSnapshot = mutex.withLock {
        require(state.projects.values.none { pins -> pins.keys.any { it in keys } }) { "Disconnect project bindings before deleting releases" }
    ...
    suspend fun bindProject(projectId: String, checksums: Map<String, String>, consent: SkillActivationConsent): SkillReleaseSnapshot = mutex.withLock {
        ...
        val releases = checksums.map { (key, hash) -> state.installed.getValue(key).also {
            require(it.pkg.checksum == hash && it.status == SkillCandidateStatus.VERIFIED && it.improvement?.passed != false) { "Invalid project release" }
        } }
