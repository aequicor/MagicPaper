    ...
    suspend fun refine(plan: Plan, message: String, planner: LlmProfile?, profiles: List<LlmProfile>, dossiers: List<ModelDossier>, searchContext: String = "", onActivity: (CodingStep) -> Unit = {}): Plan {
        ...
        val messages = mutableListOf(
            LlmMessage(LlmChatRole.SYSTEM, """
                ...
                Для оценок этапов и вариантов используй шкалу:
                ...
                isFinalization обозначает выбранный методикой итоговый этап; он не обязателен для каждого плана. Зависимость от CHOICE следует выбранному варианту; не перечисляй этапы неактивных ветвей.
                ...
                ${acceptanceChecks.executionGuidance()}
                ...
                isolatedWorkspace=true запрашивает отдельную рабочую копию в рамках методики и разрешений пользователя. Поле не расширяет разрешения режима.
