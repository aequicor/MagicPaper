...
sealed interface PlanRevisionEvent : PlanEvent {
    data class RequestStarted(val id: String, val text: String, val nodeId: String?) : PlanRevisionEvent
    data class RequestCleared(val requestId: String) : PlanRevisionEvent
    data class DialogueAppended(val messages: List<PlanningMessage>) : PlanRevisionEvent
    data class ProposalDeclined(val expectedRevision: Long, val proposalId: String) : PlanRevisionEvent
    data class ProposalPrepared(val base: Plan, val effective: Plan, val result: Plan, val assistant: PlanningMessage) : PlanRevisionEvent
    data class ProposalApplied(val base: Plan, val result: Plan) : PlanRevisionEvent
    data class RefinementFinished(val base: Plan, val result: Plan, val selection: ModelSelection?, val search: SearchProvider, val at: Long) : PlanRevisionEvent
    data class ProposalApproved(val proposal: PlanProposal, val expectedRevision: Long?, val openQuestions: Boolean,
        val pausedStages: Set<String>, val newRunId: String, val worktreeEnabled: Boolean, val at: Long) : PlanRevisionEvent
    data class InitialConfirmed(val expectedRevision: Long?, val openQuestions: Boolean, val newRunId: String, val at: Long) : PlanRevisionEvent
    data class AssignmentsRecovered(val base: Plan, val candidate: Plan) : PlanRevisionEvent
    data class StageNumbersBound(val numbers: Map<String, Int>) : PlanRevisionEvent
    data class StageRenamed(val id: String?, val name: String) : PlanRevisionEvent
    data class LegacyLinked(val parent: String, val at: Long) : PlanRevisionEvent
    data class LegacyQuestionsObserved(val questions: List<OrchestrationQuestion>) : PlanRevisionEvent
    data class LegacyPeersObserved(val peers: List<Plan>, val admitted: Map<String, SessionLegacyAttempt>) : PlanRevisionEvent
    data class EngineRestored(val engine: CodingEngine) : PlanRevisionEvent
    data class QuestionDelivered(val stages: List<String>) : PlanRevisionEvent
}
