...
fun stageReplyOrNull(text: String): StageReply? = runCatching {
...
fun StageAttempt.effectiveChatTurns(): List<StageChatTurn> {
...
fun StageAttempt.chatResponseId(index: Int): String =
...
fun StageAttempt.chatResponses(parentMessages: List<CodingMessage> = emptyList()): List<CodingMessage> {
    val activity = steps.filter { it.isVisibleActivity }
...
fun List<CodingMessage>.withStageResponses(responses: List<CodingMessage>): List<CodingMessage> {
