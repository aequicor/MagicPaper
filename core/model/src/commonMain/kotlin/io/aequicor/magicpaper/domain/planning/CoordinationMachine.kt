...
sealed interface CoordinationEvent : PlanEvent {
    data class InboxPrepared(val stageId: String, val attemptId: String, val turnIndex: Int) : CoordinationEvent
    data class InboxDelivered(val attemptId: String, val turnIndex: Int) : CoordinationEvent
    data class InboxAnswered(val attemptId: String, val turnIndex: Int) : CoordinationEvent
    data class HandoffRecorded(val record: CoordinationRecord) : CoordinationEvent
    data class HandoffSubmitted(val record: CoordinationRecord) : CoordinationEvent
    data class ScheduledDeliveryRequested(val rule: ScheduledMessage, val parent: String) : CoordinationEvent
    data class HandoffStatusRecorded(val id: String, val status: HandoffStatus, val text: String) : CoordinationEvent
    data class DecisionRecorded(val id: String, val decision: CoordinatorReply, val activity: List<CodingStep>, val replacesResult: Boolean) : CoordinationEvent
    data class DeliveryRequested(val delivery: PlanDelivery, val finalizationId: String, val retryCheckpoint: Plan? = null) : CoordinationEvent
    data class LateDeliveryObserved(val id: String, val finalizationId: String) : CoordinationEvent
    data class QuestionsObserved(val events: List<MessageEvent>, val ids: Set<String>) : CoordinationEvent
}
