...
fun CodingStep.planningPreview(): CodingStep =
...
fun CodingStep.inPlanningCall(call: String): CodingStep = copy(
    id = "$call:${id.ifBlank { "$kind:$callId" }}",
    callId = "$call:$callId",
)

/** Replace one provider fragment in place; hidden activity must never replace the whole answer. */
fun List<CodingStep>.withPlanningActivity(raw: CodingStep): List<CodingStep> {
...
fun CodingMessage.withPlanningDraft(draft: CodingDraft?): CodingMessage {
