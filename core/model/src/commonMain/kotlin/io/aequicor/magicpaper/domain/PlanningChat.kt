...
fun Plan.coordinatorResultProblem(eventId: String, decision: CoordinatorReply): String? {
    val record = coordination.firstOrNull { it.id == eventId } ?: return null
    decision.resultProblem(record)?.let { return it }
    return null
}

fun CoordinationRecord.actionOrigin(): String = if (actionRevision == 0) id else "$id-revision-$actionRevision"

/** A consumed user answer starts a new recovery window; queued answers do not. */
fun Plan.blockedTurnCount(stageId: String, attempt: StageAttempt): Int {
    var count = 0
...
fun Plan.stageRecords(stageId: String, attempt: StageAttempt): List<CoordinationRecord> = coordination.filter {
...
fun List<CoordinationRecord>.evidenceText(): String = joinToString("\n\n") { record ->
    buildString {
...
fun Plan.stageVerificationReport(stageId: String, attempt: StageAttempt): String {
...
fun StageAttempt.verificationToolEvidence(): String {
...
fun Plan.handoffForDisplay(record: CoordinationRecord): CoordinationRecord {
