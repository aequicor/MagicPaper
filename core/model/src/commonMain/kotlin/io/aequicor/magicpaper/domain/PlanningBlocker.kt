...
data class PlanningBlocker(
    ...
    val text: String get() = "$title.\n\n${if (issue.retryBlocked) issue.message else acceptance?.userSummary() ?: issue.message}"
}

fun List<PlanningBlocker>.recoveryActionLabel(): String = when {
...
fun Plan.blockingIssues(history: List<CodingMessage>): List<PlanningBlocker> {
