...
fun uniqueSchedulingId(used: Set<String>, generate: () -> String = Id::uuid): String {
    repeat(100) { val id = generate(); if (id.isNotBlank() && id !in used) return id }
    error("Не удалось выдать уникальный идентификатор")
}

fun Plan.taskSessionId(taskId: String): String = milestones.first { it.id == taskId }
    .attempts.firstOrNull()?.sessionId ?: "plan-$id-stage-$taskId"

fun MessageTrigger.normalized(now: Long): MessageTrigger {
...
fun MessageTrigger.validate(plan: Plan, questionIds: Set<String>) {
    when (kind) {
...
fun Plan.applyScheduleCommands(
...
fun Plan.waitCycleProblem(): String? {
...
fun Plan.advanceScheduledMessages(now: Long): Plan {
...
fun MessageEventKind.label(): String = when (this) {
...
fun ScheduledMessageStatus.label(): String = when (this) {
