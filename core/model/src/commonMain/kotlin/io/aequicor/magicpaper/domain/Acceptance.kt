...
fun EvidenceEnvironment.label(): String = when (this) {
...
@Serializable data class AcceptanceRecord(
    ...
    val permitsProgress: Boolean get() = status in setOf(AcceptanceStatus.ACCEPTED, AcceptanceStatus.ACCEPTED_WITH_SKIPS)
    ...
    val allChecksSkippedByUser: Boolean get() = criteria.isNotEmpty() && criteria.all(::wasSkippedByUser)
    ...
    val canRetryWithWorker: Boolean get() = status in setOf(AcceptanceStatus.FAILED, AcceptanceStatus.PARTIAL) &&
    ...
    fun automaticRepairProblem(previous: AcceptanceRecord?): String? {
...
fun Milestone.criteria(): List<AcceptanceCriterion> = acceptanceCriteria.ifEmpty {
    listOf(AcceptanceCriterion("result", acceptance.ifBlank { description }.ifBlank { title }))
}.map { it.copy(id = "$id/${it.id}") }

fun Plan.acceptanceCriteria(): List<AcceptanceCriterion> = selectedMilestones.flatMap { it.criteria() }
