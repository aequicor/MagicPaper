...
class StageMachineTest {
    ...
    @Test fun explicitInputsProduceTheSameCompleteTransitionTrace() {
        val events = listOf(StageEvent.WorkerStarting("implement", 10),
        ...
        fun replay(): List<StageTransition> {
            var state = initial.toState()
            return events.map { reduce(state, it).also { transition -> state = transition.state } }
        }
        val trace = replay()
