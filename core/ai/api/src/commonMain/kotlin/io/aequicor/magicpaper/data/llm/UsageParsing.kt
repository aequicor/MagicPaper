...
fun JsonObject.count(key: String): Long? = (get(key) as? JsonPrimitive)?.longOrNull?.takeIf { it >= 0 }
...
object UsageParsing {
    /** Shared price metadata parser for provider directories and runtime adapters. */
    fun modelPricing(rates: JsonObject): ModelPricing = ModelPricing(rates.price("prompt"), rates.price("completion"), rates.price("input_cache_read"), rates.price("input_cache_write"))
