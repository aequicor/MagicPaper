...
internal class CodexQuestionnaireBroker(
    ...
    override fun receive(id: JsonPrimitive, method: String, params: JsonObject, session: CodingSession,
        ...
        val job = scope.launch(start = CoroutineStart.LAZY) {
            try {
                ...
                val attemptId = registry.beginDelivery(requestId)
                var deliveryFailure: Throwable? = null
                try { reply(buildJsonObject { put("id", id); put("result", buildJsonObject {
                    ...
                    // A pipe write (and serverRequest/resolved without a reason) proves no acceptance.
                    withContext(NonCancellable) {
                        try { registry.finishDelivery(requestId, attemptId, QuestionnaireDeliveryOutcome.UNKNOWN) }
                        catch (failure: Exception) {
                            if (deliveryFailure != null) deliveryFailure.addSuppressed(failure) else throw failure
                        }
                    }
                }
    ...
    override fun contains(threadId: String, id: JsonPrimitive) = pending.containsKey(Key(threadId, id))
    override fun resolved(threadId: String, id: JsonElement) = remove { key, _ -> key.threadId == threadId && key.wireId == id }
    override fun clearTurn(threadId: String, turnId: String?) = remove { key, p -> key.threadId == threadId && (turnId == null || p.turnId == turnId) }
    override fun completeItem(threadId: String, itemId: String) = remove { key, p -> key.threadId == threadId && p.itemId == itemId }
    override fun clear() { remove { _, _ -> true }; received.clear() }
