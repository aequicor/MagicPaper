...
class CodexNativeFailureTest {
    ...
    /** Shape of `GetAccountRateLimitsResponse` generated by codex-cli 0.155. */
    @Test fun limitReadCarriesWindowLengthsAndAReachedLimit() = runBlocking {
        RpcFixture { method ->
            when (method) {
                "account/read" -> account()
                "account/rateLimits/read" -> Json.parseToJsonElement("""{"rateLimits":{},"rateLimitsByLimitId":{
                    "codex":{"limitId":"codex","primary":{"usedPercent":100,"windowDurationMins":300,"resetsAt":1790269200},
                        "secondary":{"usedPercent":12,"windowDurationMins":10080,"resetsAt":1790856000},
                        "rateLimitReachedType":"rate_limit_reached","planType":"pro"}},"ordinaryUsageAllowed":false}""")
                else -> error(method)
            }
        }.use { fixture ->
            val account = fixture.client.account()
            assertTrue(account.limitReached)
            assertEquals(listOf(300L, 10_080L), account.rateLimits.map { it.durationMinutes })
            assertEquals(listOf(100, 12), account.rateLimits.map { it.usedPercent })
        }
    }
