...
class SubscriptionTokenBrokerTest {
    @Test fun authenticatesRequestsAndRefreshesOnlyOnAuthorizedCalls() {
        ...
        NativeSubscriptionTokenBroker(NativeAuthTokens { "fixture-${count.incrementAndGet()}" }, NativeDiagnostics { _, _, cause, _ -> throw AssertionError(cause) }).use { broker ->
    ...
    @Test fun refreshFailureDoesNotExposeCredentialDetails() {
        NativeSubscriptionTokenBroker(NativeAuthTokens { error("secret-refresh-token") }, NativeDiagnostics { _, _, cause, _ -> assertFalse(cause.toString().contains("secret-refresh-token")) }).use { broker ->
